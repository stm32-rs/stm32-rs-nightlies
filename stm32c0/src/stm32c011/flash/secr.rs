...
pub type SEC_SIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 5>;
...
impl R {
    ///Bits 0:4 - Securable memory area size Contains the number of securable flash memory pages. Note: The number of effective bits depends on the size of the flash memory in the device.
    #[inline(always)]
    pub fn sec_size(&self) -> SEC_SIZE_R {
        SEC_SIZE_R::new((self.bits & 0x1f) as u8)
    }
