...
///Field `PU6` reader - Port B pull-up bit i (i = 15 to 0) Setting PUi bit while the corresponding PDi bit is zero and the APC bit of the PWR_CR3 register is set activates a pull-up device on the PB\[i\] I/O. On STM32C011xx, only PU7 and PU6 are available
pub type PU6_R = crate::BitReader;
///Field `PU6` writer - Port B pull-up bit i (i = 15 to 0) Setting PUi bit while the corresponding PDi bit is zero and the APC bit of the PWR_CR3 register is set activates a pull-up device on the PB\[i\] I/O. On STM32C011xx, only PU7 and PU6 are available
pub type PU6_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `PU7` reader - Port B pull-up bit i (i = 15 to 0) Setting PUi bit while the corresponding PDi bit is zero and the APC bit of the PWR_CR3 register is set activates a pull-up device on the PB\[i\] I/O. On STM32C011xx, only PU7 and PU6 are available
pub type PU7_R = crate::BitReader;
///Field `PU7` writer - Port B pull-up bit i (i = 15 to 0) Setting PUi bit while the corresponding PDi bit is zero and the APC bit of the PWR_CR3 register is set activates a pull-up device on the PB\[i\] I/O. On STM32C011xx, only PU7 and PU6 are available
