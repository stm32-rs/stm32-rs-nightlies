...
pub enum TI1S {
    ...
    B0x0 = 0,
    ///1: The TIMx_CH1, CH2 and CH3 pins are connected to the TI1 input (XOR combination) See also Section 17.3.25: Interfacing with Hall sensors on page 380
    B0x1 = 1,
}
...
impl TI1S_R {
    ...
    ///The TIMx_CH1, CH2 and CH3 pins are connected to the TI1 input (XOR combination) See also Section 17.3.25: Interfacing with Hall sensors on page 380
    #[inline(always)]
    pub fn is_b_0x1(&self) -> bool {
        *self == TI1S::B0x1
    }
...
impl W {
    ///Bit 3 - Capture/compare DMA selection
    #[inline(always)]
    pub fn ccds(&mut self) -> CCDS_W<'_, TIM3_CR2rs> {
        CCDS_W::new(self, 3)
    }
    ///Bits 4:6 - Master mode selection These bits permit to select the information to be sent in master mode to slave timers for synchronization (TRGO). The combination is as follows: When the Counter Enable signal is controlled by the trigger input, there is a delay on TRGO, except if the master/slave mode is selected (see the MSM bit description in TIMx_SMCR register). Note: The clock of the slave timer or ADC must be enabled prior to receive events from the master timer, and must not be changed on-the-fly while triggers are received from the master timer.
    #[inline(always)]
    pub fn mms(&mut self) -> MMS_W<'_, TIM3_CR2rs> {
        MMS_W::new(self, 4)
    }
    ///Bit 7 - TI1 selection
    #[inline(always)]
    pub fn ti1s(&mut self) -> TI1S_W<'_, TIM3_CR2rs> {
        TI1S_W::new(self, 7)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32C051.html#TIM3:TIM3_CR2)*/
