...
impl W {
    ///Bits 0:6 - Nominal time segment after sample point
    #[inline(always)]
    pub fn ntseg2(&mut self) -> NTSEG2_W<'_, FDCAN_NBTPrs> {
        NTSEG2_W::new(self, 0)
    }
    ///Bits 8:15 - Nominal time segment before sample point
    #[inline(always)]
    pub fn ntseg1(&mut self) -> NTSEG1_W<'_, FDCAN_NBTPrs> {
        NTSEG1_W::new(self, 8)
    }
    ///Bits 16:24 - Bit rate prescaler
    #[inline(always)]
    pub fn nbrp(&mut self) -> NBRP_W<'_, FDCAN_NBTPrs> {
        NBRP_W::new(self, 16)
    }
    ///Bits 25:31 - Nominal (re)synchronization jump width
    #[inline(always)]
    pub fn nsjw(&mut self) -> NSJW_W<'_, FDCAN_NBTPrs> {
        NSJW_W::new(self, 25)
    }
}
