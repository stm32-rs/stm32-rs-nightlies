...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum URS {
    ///0: Any of the following events generate an update interrupt if enabled.
    B0x0 = 0,
...
#[repr(u8)]
pub enum CKD {
    ///0: tless thansub>DTS less than/sub>= tless thansub>CK_INTless than/sub>
    B0x0 = 0,
    ///1: tless thansub>DTSless than/sub> = 2*tless thansub>CK_INTless than/sub>
    B0x1 = 1,
    ///2: tless thansub>DTSless than/sub> = 4*tless thansub>CK_INTless than/sub>
    B0x2 = 2,
}
...
impl CKD_R {
    ...
    ///tless thansub>DTS less than/sub>= tless thansub>CK_INTless than/sub>
    #[inline(always)]
    pub fn is_b_0x0(&self) -> bool {
        *self == CKD::B0x0
    }
    ///tless thansub>DTSless than/sub> = 2*tless thansub>CK_INTless than/sub>
    #[inline(always)]
    pub fn is_b_0x1(&self) -> bool {
        *self == CKD::B0x1
    }
    ///tless thansub>DTSless than/sub> = 4*tless thansub>CK_INTless than/sub>
    #[inline(always)]
...
impl W {
    ///Bit 0 - Counter enable
    #[inline(always)]
    pub fn cen(&mut self) -> CEN_W<'_, TIM15_CR1rs> {
        CEN_W::new(self, 0)
    }
    ///Bit 1 - Update disable
    #[inline(always)]
    pub fn udis(&mut self) -> UDIS_W<'_, TIM15_CR1rs> {
        UDIS_W::new(self, 1)
    }
    ///Bit 2 - Update request source
    #[inline(always)]
    pub fn urs(&mut self) -> URS_W<'_, TIM15_CR1rs> {
        URS_W::new(self, 2)
    }
    ///Bit 3 - One-pulse mode
    #[inline(always)]
    pub fn opm(&mut self) -> OPM_W<'_, TIM15_CR1rs> {
        OPM_W::new(self, 3)
    }
    ///Bit 7 - Auto-reload preload enable
    #[inline(always)]
    pub fn arpe(&mut self) -> ARPE_W<'_, TIM15_CR1rs> {
        ARPE_W::new(self, 7)
    }
    ///Bits 8:9 - Clock division
    #[inline(always)]
    pub fn ckd(&mut self) -> CKD_W<'_, TIM15_CR1rs> {
        CKD_W::new(self, 8)
    }
    ///Bit 11 - UIF status bit remapping
    #[inline(always)]
    pub fn uifremap(&mut self) -> UIFREMAP_W<'_, TIM15_CR1rs> {
