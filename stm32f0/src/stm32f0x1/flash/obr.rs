...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum N_BOOT0 {
    ///0: When BOOT_SEL is cleared, select the device boot mode
    Disabled = 0,
    ///1: When BOOT_SEL is cleared, select the device boot mode
    Enabled = 1,
}
...
impl RAM_PARITY_CHECK_R {
    ...
    pub const fn variant(&self) -> RAM_PARITY_CHECK {
        match self.bits {
            false => RAM_PARITY_CHECK::Enabled,
            true => RAM_PARITY_CHECK::Disabled,
        }
    }
    ///RAM parity check enabled
    #[inline(always)]
    pub fn is_enabled(&self) -> bool {
        *self == RAM_PARITY_CHECK::Enabled
    }
    ///RAM parity check disabled
    #[inline(always)]
    pub fn is_disabled(&self) -> bool {
        *self == RAM_PARITY_CHECK::Disabled
    }
}
/**BOOT_SEL
...
impl From<BOOT_SEL> for bool {
    #[inline(always)]
    fn from(variant: BOOT_SEL) -> Self {
        variant as u8 != 0
    }
}
///Field `BOOT_SEL` reader - BOOT_SEL
pub type BOOT_SEL_R = crate::BitReader<BOOT_SEL>;
impl BOOT_SEL_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> BOOT_SEL {
        match self.bits {
            false => BOOT_SEL::NBoot0,
            true => BOOT_SEL::Boot0,
        }
    }
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F0x1.html#Flash:OBR)*/
pub struct OBRrs;
...
impl crate::Resettable for OBRrs {
    const RESET_VALUE: u32 = 0x03ff_fff2;
}
