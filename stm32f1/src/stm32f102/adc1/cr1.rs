...
pub enum DUALMOD {
    ...
    ///3: Combined injected simultaneous + fast interleaved mode
    InjectedFastInterleaved = 3,
    ///4: Combined injected simultaneous + slow interleaved mode
    InjectedSlowInterleaved = 4,
    ///5: Injected simultaneous mode only
    Injected = 5,
    ///6: Regular simultaneous mode only
    Regular = 6,
    ///7: Fast interleaved mode only
    FastInterleaved = 7,
    ///8: Slow interleaved mode only
    SlowInterleaved = 8,
...
impl W {
    ...
    ///Bits 16:19 - Dual mode selection
    #[inline(always)]
    pub fn dualmod(&mut self) -> DUALMOD_W<'_, CR1rs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F102.html#ADC1:CR1)*/
pub struct CR1rs;
