...
///Field `MR(0-19)` reader - Interrupt Mask on line %s
...
///Field `MR(0-19)` writer - Interrupt Mask on line %s
...
impl R {
    ...
    pub fn mr_iter(&self) -> impl Iterator<Item = MR_R> + '_ {
        (0..20).map(move |n| MR_R::new(((self.bits >> n) & 1) != 0))
    }
