...
///Field `SWIER(0-19)` reader - Software Interrupt on line %s
...
///Field `SWIER(0-19)` writer - Software Interrupt on line %s
...
impl R {
    ///Software Interrupt on line (0-19)
    ///
    ...
    pub fn swier_iter(&self) -> impl Iterator<Item = SWIER_R> + '_ {
        (0..20).map(move |n| SWIER_R::new(((self.bits >> n) & 1) != 0))
    }
