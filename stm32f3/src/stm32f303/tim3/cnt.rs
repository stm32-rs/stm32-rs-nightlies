...
///Field `UIFCPY` reader - if IUFREMAP=0 than CNT with read write access else UIFCPY with read only access
pub type UIFCPY_R = crate::BitReader;
///Field `UIFCPY` writer - if IUFREMAP=0 than CNT with read write access else UIFCPY with read only access
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F303.html#TIM3:CNT)*/
pub struct CNTrs;
