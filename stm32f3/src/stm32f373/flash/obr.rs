...
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum SRAM_PARITY_CHECK {
    ///0: RAM parity check disabled
    Disabled = 0,
    ///1: RAM parity check enabled
    Enabled = 1,
}
impl From<SRAM_PARITY_CHECK> for bool {
    #[inline(always)]
    fn from(variant: SRAM_PARITY_CHECK) -> Self {
        variant as u8 != 0
    }
}
///Field `SRAM_PARITY_CHECK` reader - SRAM_PARITY_CHECK
pub type SRAM_PARITY_CHECK_R = crate::BitReader<SRAM_PARITY_CHECK>;
impl SRAM_PARITY_CHECK_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> SRAM_PARITY_CHECK {
        match self.bits {
            false => SRAM_PARITY_CHECK::Disabled,
            true => SRAM_PARITY_CHECK::Enabled,
        }
    }
    ///RAM parity check disabled
    #[inline(always)]
    pub fn is_disabled(&self) -> bool {
        *self == SRAM_PARITY_CHECK::Disabled
    }
    ///RAM parity check enabled
    #[inline(always)]
    pub fn is_enabled(&self) -> bool {
        *self == SRAM_PARITY_CHECK::Enabled
    }
}
/**SDADC12_VDD_MONITOR
...
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum SDADC12_VDD_MONITOR {
    ///0: VDDSD12 monitoring disabled
    Disabled = 0,
    ///1: VDDSD12 monitoring enabled
    Enabled = 1,
}
impl From<SDADC12_VDD_MONITOR> for bool {
    #[inline(always)]
    fn from(variant: SDADC12_VDD_MONITOR) -> Self {
        variant as u8 != 0
    }
}
///Field `SDADC12_VDD_MONITOR` reader - SDADC12_VDD_MONITOR
pub type SDADC12_VDD_MONITOR_R = crate::BitReader<SDADC12_VDD_MONITOR>;
impl SDADC12_VDD_MONITOR_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> SDADC12_VDD_MONITOR {
        match self.bits {
            false => SDADC12_VDD_MONITOR::Disabled,
            true => SDADC12_VDD_MONITOR::Enabled,
        }
    }
    ///VDDSD12 monitoring disabled
    #[inline(always)]
    pub fn is_disabled(&self) -> bool {
        *self == SDADC12_VDD_MONITOR::Disabled
    }
    ///VDDSD12 monitoring enabled
    #[inline(always)]
    pub fn is_enabled(&self) -> bool {
        *self == SDADC12_VDD_MONITOR::Enabled
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F373.html#FLASH:OBR)*/
pub struct OBRrs;
