...
#[cfg_attr(feature = "defmt", derive(defmt::Format))]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum DAC_TRIG_RMP {
    ///0: Not remapped
    NotRemapped = 0,
    ///1: DAC trigger is TIM3_TRGO
    Remapped = 1,
}
impl From<DAC_TRIG_RMP> for bool {
    #[inline(always)]
    fn from(variant: DAC_TRIG_RMP) -> Self {
        variant as u8 != 0
    }
}
///Field `DAC_TRIG_RMP` reader - DAC trigger remap (when TSEL = 001)
pub type DAC_TRIG_RMP_R = crate::BitReader<DAC_TRIG_RMP>;
impl DAC_TRIG_RMP_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> DAC_TRIG_RMP {
        match self.bits {
            false => DAC_TRIG_RMP::NotRemapped,
            true => DAC_TRIG_RMP::Remapped,
        }
    }
    ///Not remapped
    #[inline(always)]
    pub fn is_not_remapped(&self) -> bool {
        *self == DAC_TRIG_RMP::NotRemapped
    }
    ///DAC trigger is TIM3_TRGO
    #[inline(always)]
    pub fn is_remapped(&self) -> bool {
        *self == DAC_TRIG_RMP::Remapped
    }
}
///Field `DAC_TRIG_RMP` writer - DAC trigger remap (when TSEL = 001)
pub type DAC_TRIG_RMP_W<'a, REG> = crate::BitWriter<'a, REG, DAC_TRIG_RMP>;
impl<'a, REG> DAC_TRIG_RMP_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Not remapped
    #[inline(always)]
    pub fn not_remapped(self) -> &'a mut crate::W<REG> {
        self.variant(DAC_TRIG_RMP::NotRemapped)
    }
    ///DAC trigger is TIM3_TRGO
    #[inline(always)]
    pub fn remapped(self) -> &'a mut crate::W<REG> {
        self.variant(DAC_TRIG_RMP::Remapped)
    }
}
...
impl ENCODER_MODE_R {
    ...
    pub const fn variant(&self) -> Option<ENCODER_MODE> {
        match self.bits {
            ...
            2 => Some(ENCODER_MODE::MapTim3tim15),
            _ => None,
        }
    }
    ///No redirection
...
impl W {
    ...
    ///Bit 7 - DAC trigger remap (when TSEL = 001)
    #[inline(always)]
    pub fn dac_trig_rmp(&mut self) -> DAC_TRIG_RMP_W<'_, CFGR1rs> {
        DAC_TRIG_RMP_W::new(self, 7)
    }
    ///Bit 11 - TIM16 DMA request remapping bit
    #[inline(always)]
