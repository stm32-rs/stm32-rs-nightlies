...
impl PVDO_R {
    ...
    /**Vdd is higher than the PVD threshold selected with the PLS\[2:0\]
    bits*/
    #[inline(always)]
    pub fn is_higher(&self) -> bool {
        *self == PVDO::Higher
    }
    /**Vdd is lower than the PVD threshold selected with the PLS\[2:0\]
    bits*/
    #[inline(always)]
...
impl From<EWUP> for bool {
    #[inline(always)]
    fn from(variant: EWUP) -> Self {
        variant as u8 != 0
    }
}
///Field `EWUP` reader - Enable WKUP pin
pub type EWUP_R = crate::BitReader<EWUP>;
impl EWUP_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> EWUP {
        match self.bits {
            false => EWUP::Gpio,
            true => EWUP::WakeUp,
        }
...
pub type EWUP_W<'a, REG> = crate::BitWriter<'a, REG, EWUP>;
impl<'a, REG> EWUP_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ...
    pub fn gpio(self) -> &'a mut crate::W<REG> {
        self.variant(EWUP::Gpio)
    }
    ///WKUP1 pin is used for wakeup from Standby mode and forced in input pull down configuration (rising edge or falling on WKUP pin wakes-up the system from Standby mode)
    #[inline(always)]
    pub fn wake_up(self) -> &'a mut crate::W<REG> {
        self.variant(EWUP::WakeUp)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F407.html#PWR:CSR)*/
pub struct CSRrs;
