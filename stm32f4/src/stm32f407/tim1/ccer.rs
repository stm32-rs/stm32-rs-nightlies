...
///Field `CCNP(1-3)` reader - Capture/Compare %s output Polarity
...
///Field `CCNP(1-3)` writer - Capture/Compare %s output Polarity
...
impl R {
    ...
    ///Capture/Compare (1-3) output Polarity
    ///
    ...
    pub fn ccnp_iter(&self) -> impl Iterator<Item = CCNP_R> + '_ {
        (0..3).map(move |n| CCNP_R::new(((self.bits >> (n * 4 + 3)) & 1) != 0))
    }
