...
///Field `nWRP(0-4)` reader - Not write protect
...
///Field `nWRP(0-4)` writer - Not write protect
...
impl R {
    ...
    pub fn n_wrp_iter(&self) -> impl Iterator<Item = N_WRP_R> + '_ {
        (0..5).map(move |n| N_WRP_R::new(((self.bits >> (n + 16)) & 1) != 0))
    }
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F410.html#FLASH:OPTCR)*/
pub struct OPTCRrs;
...
///`reset()` method sets OPTCR to value 0x7fff_aaed
impl crate::Resettable for OPTCRrs {
    const RESET_VALUE: u32 = 0x7fff_aaed;
}
