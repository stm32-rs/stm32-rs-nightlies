...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PVDL {
    /**0: PVD interrupt not connected to TIM1/8 Break input. PVDE and PVDS\[2:0\]
    can be read and modified*/
    NotConnected = 0,
    /**1: PVD interrupt connected to TIM1/8 Break input. PVDE and PVDS\[2:0\]
    are read-only*/
    Connected = 1,
}
...
impl PVDL_R {
    ...
    /**PVD interrupt not connected to TIM1/8 Break input. PVDE and PVDS\[2:0\]
    can be read and modified*/
    #[inline(always)]
    pub fn is_not_connected(&self) -> bool {
        *self == PVDL::NotConnected
    }
    /**PVD interrupt connected to TIM1/8 Break input. PVDE and PVDS\[2:0\]
    are read-only*/
    #[inline(always)]
    pub fn is_connected(&self) -> bool {
        *self == PVDL::Connected
