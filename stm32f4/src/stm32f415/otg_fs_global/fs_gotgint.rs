///Register `FS_GOTGINT` reader
pub type R = crate::R<FS_GOTGINTrs>;
///Register `FS_GOTGINT` writer
...
impl W {
    ///Bit 2 - Session end detected
    #[inline(always)]
    pub fn sedet(&mut self) -> SEDET_W<'_, FS_GOTGINTrs> {
        SEDET_W::new(self, 2)
    }
    ///Bit 8 - Session request success status change
    #[inline(always)]
    pub fn srsschg(&mut self) -> SRSSCHG_W<'_, FS_GOTGINTrs> {
        SRSSCHG_W::new(self, 8)
    }
    ///Bit 9 - Host negotiation success status change
    #[inline(always)]
    pub fn hnsschg(&mut self) -> HNSSCHG_W<'_, FS_GOTGINTrs> {
        HNSSCHG_W::new(self, 9)
    }
    ///Bit 17 - Host negotiation detected
    #[inline(always)]
    pub fn hngdet(&mut self) -> HNGDET_W<'_, FS_GOTGINTrs> {
        HNGDET_W::new(self, 17)
    }
    ///Bit 18 - A-device timeout change
    #[inline(always)]
    pub fn adtochg(&mut self) -> ADTOCHG_W<'_, FS_GOTGINTrs> {
        ADTOCHG_W::new(self, 18)
    }
    ///Bit 19 - Debounce done
    #[inline(always)]
    pub fn dbcdne(&mut self) -> DBCDNE_W<'_, FS_GOTGINTrs> {
...
///`read()` method returns [`fs_gotgint::R`](R) reader structure
impl crate::Readable for FS_GOTGINTrs {}
///`write(|w| ..)` method takes [`fs_gotgint::W`](W) writer structure
impl crate::Writable for FS_GOTGINTrs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets FS_GOTGINT to value 0
