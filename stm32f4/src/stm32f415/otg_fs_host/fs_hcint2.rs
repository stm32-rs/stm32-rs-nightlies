///Register `FS_HCINT2` reader
pub type R = crate::R<FS_HCINT2rs>;
///Register `FS_HCINT2` writer
...
impl W {
    ///Bit 0 - Transfer completed
    #[inline(always)]
    pub fn xfrc(&mut self) -> XFRC_W<'_, FS_HCINT2rs> {
        XFRC_W::new(self, 0)
    }
    ///Bit 1 - Channel halted
    #[inline(always)]
    pub fn chh(&mut self) -> CHH_W<'_, FS_HCINT2rs> {
        CHH_W::new(self, 1)
    }
    ///Bit 3 - STALL response received interrupt
    #[inline(always)]
    pub fn stall(&mut self) -> STALL_W<'_, FS_HCINT2rs> {
        STALL_W::new(self, 3)
    }
    ///Bit 4 - NAK response received interrupt
    #[inline(always)]
    pub fn nak(&mut self) -> NAK_W<'_, FS_HCINT2rs> {
        NAK_W::new(self, 4)
    }
    ///Bit 5 - ACK response received/transmitted interrupt
    #[inline(always)]
    pub fn ack(&mut self) -> ACK_W<'_, FS_HCINT2rs> {
        ACK_W::new(self, 5)
    }
    ///Bit 7 - Transaction error
    #[inline(always)]
    pub fn txerr(&mut self) -> TXERR_W<'_, FS_HCINT2rs> {
        TXERR_W::new(self, 7)
    }
    ///Bit 8 - Babble error
    #[inline(always)]
    pub fn bberr(&mut self) -> BBERR_W<'_, FS_HCINT2rs> {
        BBERR_W::new(self, 8)
    }
    ///Bit 9 - Frame overrun
    #[inline(always)]
    pub fn frmor(&mut self) -> FRMOR_W<'_, FS_HCINT2rs> {
        FRMOR_W::new(self, 9)
    }
    ///Bit 10 - Data toggle error
    #[inline(always)]
    pub fn dterr(&mut self) -> DTERR_W<'_, FS_HCINT2rs> {
        DTERR_W::new(self, 10)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F415.html#OTG_FS_HOST:FS_HCINT2)*/
pub struct FS_HCINT2rs;
impl crate::RegisterSpec for FS_HCINT2rs {
    type Ux = u32;
}
///`read()` method returns [`fs_hcint2::R`](R) reader structure
impl crate::Readable for FS_HCINT2rs {}
///`write(|w| ..)` method takes [`fs_hcint2::W`](W) writer structure
impl crate::Writable for FS_HCINT2rs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets FS_HCINT2 to value 0
