///Register `OTG_HS_HCCHAR0` reader
pub type R = crate::R<OTG_HS_HCCHAR0rs>;
///Register `OTG_HS_HCCHAR0` writer
pub type W = crate::W<OTG_HS_HCCHAR0rs>;
...
impl W {
    ///Bits 0:10 - Maximum packet size
    #[inline(always)]
    pub fn mpsiz(&mut self) -> MPSIZ_W<'_, OTG_HS_HCCHAR0rs> {
        MPSIZ_W::new(self, 0)
    }
    ///Bits 11:14 - Endpoint number
    #[inline(always)]
    pub fn epnum(&mut self) -> EPNUM_W<'_, OTG_HS_HCCHAR0rs> {
        EPNUM_W::new(self, 11)
    }
    ///Bit 15 - Endpoint direction
    #[inline(always)]
    pub fn epdir(&mut self) -> EPDIR_W<'_, OTG_HS_HCCHAR0rs> {
        EPDIR_W::new(self, 15)
    }
    ///Bit 17 - Low-speed device
    #[inline(always)]
    pub fn lsdev(&mut self) -> LSDEV_W<'_, OTG_HS_HCCHAR0rs> {
        LSDEV_W::new(self, 17)
    }
    ///Bits 18:19 - Endpoint type
    #[inline(always)]
    pub fn eptyp(&mut self) -> EPTYP_W<'_, OTG_HS_HCCHAR0rs> {
        EPTYP_W::new(self, 18)
    }
    ///Bits 20:21 - Multi Count (MC) / Error Count (EC)
    #[inline(always)]
    pub fn mc(&mut self) -> MC_W<'_, OTG_HS_HCCHAR0rs> {
        MC_W::new(self, 20)
    }
    ///Bits 22:28 - Device address
    #[inline(always)]
    pub fn dad(&mut self) -> DAD_W<'_, OTG_HS_HCCHAR0rs> {
        DAD_W::new(self, 22)
    }
    ///Bit 29 - Odd frame
    #[inline(always)]
    pub fn oddfrm(&mut self) -> ODDFRM_W<'_, OTG_HS_HCCHAR0rs> {
        ODDFRM_W::new(self, 29)
    }
    ///Bit 30 - Channel disable
    #[inline(always)]
    pub fn chdis(&mut self) -> CHDIS_W<'_, OTG_HS_HCCHAR0rs> {
        CHDIS_W::new(self, 30)
    }
    ///Bit 31 - Channel enable
    #[inline(always)]
    pub fn chena(&mut self) -> CHENA_W<'_, OTG_HS_HCCHAR0rs> {
        CHENA_W::new(self, 31)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F415.html#OTG_HS_HOST:OTG_HS_HCCHAR0)*/
pub struct OTG_HS_HCCHAR0rs;
impl crate::RegisterSpec for OTG_HS_HCCHAR0rs {
    type Ux = u32;
}
///`read()` method returns [`otg_hs_hcchar0::R`](R) reader structure
impl crate::Readable for OTG_HS_HCCHAR0rs {}
///`write(|w| ..)` method takes [`otg_hs_hcchar0::W`](W) writer structure
impl crate::Writable for OTG_HS_HCCHAR0rs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets OTG_HS_HCCHAR0 to value 0
impl crate::Resettable for OTG_HS_HCCHAR0rs {}
