///Register `OTG_HS_HCINT4` reader
pub type R = crate::R<OTG_HS_HCINT4rs>;
///Register `OTG_HS_HCINT4` writer
pub type W = crate::W<OTG_HS_HCINT4rs>;
...
impl W {
    ///Bit 0 - Transfer completed
    #[inline(always)]
    pub fn xfrc(&mut self) -> XFRC_W<'_, OTG_HS_HCINT4rs> {
        XFRC_W::new(self, 0)
    }
    ///Bit 1 - Channel halted
    #[inline(always)]
    pub fn chh(&mut self) -> CHH_W<'_, OTG_HS_HCINT4rs> {
        CHH_W::new(self, 1)
    }
    ///Bit 2 - AHB error
    #[inline(always)]
    pub fn ahberr(&mut self) -> AHBERR_W<'_, OTG_HS_HCINT4rs> {
        AHBERR_W::new(self, 2)
    }
    ///Bit 3 - STALL response received interrupt
    #[inline(always)]
    pub fn stall(&mut self) -> STALL_W<'_, OTG_HS_HCINT4rs> {
        STALL_W::new(self, 3)
    }
    ///Bit 4 - NAK response received interrupt
    #[inline(always)]
    pub fn nak(&mut self) -> NAK_W<'_, OTG_HS_HCINT4rs> {
        NAK_W::new(self, 4)
    }
    ///Bit 5 - ACK response received/transmitted interrupt
    #[inline(always)]
    pub fn ack(&mut self) -> ACK_W<'_, OTG_HS_HCINT4rs> {
        ACK_W::new(self, 5)
    }
    ///Bit 6 - Response received interrupt
    #[inline(always)]
    pub fn nyet(&mut self) -> NYET_W<'_, OTG_HS_HCINT4rs> {
        NYET_W::new(self, 6)
    }
    ///Bit 7 - Transaction error
    #[inline(always)]
    pub fn txerr(&mut self) -> TXERR_W<'_, OTG_HS_HCINT4rs> {
        TXERR_W::new(self, 7)
    }
    ///Bit 8 - Babble error
    #[inline(always)]
    pub fn bberr(&mut self) -> BBERR_W<'_, OTG_HS_HCINT4rs> {
        BBERR_W::new(self, 8)
    }
    ///Bit 9 - Frame overrun
    #[inline(always)]
    pub fn frmor(&mut self) -> FRMOR_W<'_, OTG_HS_HCINT4rs> {
        FRMOR_W::new(self, 9)
    }
    ///Bit 10 - Data toggle error
    #[inline(always)]
    pub fn dterr(&mut self) -> DTERR_W<'_, OTG_HS_HCINT4rs> {
        DTERR_W::new(self, 10)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F415.html#OTG_HS_HOST:OTG_HS_HCINT4)*/
pub struct OTG_HS_HCINT4rs;
impl crate::RegisterSpec for OTG_HS_HCINT4rs {
    type Ux = u32;
}
///`read()` method returns [`otg_hs_hcint4::R`](R) reader structure
impl crate::Readable for OTG_HS_HCINT4rs {}
///`write(|w| ..)` method takes [`otg_hs_hcint4::W`](W) writer structure
impl crate::Writable for OTG_HS_HCINT4rs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets OTG_HS_HCINT4 to value 0
impl crate::Resettable for OTG_HS_HCINT4rs {}
