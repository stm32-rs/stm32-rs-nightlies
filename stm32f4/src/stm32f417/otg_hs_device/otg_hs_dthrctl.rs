///Register `OTG_HS_DTHRCTL` reader
pub type R = crate::R<OTG_HS_DTHRCTLrs>;
///Register `OTG_HS_DTHRCTL` writer
pub type W = crate::W<OTG_HS_DTHRCTLrs>;
...
impl W {
    ///Bit 0 - Nonisochronous IN endpoints threshold enable
    #[inline(always)]
    pub fn nonisothren(&mut self) -> NONISOTHREN_W<'_, OTG_HS_DTHRCTLrs> {
        NONISOTHREN_W::new(self, 0)
    }
    ///Bit 1 - ISO IN endpoint threshold enable
    #[inline(always)]
    pub fn isothren(&mut self) -> ISOTHREN_W<'_, OTG_HS_DTHRCTLrs> {
        ISOTHREN_W::new(self, 1)
    }
    ///Bits 2:10 - Transmit threshold length
    #[inline(always)]
    pub fn txthrlen(&mut self) -> TXTHRLEN_W<'_, OTG_HS_DTHRCTLrs> {
        TXTHRLEN_W::new(self, 2)
    }
    ///Bit 16 - Receive threshold enable
    #[inline(always)]
    pub fn rxthren(&mut self) -> RXTHREN_W<'_, OTG_HS_DTHRCTLrs> {
        RXTHREN_W::new(self, 16)
    }
    ///Bits 17:25 - Receive threshold length
    #[inline(always)]
    pub fn rxthrlen(&mut self) -> RXTHRLEN_W<'_, OTG_HS_DTHRCTLrs> {
        RXTHRLEN_W::new(self, 17)
    }
    ///Bit 27 - Arbiter parking enable
    #[inline(always)]
    pub fn arpen(&mut self) -> ARPEN_W<'_, OTG_HS_DTHRCTLrs> {
        ARPEN_W::new(self, 27)
    }
}
...
pub struct OTG_HS_DTHRCTLrs;
impl crate::RegisterSpec for OTG_HS_DTHRCTLrs {
    type Ux = u32;
}
///`read()` method returns [`otg_hs_dthrctl::R`](R) reader structure
impl crate::Readable for OTG_HS_DTHRCTLrs {}
///`write(|w| ..)` method takes [`otg_hs_dthrctl::W`](W) writer structure
impl crate::Writable for OTG_HS_DTHRCTLrs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets OTG_HS_DTHRCTL to value 0
impl crate::Resettable for OTG_HS_DTHRCTLrs {}
