...
impl W {
    ...
    ///Bit 3 - Timeout condition mask
    #[inline(always)]
    pub fn tom(&mut self) -> TOM_W<'_, DOEPEACHMSK1rs> {
        TOM_W::new(self, 3)
    }
    ///Bit 4 - IN token received when TxFIFO empty mask
    #[inline(always)]
    pub fn ittxfemsk(&mut self) -> ITTXFEMSK_W<'_, DOEPEACHMSK1rs> {
        ITTXFEMSK_W::new(self, 4)
    }
    ///Bit 5 - IN token received with EP mismatch mask
    #[inline(always)]
    pub fn inepnmm(&mut self) -> INEPNMM_W<'_, DOEPEACHMSK1rs> {
        INEPNMM_W::new(self, 5)
    }
    ///Bit 6 - IN endpoint NAK effective mask
    #[inline(always)]
    pub fn inepnem(&mut self) -> INEPNEM_W<'_, DOEPEACHMSK1rs> {
        INEPNEM_W::new(self, 6)
    }
    ///Bit 8 - OUT packet error mask
    #[inline(always)]
    pub fn txfurm(&mut self) -> TXFURM_W<'_, DOEPEACHMSK1rs> {
        TXFURM_W::new(self, 8)
    }
    ///Bit 9 - BNA interrupt mask
    #[inline(always)]
    pub fn bim(&mut self) -> BIM_W<'_, DOEPEACHMSK1rs> {
        BIM_W::new(self, 9)
    }
    ///Bit 12 - Bubble error interrupt mask
    #[inline(always)]
    ...
    ///Bit 13 - NAK interrupt mask
    #[inline(always)]
    pub fn nakm(&mut self) -> NAKM_W<'_, DOEPEACHMSK1rs> {
        NAKM_W::new(self, 13)
    }
    ///Bit 14 - NYET interrupt mask
    #[inline(always)]
    pub fn nyetm(&mut self) -> NYETM_W<'_, DOEPEACHMSK1rs> {
        NYETM_W::new(self, 14)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F427.html#OTG_HS_DEVICE:DOEPEACHMSK1)*/
