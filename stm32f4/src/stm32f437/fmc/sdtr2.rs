...
impl W {
    ///Bits 0:3 - Load Mode Register to Active
    #[inline(always)]
    pub fn tmrd(&mut self) -> TMRD_W<'_, SDTR2rs> {
        TMRD_W::new(self, 0)
    }
    ///Bits 4:7 - Exit self-refresh delay
    #[inline(always)]
    pub fn txsr(&mut self) -> TXSR_W<'_, SDTR2rs> {
        TXSR_W::new(self, 4)
    }
    ///Bits 8:11 - Self refresh time
    #[inline(always)]
    pub fn tras(&mut self) -> TRAS_W<'_, SDTR2rs> {
        TRAS_W::new(self, 8)
    }
    ///Bits 12:15 - Row cycle delay
    #[inline(always)]
    pub fn trc(&mut self) -> TRC_W<'_, SDTR2rs> {
        TRC_W::new(self, 12)
    }
    ///Bits 16:19 - Recovery delay
    #[inline(always)]
    pub fn twr(&mut self) -> TWR_W<'_, SDTR2rs> {
        TWR_W::new(self, 16)
    }
    ///Bits 20:23 - Row precharge delay
    #[inline(always)]
    pub fn trp(&mut self) -> TRP_W<'_, SDTR2rs> {
        TRP_W::new(self, 20)
    }
    ///Bits 24:27 - Row to column delay
    #[inline(always)]
    pub fn trcd(&mut self) -> TRCD_W<'_, SDTR2rs> {
        TRCD_W::new(self, 24)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F437.html#FMC:SDTR2)*/
pub struct SDTR2rs;
