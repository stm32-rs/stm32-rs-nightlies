...
///Field `FFLUS` reader - FIFO flush
pub type FFLUS_R = crate::BitReader;
///Field `FFLUS` writer - FIFO flush
pub type FFLUS_W<'a, REG> = crate::BitWriter<'a, REG>;
...
impl W {
    ///Bits 0:2 - FIFO threshold
    #[inline(always)]
    pub fn fth(&mut self) -> FTH_W<'_, ACR2rs> {
        FTH_W::new(self, 0)
    }
    ///Bit 3 - FIFO flush
    #[inline(always)]
    pub fn fflus(&mut self) -> FFLUS_W<'_, ACR2rs> {
        FFLUS_W::new(self, 3)
    }
    ///Bit 4 - Tristate management on data line
    #[inline(always)]
    pub fn tris(&mut self) -> TRIS_W<'_, ACR2rs> {
        TRIS_W::new(self, 4)
    }
    ///Bit 5 - Mute
    #[inline(always)]
    pub fn mute(&mut self) -> MUTE_W<'_, ACR2rs> {
        MUTE_W::new(self, 5)
    }
    ///Bit 6 - Mute value
    #[inline(always)]
    pub fn muteval(&mut self) -> MUTEVAL_W<'_, ACR2rs> {
        MUTEVAL_W::new(self, 6)
    }
    ///Bits 7:12 - Mute counter
    #[inline(always)]
    pub fn mutecn(&mut self) -> MUTECN_W<'_, ACR2rs> {
        MUTECN_W::new(self, 7)
    }
    ///Bit 13 - Complement bit
    #[inline(always)]
    pub fn cpl(&mut self) -> CPL_W<'_, ACR2rs> {
        CPL_W::new(self, 13)
    }
    ///Bits 14:15 - Companding mode
    #[inline(always)]
    pub fn comp(&mut self) -> COMP_W<'_, ACR2rs> {
        COMP_W::new(self, 14)
    }
}
