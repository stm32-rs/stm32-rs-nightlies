...
impl W {
    ...
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `DATA0` field.</div>
    #[inline(always)]
    pub fn data(&mut self, n: u8) -> DATA_W<'_, TDLRrs> {
        #[allow(clippy::no_effect)]
        [(); 4][n as usize];
        DATA_W::new(self, n * 8)
    }
    ///Bits 0:7 - DATA0
    #[inline(always)]
    pub fn data0(&mut self) -> DATA_W<'_, TDLRrs> {
        DATA_W::new(self, 0)
    }
    ///Bits 8:15 - DATA1
    #[inline(always)]
    pub fn data1(&mut self) -> DATA_W<'_, TDLRrs> {
        DATA_W::new(self, 8)
    }
    ///Bits 16:23 - DATA2
    #[inline(always)]
    pub fn data2(&mut self) -> DATA_W<'_, TDLRrs> {
        DATA_W::new(self, 16)
    }
    ///Bits 24:31 - DATA3
    #[inline(always)]
    pub fn data3(&mut self) -> DATA_W<'_, TDLRrs> {
        DATA_W::new(self, 24)
    }
}
