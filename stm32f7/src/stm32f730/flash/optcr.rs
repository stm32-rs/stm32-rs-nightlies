...
///Field `IWDG_SW` reader - WDG_SW User option bytes
...
///Field `IWDG_SW` writer - WDG_SW User option bytes
...
///Field `nWRP(0-7)` reader - Not write protect
...
///Field `nWRP(0-7)` writer - Not write protect
...
impl R {
    ...
    pub fn n_wrp_iter(&self) -> impl Iterator<Item = N_WRP_R> + '_ {
        (0..8).map(move |n| N_WRP_R::new(((self.bits >> (n + 16)) & 1) != 0))
    }
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32F730.html#FLASH:OPTCR)*/
pub struct OPTCRrs;
...
impl crate::Resettable for OPTCRrs {
    const RESET_VALUE: u32 = 0xc0ff_aafd;
}
