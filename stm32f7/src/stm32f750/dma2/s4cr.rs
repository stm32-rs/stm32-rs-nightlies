...
impl W {
    ///Bit 0 - Stream enable / flag stream ready when read low
    #[inline(always)]
    pub fn en(&mut self) -> EN_W<S4CRrs> {
        EN_W::new(self, 0)
    }
    ///Bit 1 - Direct mode error interrupt enable
    #[inline(always)]
    pub fn dmeie(&mut self) -> DMEIE_W<S4CRrs> {
        DMEIE_W::new(self, 1)
    }
    ///Bit 2 - Transfer error interrupt enable
    #[inline(always)]
    pub fn teie(&mut self) -> TEIE_W<S4CRrs> {
        TEIE_W::new(self, 2)
    }
    ///Bit 3 - Half transfer interrupt enable
    #[inline(always)]
    pub fn htie(&mut self) -> HTIE_W<S4CRrs> {
        HTIE_W::new(self, 3)
    }
    ///Bit 4 - Transfer complete interrupt enable
    #[inline(always)]
    pub fn tcie(&mut self) -> TCIE_W<S4CRrs> {
        TCIE_W::new(self, 4)
    }
    ///Bit 5 - Peripheral flow controller
    #[inline(always)]
    pub fn pfctrl(&mut self) -> PFCTRL_W<S4CRrs> {
        PFCTRL_W::new(self, 5)
    }
    ///Bits 6:7 - Data transfer direction
    #[inline(always)]
    pub fn dir(&mut self) -> DIR_W<S4CRrs> {
        DIR_W::new(self, 6)
    }
    ///Bit 8 - Circular mode
    #[inline(always)]
    pub fn circ(&mut self) -> CIRC_W<S4CRrs> {
        CIRC_W::new(self, 8)
    }
    ///Bit 9 - Peripheral increment mode
    #[inline(always)]
    pub fn pinc(&mut self) -> PINC_W<S4CRrs> {
        PINC_W::new(self, 9)
    }
    ///Bit 10 - Memory increment mode
    #[inline(always)]
    pub fn minc(&mut self) -> MINC_W<S4CRrs> {
        MINC_W::new(self, 10)
    }
    ///Bits 11:12 - Peripheral data size
    #[inline(always)]
    pub fn psize(&mut self) -> PSIZE_W<S4CRrs> {
        PSIZE_W::new(self, 11)
    }
    ///Bits 13:14 - Memory data size
    #[inline(always)]
    pub fn msize(&mut self) -> MSIZE_W<S4CRrs> {
        MSIZE_W::new(self, 13)
    }
    ///Bit 15 - Peripheral increment offset size
    #[inline(always)]
    pub fn pincos(&mut self) -> PINCOS_W<S4CRrs> {
        PINCOS_W::new(self, 15)
    }
    ///Bits 16:17 - Priority level
    #[inline(always)]
    pub fn pl(&mut self) -> PL_W<S4CRrs> {
        PL_W::new(self, 16)
    }
    ///Bit 18 - Double buffer mode
    #[inline(always)]
    pub fn dbm(&mut self) -> DBM_W<S4CRrs> {
        DBM_W::new(self, 18)
    }
    ///Bit 19 - Current target (only in double buffer mode)
    #[inline(always)]
    pub fn ct(&mut self) -> CT_W<S4CRrs> {
        CT_W::new(self, 19)
    }
    ///Bit 20 - ACK
    #[inline(always)]
    pub fn ack(&mut self) -> ACK_W<S4CRrs> {
        ACK_W::new(self, 20)
    }
    ///Bits 21:22 - Peripheral burst transfer configuration
    #[inline(always)]
    pub fn pburst(&mut self) -> PBURST_W<S4CRrs> {
        PBURST_W::new(self, 21)
    }
    ///Bits 23:24 - Memory burst transfer configuration
    #[inline(always)]
    pub fn mburst(&mut self) -> MBURST_W<S4CRrs> {
        MBURST_W::new(self, 23)
    }
    ///Bits 25:27 - Channel selection
    #[inline(always)]
    pub fn chsel(&mut self) -> CHSEL_W<S4CRrs> {
        CHSEL_W::new(self, 25)
    }
}
