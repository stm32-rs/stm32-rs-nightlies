...
///Field `GIF(1-5)` reader - Channel %s Global interrupt flag
...
///Field `HTIF(1-5)` reader - Channel %s Half Transfer Complete flag
...
///Field `TEIF(1-5)` reader - Channel %s Transfer Error flag
...
impl R {
    ...
    pub fn gif_iter(&self) -> impl Iterator<Item = GIF_R> + '_ {
        (0..5).map(move |n| GIF_R::new(((self.bits >> (n * 4)) & 1) != 0))
    }
    ...
    pub fn tcif_iter(&self) -> impl Iterator<Item = TCIF_R> + '_ {
        (0..5).map(move |n| TCIF_R::new(((self.bits >> (n * 4 + 1)) & 1) != 0))
    }
    ...
    pub fn htif_iter(&self) -> impl Iterator<Item = HTIF_R> + '_ {
        (0..5).map(move |n| HTIF_R::new(((self.bits >> (n * 4 + 2)) & 1) != 0))
    }
    ...
    pub fn teif_iter(&self) -> impl Iterator<Item = TEIF_R> + '_ {
        (0..5).map(move |n| TEIF_R::new(((self.bits >> (n * 4 + 3)) & 1) != 0))
    }
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32G041.html#DMA1:ISR)*/
pub struct ISRrs;
