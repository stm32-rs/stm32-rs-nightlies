...
/**Field `WUT` reader - Wakeup auto-reload value bits When the wakeup timer is enabled (WUTE set to 1), the WUTF flag is set every (WUT\[15:0\]+1) ck_wut cycles. The ck_wut period is selected through WUCKSEL\[2:0\]
...
/**Field `WUT` writer - Wakeup auto-reload value bits When the wakeup timer is enabled (WUTE set to 1), the WUTF flag is set every (WUT\[15:0\]+1) ck_wut cycles. The ck_wut period is selected through WUCKSEL\[2:0\]
bits of the RTC_CR register. When WUCKSEL\[2\]
= 1, the wakeup timer becomes 17-bits and WUCKSEL\[1\]
effectively becomes WUT\[16\]
the most-significant bit to be reloaded into the timer. The first assertion of WUTF occurs between WUT and (WUT + 1) ck_wut cycles after WUTE is set. Setting WUT\[15:0\]
...
impl R {
    /**Bits 0:15 - Wakeup auto-reload value bits When the wakeup timer is enabled (WUTE set to 1), the WUTF flag is set every (WUT\[15:0\]+1) ck_wut cycles. The ck_wut period is selected through WUCKSEL\[2:0\]
    bits of the RTC_CR register. When WUCKSEL\[2\]
    = 1, the wakeup timer becomes 17-bits and WUCKSEL\[1\]
    effectively becomes WUT\[16\]
    the most-significant bit to be reloaded into the timer. The first assertion of WUTF occurs between WUT and (WUT + 1) ck_wut cycles after WUTE is set. Setting WUT\[15:0\]
