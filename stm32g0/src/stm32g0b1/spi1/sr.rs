...
/**FIFO reception level These bits are set and cleared by hardware. Note: These bits are not used in I^2S mode and in SPI receive-only mode while CRC calculation is enabled.
...
///Field `FRLVL` reader - FIFO reception level These bits are set and cleared by hardware. Note: These bits are not used in I^2S mode and in SPI receive-only mode while CRC calculation is enabled.
...
impl R {
    ...
    ///Bits 9:10 - FIFO reception level These bits are set and cleared by hardware. Note: These bits are not used in I^2S mode and in SPI receive-only mode while CRC calculation is enabled.
    #[inline(always)]
