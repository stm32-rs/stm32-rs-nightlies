...
///Field `BKCMP3E` reader - BRK COMP3 enable This bit enables the COMP3 for the timerâs BRK input. COMP3 output is 'ORedâ with the other BRK sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
pub type BKCMP3E_R = crate::BitReader;
///Field `BKCMP3E` writer - BRK COMP3 enable This bit enables the COMP3 for the timerâs BRK input. COMP3 output is 'ORedâ with the other BRK sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
...
///Field `BKCMP3P` reader - BRK COMP3 input polarity This bit selects the COMP3 input sensitivity. It must be programmed together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
pub type BKCMP3P_R = crate::BitReader;
///Field `BKCMP3P` writer - BRK COMP3 input polarity This bit selects the COMP3 input sensitivity. It must be programmed together with the BKP polarity bit. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
...
impl R {
    ...
    ///Bit 3 - BRK COMP3 enable This bit enables the COMP3 for the timerâs BRK input. COMP3 output is 'ORedâ with the other BRK sources. Note: This bit can not be modified as long as LOCK level 1 has been programmed (LOCK bits in TIMx_BDTR register).
    #[inline(always)]
