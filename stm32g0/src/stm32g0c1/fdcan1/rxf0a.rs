...
/**Field `F0AI` reader - Rx FIFO 0 acknowledge index After the Host has read a message or a sequence of messages from Rx FIFO 0 it has to write the buffer index of the last element read from Rx FIFO 0 to F0AI. This sets the Rx FIFO0 get index RXF0S\[F0GI\]
to F0AI + 1 and update the FIFO 0 fill level RXF0S\[F0FL\].*/
pub type F0AI_R = crate::FieldReader;
/**Field `F0AI` writer - Rx FIFO 0 acknowledge index After the Host has read a message or a sequence of messages from Rx FIFO 0 it has to write the buffer index of the last element read from Rx FIFO 0 to F0AI. This sets the Rx FIFO0 get index RXF0S\[F0GI\]
to F0AI + 1 and update the FIFO 0 fill level RXF0S\[F0FL\].*/
pub type F0AI_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
impl R {
    /**Bits 0:2 - Rx FIFO 0 acknowledge index After the Host has read a message or a sequence of messages from Rx FIFO 0 it has to write the buffer index of the last element read from Rx FIFO 0 to F0AI. This sets the Rx FIFO0 get index RXF0S\[F0GI\]
    to F0AI + 1 and update the FIFO 0 fill level RXF0S\[F0FL\].*/
    #[inline(always)]
