...
///Field `SYNCIN` reader - synchronization input
...
///Field `SYNCIN` writer - synchronization input
...
///Field `TCEN(A,B,C,D,E,F)` reader - Timer %s counter enable
...
///Field `TCEN(A,B,C,D,E,F)` writer - Timer %s counter enable
...
impl R {
    ...
    ///Timer (A,B,C,D,E,F) counter enable
    ///
    ...
    pub fn tcen_iter(&self) -> impl Iterator<Item = TCEN_R> + '_ {
        (0..6).map(move |n| TCEN_R::new(((self.bits >> (n + 17)) & 1) != 0))
    }
    ...
    #[inline(always)]
    pub fn tfcen(&self) -> TCEN_R {
        TCEN_R::new(((self.bits >> 22) & 1) != 0)
    }
...
impl W {
    ...
    ///Bit 22 - Timer F counter enable
    #[inline(always)]
    pub fn tfcen(&mut self) -> TCEN_W<'_, CRrs> {
        TCEN_W::new(self, 22)
    }
    ///Bits 25:26 - AC Synchronization
    #[inline(always)]
