///Register `CPT1DCR` reader
pub type R = crate::R<CPT1DCRrs>;
///Register `CPT1DCR` writer
pub type W = crate::W<CPT1DCRrs>;
///Field `SWCPT` reader - Software Capture
pub type SWCPT_R = crate::BitReader;
///Field `SWCPT` writer - Software Capture
pub type SWCPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `UDPCPT` reader - Update Capture
pub type UDPCPT_R = crate::BitReader;
///Field `UDPCPT` writer - Update Capture
pub type UDPCPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV1CPT` reader - External Event 1 Capture
pub type EXEV1CPT_R = crate::BitReader;
///Field `EXEV1CPT` writer - External Event 1 Capture
pub type EXEV1CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV2CPT` reader - External Event 2 Capture
pub type EXEV2CPT_R = crate::BitReader;
///Field `EXEV2CPT` writer - External Event 2 Capture
pub type EXEV2CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV3CPT` reader - External Event 3 Capture
pub type EXEV3CPT_R = crate::BitReader;
///Field `EXEV3CPT` writer - External Event 3 Capture
pub type EXEV3CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV4CPT` reader - External Event 4 Capture
pub type EXEV4CPT_R = crate::BitReader;
///Field `EXEV4CPT` writer - External Event 4 Capture
pub type EXEV4CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV5CPT` reader - External Event 5 Capture
pub type EXEV5CPT_R = crate::BitReader;
///Field `EXEV5CPT` writer - External Event 5 Capture
pub type EXEV5CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV6CPT` reader - External Event 6 Capture
pub type EXEV6CPT_R = crate::BitReader;
///Field `EXEV6CPT` writer - External Event 6 Capture
pub type EXEV6CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV7CPT` reader - External Event 7 Capture
pub type EXEV7CPT_R = crate::BitReader;
///Field `EXEV7CPT` writer - External Event 7 Capture
pub type EXEV7CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV8CPT` reader - External Event 8 Capture
pub type EXEV8CPT_R = crate::BitReader;
///Field `EXEV8CPT` writer - External Event 8 Capture
pub type EXEV8CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV9CPT` reader - External Event 9 Capture
pub type EXEV9CPT_R = crate::BitReader;
///Field `EXEV9CPT` writer - External Event 9 Capture
pub type EXEV9CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EXEV10CPT` reader - External Event 10 Capture
pub type EXEV10CPT_R = crate::BitReader;
///Field `EXEV10CPT` writer - External Event 10 Capture
pub type EXEV10CPT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TA1SET` reader - Timer A output 1 Set
pub type TA1SET_R = crate::BitReader;
///Field `TA1SET` writer - Timer A output 1 Set
pub type TA1SET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TA1RST` reader - Timer A output 1 Reset
pub type TA1RST_R = crate::BitReader;
///Field `TA1RST` writer - Timer A output 1 Reset
pub type TA1RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TACMP1` reader - Timer A Compare 1
pub type TACMP1_R = crate::BitReader;
///Field `TACMP1` writer - Timer A Compare 1
pub type TACMP1_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TACMP2` reader - Timer A Compare 2
pub type TACMP2_R = crate::BitReader;
///Field `TACMP2` writer - Timer A Compare 2
pub type TACMP2_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TB1SET` reader - Timer B output 1 Set
pub type TB1SET_R = crate::BitReader;
///Field `TB1SET` writer - Timer B output 1 Set
pub type TB1SET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TB1RST` reader - Timer B output 1 Reset
pub type TB1RST_R = crate::BitReader;
///Field `TB1RST` writer - Timer B output 1 Reset
pub type TB1RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TBCMP1` reader - Timer B Compare 1
pub type TBCMP1_R = crate::BitReader;
///Field `TBCMP1` writer - Timer B Compare 1
pub type TBCMP1_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TBCMP2` reader - Timer B Compare 2
pub type TBCMP2_R = crate::BitReader;
///Field `TBCMP2` writer - Timer B Compare 2
pub type TBCMP2_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TC1SET` reader - Timer C output 1 Set
pub type TC1SET_R = crate::BitReader;
///Field `TC1SET` writer - Timer C output 1 Set
pub type TC1SET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TC1RST` reader - Timer C output 1 Reset
pub type TC1RST_R = crate::BitReader;
///Field `TC1RST` writer - Timer C output 1 Reset
pub type TC1RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TCCMP1` reader - Timer C Compare 1
pub type TCCMP1_R = crate::BitReader;
///Field `TCCMP1` writer - Timer C Compare 1
pub type TCCMP1_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TCCMP2` reader - Timer C Compare 2
pub type TCCMP2_R = crate::BitReader;
///Field `TCCMP2` writer - Timer C Compare 2
pub type TCCMP2_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TF1SET` reader - TF1SET
pub type TF1SET_R = crate::BitReader;
///Field `TF1SET` writer - TF1SET
pub type TF1SET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TF1RST` reader - TF1RST
pub type TF1RST_R = crate::BitReader;
///Field `TF1RST` writer - TF1RST
pub type TF1RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TFCMP1` reader - TFCMP1
pub type TFCMP1_R = crate::BitReader;
///Field `TFCMP1` writer - TFCMP1
pub type TFCMP1_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TFCMP2` reader - TFCMP2
pub type TFCMP2_R = crate::BitReader;
///Field `TFCMP2` writer - TFCMP2
pub type TFCMP2_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TE1SET` reader - Timer E output 1 Set
pub type TE1SET_R = crate::BitReader;
///Field `TE1SET` writer - Timer E output 1 Set
pub type TE1SET_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TE1RST` reader - Timer E output 1 Reset
pub type TE1RST_R = crate::BitReader;
///Field `TE1RST` writer - Timer E output 1 Reset
pub type TE1RST_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TECMP1` reader - Timer E Compare 1
pub type TECMP1_R = crate::BitReader;
///Field `TECMP1` writer - Timer E Compare 1
pub type TECMP1_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TECMP2` reader - Timer E Compare 2
pub type TECMP2_R = crate::BitReader;
///Field `TECMP2` writer - Timer E Compare 2
pub type TECMP2_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0 - Software Capture
    #[inline(always)]
    pub fn swcpt(&self) -> SWCPT_R {
        SWCPT_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - Update Capture
    #[inline(always)]
    pub fn udpcpt(&self) -> UDPCPT_R {
        UDPCPT_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - External Event 1 Capture
    #[inline(always)]
    pub fn exev1cpt(&self) -> EXEV1CPT_R {
        EXEV1CPT_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - External Event 2 Capture
    #[inline(always)]
    pub fn exev2cpt(&self) -> EXEV2CPT_R {
        EXEV2CPT_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 4 - External Event 3 Capture
    #[inline(always)]
    pub fn exev3cpt(&self) -> EXEV3CPT_R {
        EXEV3CPT_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - External Event 4 Capture
    #[inline(always)]
    pub fn exev4cpt(&self) -> EXEV4CPT_R {
        EXEV4CPT_R::new(((self.bits >> 5) & 1) != 0)
    }
    ///Bit 6 - External Event 5 Capture
    #[inline(always)]
    pub fn exev5cpt(&self) -> EXEV5CPT_R {
        EXEV5CPT_R::new(((self.bits >> 6) & 1) != 0)
    }
    ///Bit 7 - External Event 6 Capture
    #[inline(always)]
    pub fn exev6cpt(&self) -> EXEV6CPT_R {
        EXEV6CPT_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8 - External Event 7 Capture
    #[inline(always)]
    pub fn exev7cpt(&self) -> EXEV7CPT_R {
        EXEV7CPT_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9 - External Event 8 Capture
    #[inline(always)]
    pub fn exev8cpt(&self) -> EXEV8CPT_R {
        EXEV8CPT_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10 - External Event 9 Capture
    #[inline(always)]
    pub fn exev9cpt(&self) -> EXEV9CPT_R {
        EXEV9CPT_R::new(((self.bits >> 10) & 1) != 0)
    }
    ///Bit 11 - External Event 10 Capture
    #[inline(always)]
    pub fn exev10cpt(&self) -> EXEV10CPT_R {
        EXEV10CPT_R::new(((self.bits >> 11) & 1) != 0)
    }
    ///Bit 12 - Timer A output 1 Set
    #[inline(always)]
    pub fn ta1set(&self) -> TA1SET_R {
        TA1SET_R::new(((self.bits >> 12) & 1) != 0)
    }
    ///Bit 13 - Timer A output 1 Reset
    #[inline(always)]
    pub fn ta1rst(&self) -> TA1RST_R {
        TA1RST_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14 - Timer A Compare 1
    #[inline(always)]
    pub fn tacmp1(&self) -> TACMP1_R {
        TACMP1_R::new(((self.bits >> 14) & 1) != 0)
    }
    ///Bit 15 - Timer A Compare 2
    #[inline(always)]
    pub fn tacmp2(&self) -> TACMP2_R {
        TACMP2_R::new(((self.bits >> 15) & 1) != 0)
    }
    ///Bit 16 - Timer B output 1 Set
    #[inline(always)]
    pub fn tb1set(&self) -> TB1SET_R {
        TB1SET_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17 - Timer B output 1 Reset
    #[inline(always)]
    pub fn tb1rst(&self) -> TB1RST_R {
        TB1RST_R::new(((self.bits >> 17) & 1) != 0)
    }
    ///Bit 18 - Timer B Compare 1
    #[inline(always)]
    pub fn tbcmp1(&self) -> TBCMP1_R {
        TBCMP1_R::new(((self.bits >> 18) & 1) != 0)
    }
    ///Bit 19 - Timer B Compare 2
    #[inline(always)]
    pub fn tbcmp2(&self) -> TBCMP2_R {
        TBCMP2_R::new(((self.bits >> 19) & 1) != 0)
    }
    ///Bit 20 - Timer C output 1 Set
    #[inline(always)]
    pub fn tc1set(&self) -> TC1SET_R {
        TC1SET_R::new(((self.bits >> 20) & 1) != 0)
    }
    ///Bit 21 - Timer C output 1 Reset
    #[inline(always)]
    pub fn tc1rst(&self) -> TC1RST_R {
        TC1RST_R::new(((self.bits >> 21) & 1) != 0)
    }
    ///Bit 22 - Timer C Compare 1
    #[inline(always)]
    pub fn tccmp1(&self) -> TCCMP1_R {
        TCCMP1_R::new(((self.bits >> 22) & 1) != 0)
    }
    ///Bit 23 - Timer C Compare 2
    #[inline(always)]
    pub fn tccmp2(&self) -> TCCMP2_R {
        TCCMP2_R::new(((self.bits >> 23) & 1) != 0)
    }
    ///Bit 24 - TF1SET
    #[inline(always)]
    pub fn tf1set(&self) -> TF1SET_R {
        TF1SET_R::new(((self.bits >> 24) & 1) != 0)
    }
    ///Bit 25 - TF1RST
    #[inline(always)]
    pub fn tf1rst(&self) -> TF1RST_R {
        TF1RST_R::new(((self.bits >> 25) & 1) != 0)
    }
    ///Bit 26 - TFCMP1
    #[inline(always)]
    pub fn tfcmp1(&self) -> TFCMP1_R {
        TFCMP1_R::new(((self.bits >> 26) & 1) != 0)
    }
    ///Bit 27 - TFCMP2
    #[inline(always)]
    pub fn tfcmp2(&self) -> TFCMP2_R {
        TFCMP2_R::new(((self.bits >> 27) & 1) != 0)
    }
    ///Bit 28 - Timer E output 1 Set
    #[inline(always)]
    pub fn te1set(&self) -> TE1SET_R {
        TE1SET_R::new(((self.bits >> 28) & 1) != 0)
    }
    ///Bit 29 - Timer E output 1 Reset
    #[inline(always)]
    pub fn te1rst(&self) -> TE1RST_R {
        TE1RST_R::new(((self.bits >> 29) & 1) != 0)
    }
    ///Bit 30 - Timer E Compare 1
    #[inline(always)]
    pub fn tecmp1(&self) -> TECMP1_R {
        TECMP1_R::new(((self.bits >> 30) & 1) != 0)
    }
    ///Bit 31 - Timer E Compare 2
    #[inline(always)]
    pub fn tecmp2(&self) -> TECMP2_R {
        TECMP2_R::new(((self.bits >> 31) & 1) != 0)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("CPT1DCR")
            .field("tecmp2", &self.tecmp2())
            .field("tecmp1", &self.tecmp1())
            .field("te1rst", &self.te1rst())
            .field("te1set", &self.te1set())
            .field("tfcmp2", &self.tfcmp2())
            .field("tfcmp1", &self.tfcmp1())
            .field("tf1rst", &self.tf1rst())
            .field("tf1set", &self.tf1set())
            .field("tccmp2", &self.tccmp2())
            .field("tccmp1", &self.tccmp1())
            .field("tc1rst", &self.tc1rst())
            .field("tc1set", &self.tc1set())
            .field("tbcmp2", &self.tbcmp2())
            .field("tbcmp1", &self.tbcmp1())
            .field("tb1rst", &self.tb1rst())
            .field("tb1set", &self.tb1set())
            .field("tacmp2", &self.tacmp2())
            .field("tacmp1", &self.tacmp1())
            .field("ta1rst", &self.ta1rst())
            .field("ta1set", &self.ta1set())
            .field("exev10cpt", &self.exev10cpt())
            .field("exev9cpt", &self.exev9cpt())
            .field("exev8cpt", &self.exev8cpt())
            .field("exev7cpt", &self.exev7cpt())
            .field("exev6cpt", &self.exev6cpt())
            .field("exev5cpt", &self.exev5cpt())
            .field("exev4cpt", &self.exev4cpt())
            .field("exev3cpt", &self.exev3cpt())
            .field("exev2cpt", &self.exev2cpt())
            .field("exev1cpt", &self.exev1cpt())
            .field("udpcpt", &self.udpcpt())
            .field("swcpt", &self.swcpt())
            .finish()
    }
}
impl W {
    ///Bit 0 - Software Capture
    #[inline(always)]
    #[must_use]
    pub fn swcpt(&mut self) -> SWCPT_W<CPT1DCRrs> {
        SWCPT_W::new(self, 0)
    }
    ///Bit 1 - Update Capture
    #[inline(always)]
    #[must_use]
    pub fn udpcpt(&mut self) -> UDPCPT_W<CPT1DCRrs> {
        UDPCPT_W::new(self, 1)
    }
    ///Bit 2 - External Event 1 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev1cpt(&mut self) -> EXEV1CPT_W<CPT1DCRrs> {
        EXEV1CPT_W::new(self, 2)
    }
    ///Bit 3 - External Event 2 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev2cpt(&mut self) -> EXEV2CPT_W<CPT1DCRrs> {
        EXEV2CPT_W::new(self, 3)
    }
    ///Bit 4 - External Event 3 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev3cpt(&mut self) -> EXEV3CPT_W<CPT1DCRrs> {
        EXEV3CPT_W::new(self, 4)
    }
    ///Bit 5 - External Event 4 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev4cpt(&mut self) -> EXEV4CPT_W<CPT1DCRrs> {
        EXEV4CPT_W::new(self, 5)
    }
    ///Bit 6 - External Event 5 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev5cpt(&mut self) -> EXEV5CPT_W<CPT1DCRrs> {
        EXEV5CPT_W::new(self, 6)
    }
    ///Bit 7 - External Event 6 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev6cpt(&mut self) -> EXEV6CPT_W<CPT1DCRrs> {
        EXEV6CPT_W::new(self, 7)
    }
    ///Bit 8 - External Event 7 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev7cpt(&mut self) -> EXEV7CPT_W<CPT1DCRrs> {
        EXEV7CPT_W::new(self, 8)
    }
    ///Bit 9 - External Event 8 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev8cpt(&mut self) -> EXEV8CPT_W<CPT1DCRrs> {
        EXEV8CPT_W::new(self, 9)
    }
    ///Bit 10 - External Event 9 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev9cpt(&mut self) -> EXEV9CPT_W<CPT1DCRrs> {
        EXEV9CPT_W::new(self, 10)
    }
    ///Bit 11 - External Event 10 Capture
    #[inline(always)]
    #[must_use]
    pub fn exev10cpt(&mut self) -> EXEV10CPT_W<CPT1DCRrs> {
        EXEV10CPT_W::new(self, 11)
    }
    ///Bit 12 - Timer A output 1 Set
    #[inline(always)]
    #[must_use]
    pub fn ta1set(&mut self) -> TA1SET_W<CPT1DCRrs> {
        TA1SET_W::new(self, 12)
    }
    ///Bit 13 - Timer A output 1 Reset
    #[inline(always)]
    #[must_use]
    pub fn ta1rst(&mut self) -> TA1RST_W<CPT1DCRrs> {
        TA1RST_W::new(self, 13)
    }
    ///Bit 14 - Timer A Compare 1
    #[inline(always)]
    #[must_use]
    pub fn tacmp1(&mut self) -> TACMP1_W<CPT1DCRrs> {
        TACMP1_W::new(self, 14)
    }
    ///Bit 15 - Timer A Compare 2
    #[inline(always)]
    #[must_use]
    pub fn tacmp2(&mut self) -> TACMP2_W<CPT1DCRrs> {
        TACMP2_W::new(self, 15)
    }
    ///Bit 16 - Timer B output 1 Set
    #[inline(always)]
    #[must_use]
    pub fn tb1set(&mut self) -> TB1SET_W<CPT1DCRrs> {
        TB1SET_W::new(self, 16)
    }
    ///Bit 17 - Timer B output 1 Reset
    #[inline(always)]
    #[must_use]
    pub fn tb1rst(&mut self) -> TB1RST_W<CPT1DCRrs> {
        TB1RST_W::new(self, 17)
    }
    ///Bit 18 - Timer B Compare 1
    #[inline(always)]
    #[must_use]
    pub fn tbcmp1(&mut self) -> TBCMP1_W<CPT1DCRrs> {
        TBCMP1_W::new(self, 18)
    }
    ///Bit 19 - Timer B Compare 2
    #[inline(always)]
    #[must_use]
    pub fn tbcmp2(&mut self) -> TBCMP2_W<CPT1DCRrs> {
        TBCMP2_W::new(self, 19)
    }
    ///Bit 20 - Timer C output 1 Set
    #[inline(always)]
    #[must_use]
    pub fn tc1set(&mut self) -> TC1SET_W<CPT1DCRrs> {
        TC1SET_W::new(self, 20)
    }
    ///Bit 21 - Timer C output 1 Reset
    #[inline(always)]
    #[must_use]
    pub fn tc1rst(&mut self) -> TC1RST_W<CPT1DCRrs> {
        TC1RST_W::new(self, 21)
    }
    ///Bit 22 - Timer C Compare 1
    #[inline(always)]
    #[must_use]
    pub fn tccmp1(&mut self) -> TCCMP1_W<CPT1DCRrs> {
        TCCMP1_W::new(self, 22)
    }
    ///Bit 23 - Timer C Compare 2
    #[inline(always)]
    #[must_use]
    pub fn tccmp2(&mut self) -> TCCMP2_W<CPT1DCRrs> {
        TCCMP2_W::new(self, 23)
    }
    ///Bit 24 - TF1SET
    #[inline(always)]
    #[must_use]
    pub fn tf1set(&mut self) -> TF1SET_W<CPT1DCRrs> {
        TF1SET_W::new(self, 24)
    }
    ///Bit 25 - TF1RST
    #[inline(always)]
    #[must_use]
    pub fn tf1rst(&mut self) -> TF1RST_W<CPT1DCRrs> {
        TF1RST_W::new(self, 25)
    }
    ///Bit 26 - TFCMP1
    #[inline(always)]
    #[must_use]
    pub fn tfcmp1(&mut self) -> TFCMP1_W<CPT1DCRrs> {
        TFCMP1_W::new(self, 26)
    }
    ///Bit 27 - TFCMP2
    #[inline(always)]
    #[must_use]
    pub fn tfcmp2(&mut self) -> TFCMP2_W<CPT1DCRrs> {
        TFCMP2_W::new(self, 27)
    }
    ///Bit 28 - Timer E output 1 Set
    #[inline(always)]
    #[must_use]
    pub fn te1set(&mut self) -> TE1SET_W<CPT1DCRrs> {
        TE1SET_W::new(self, 28)
    }
    ///Bit 29 - Timer E output 1 Reset
    #[inline(always)]
    #[must_use]
    pub fn te1rst(&mut self) -> TE1RST_W<CPT1DCRrs> {
        TE1RST_W::new(self, 29)
    }
    ///Bit 30 - Timer E Compare 1
    #[inline(always)]
    #[must_use]
    pub fn tecmp1(&mut self) -> TECMP1_W<CPT1DCRrs> {
        TECMP1_W::new(self, 30)
    }
    ///Bit 31 - Timer E Compare 2
    #[inline(always)]
    #[must_use]
    pub fn tecmp2(&mut self) -> TECMP2_W<CPT1DCRrs> {
        TECMP2_W::new(self, 31)
    }
}
/**Timerx Capture 2 Control Register

You can [`read`](crate::Reg::read) this register and get [`cpt1dcr::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`cpt1dcr::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32G474xx.html#HRTIM_TIMD:CPT1DCR)*/
pub struct CPT1DCRrs;
impl crate::RegisterSpec for CPT1DCRrs {
    type Ux = u32;
}
///`read()` method returns [`cpt1dcr::R`](R) reader structure
impl crate::Readable for CPT1DCRrs {}
///`write(|w| ..)` method takes [`cpt1dcr::W`](W) writer structure
impl crate::Writable for CPT1DCRrs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets CPT1DCR to value 0
impl crate::Resettable for CPT1DCRrs {
    const RESET_VALUE: u32 = 0;
}
