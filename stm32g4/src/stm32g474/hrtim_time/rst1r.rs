...
impl From<TIMACMP4> for bool {
    #[inline(always)]
    fn from(variant: TIMACMP4) -> Self {
        variant as u8 != 0
    }
}
///Field `TIMACMP4` reader - Timer A Compare 4
pub type TIMACMP4_R = crate::BitReader<TIMACMP4>;
impl TIMACMP4_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> TIMACMP4 {
        match self.bits {
            false => TIMACMP4::NoEffect,
            true => TIMACMP4::SetInactive,
        }
    }
    ///Timer event has no effect
    #[inline(always)]
    pub fn is_no_effect(&self) -> bool {
        *self == TIMACMP4::NoEffect
    }
    ///Timer event forces the output to its inactive state
    #[inline(always)]
    pub fn is_set_inactive(&self) -> bool {
        *self == TIMACMP4::SetInactive
    }
}
///Field `TIMACMP4` writer - Timer A Compare 4
pub type TIMACMP4_W<'a, REG> = crate::BitWriter<'a, REG, TIMACMP4>;
impl<'a, REG> TIMACMP4_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Timer event has no effect
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32G474xx.html#HRTIM_TIME:RST1R)*/
pub struct RST1Rrs;
