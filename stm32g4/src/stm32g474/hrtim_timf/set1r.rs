...
impl From<TIMACMP3> for bool {
    #[inline(always)]
    fn from(variant: TIMACMP3) -> Self {
        variant as u8 != 0
    }
}
///Field `TIMACMP3` reader - Timer A Compare 3
pub type TIMACMP3_R = crate::BitReader<TIMACMP3>;
impl TIMACMP3_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> TIMACMP3 {
        match self.bits {
            false => TIMACMP3::NoEffect,
            true => TIMACMP3::SetActive,
        }
    }
    ///Timer event has no effect
    #[inline(always)]
    pub fn is_no_effect(&self) -> bool {
        *self == TIMACMP3::NoEffect
    }
    ///Timer event forces the output to its active state
    #[inline(always)]
    pub fn is_set_active(&self) -> bool {
        *self == TIMACMP3::SetActive
    }
}
///Field `TIMACMP3` writer - Timer A Compare 3
pub type TIMACMP3_W<'a, REG> = crate::BitWriter<'a, REG, TIMACMP3>;
impl<'a, REG> TIMACMP3_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Timer event has no effect
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32G474xx.html#HRTIM_TIMF:SET1R)*/
pub struct SET1Rrs;
