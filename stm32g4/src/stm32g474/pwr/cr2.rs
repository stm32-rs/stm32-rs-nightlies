...
///Field `PVDE` reader - Programmable voltage detector enable Note: This bit is write-protected when the PVDL bit is set in the SYSCFG_CFGR2 register. The protection can be reset only by a system reset.
pub type PVDE_R = crate::BitReader;
///Field `PVDE` writer - Programmable voltage detector enable Note: This bit is write-protected when the PVDL bit is set in the SYSCFG_CFGR2 register. The protection can be reset only by a system reset.
pub type PVDE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `PVDLS` reader - Programmable voltage detector level selection. These bits select the PVD falling threshold: Note: These bits are write-protected when the PVDL bit is set in the SYSCFG_CFGR2 register. The protection can be reset only by a system reset.
pub type PVDLS_R = crate::FieldReader;
///Field `PVDLS` writer - Programmable voltage detector level selection. These bits select the PVD falling threshold: Note: These bits are write-protected when the PVDL bit is set in the SYSCFG_CFGR2 register. The protection can be reset only by a system reset.
...
impl W {
    ...
    ///Bits 1:3 - Programmable voltage detector level selection. These bits select the PVD falling threshold: Note: These bits are write-protected when the PVDL bit is set in the SYSCFG_CFGR2 register. The protection can be reset only by a system reset.
    #[inline(always)]
    pub fn pvdls(&mut self) -> PVDLS_W<CR2rs> {
        PVDLS_W::new(self, 1)
    }
    ///Bit 6 - Peripheral voltage monitoring 3 enable: V<sub>DDA</sub> vs. ADC/COMP min voltage 1.62V
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32G474.html#PWR:CR2)*/
pub struct CR2rs;
