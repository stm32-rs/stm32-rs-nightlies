...
///Field `T1ODS(A,B,C,D,E,F)` reader - Timer %s Output 1 disable status
...
///Field `T2ODS(A,B,C,D,E,F)` reader - Timer %s Output 2 disable status
pub use T1ODS_R as T2ODS_R;
impl R {
    ///Timer (A,B,C,D,E,F) Output 1 disable status
    ///
    ...
    pub fn t1ods_iter(&self) -> impl Iterator<Item = T1ODS_R> + '_ {
        (0..6).map(move |n| T1ODS_R::new(((self.bits >> (n * 2)) & 1) != 0))
    }
    ...
    #[inline(always)]
    pub fn tf1ods(&self) -> T1ODS_R {
        T1ODS_R::new(((self.bits >> 10) & 1) != 0)
    }
    ///Timer (A,B,C,D,E,F) Output 2 disable status
    ///
    ...
    pub fn t2ods_iter(&self) -> impl Iterator<Item = T2ODS_R> + '_ {
        (0..6).map(move |n| T2ODS_R::new(((self.bits >> (n * 2 + 1)) & 1) != 0))
    }
    ...
    #[inline(always)]
    pub fn tf2ods(&self) -> T2ODS_R {
        T2ODS_R::new(((self.bits >> 11) & 1) != 0)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32G484xx.html#HRTIM_Common:ODSR)*/
pub struct ODSRrs;
