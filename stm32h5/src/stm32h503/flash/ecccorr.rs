...
///Field `BK_ECC` reader - ECC bank flag for corrected ECC error It indicates which bank is concerned by ECC error
pub type BK_ECC_R = crate::BitReader;
///Field `SYSF_ECC` reader - ECC flag for corrected ECC error in system FLASH It indicates if system Flash memory is concerned by ECC error.
...
impl W {
    ///Bit 25 - ECC single correction error interrupt enable bit When ECCCIE bit is set to 1, an interrupt is generated when an ECC single correction error occurs during a read operation.
    #[inline(always)]
    pub fn ecccie(&mut self) -> ECCCIE_W<'_, ECCCORRrs> {
        ECCCIE_W::new(self, 25)
    }
    ///Bit 30 - ECC correction set by hardware when single ECC error has been detected and corrected. Cleared by writing 1.
    #[inline(always)]
    pub fn eccc(&mut self) -> ECCC_W<'_, ECCCORRrs> {
        ECCC_W::new(self, 30)
    }
}
