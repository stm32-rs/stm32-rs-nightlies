///Register `C3TR2` reader
pub type R = crate::R<C3TR2rs>;
///Register `C3TR2` writer
pub type W = crate::W<C3TR2rs>;
...
impl W {
    ...
    value) to different active GPDMA channels (GPDMA_CxCR.EN = 1 and GPDMA_CxTR2.SWREQ = 0 for these channels). GPDMA is not intended to hardware support the case of simultaneous enabled channels incorrectly configured with a same hardware peripheral request signal, and there is no user setting error reporting.*/
    #[inline(always)]
    pub fn reqsel(&mut self) -> REQSEL_W<C3TR2rs> {
        REQSEL_W::new(self, 0)
    }
    ///Bit 9 - software request This bit is internally taken into account when GPDMA_CxCR.EN is asserted.
    #[inline(always)]
    pub fn swreq(&mut self) -> SWREQ_W<C3TR2rs> {
        SWREQ_W::new(self, 9)
    }
    ///Bit 10 - destination hardware request This bit is ignored if channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer). Else: Note: If the channel x is activated (GPDMA_CxCR.EN is asserted) with SWREQ = 0 and PFREQ = 1 (peripheral hardware request with peripheral flow-control mode), any software assertion to this DREQ bit is ignored: in peripheral flow-control mode, only a peripheral-to-memory transfer is supported.
    #[inline(always)]
    pub fn dreq(&mut self) -> DREQ_W<C3TR2rs> {
        DREQ_W::new(self, 10)
    }
    ///Bit 11 - Block hardware request If the channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer), this bit is ignored. Else:
    #[inline(always)]
    pub fn breq(&mut self) -> BREQ_W<C3TR2rs> {
        BREQ_W::new(self, 11)
    }
    /**Bit 12 - Hardware request in peripheral flow control mode Important: If a given channel x is not implemented with this feature, this bit is reserved and PFREQ is not present (see Section 14.3.2 for the list of the implemented channels with this feature. If the channel x is activated (GPDMA_CxCR.EN asserted) with SWREQ = 1 (software request for a memory-to-memory transfer), this bit is ignored. Else: Note: In peripheral flow control mode, there are the following restrictions: Note: - no 2D/repeated block support (GPDMA_CxBR1.BRC\[10:0\]
    must be set to 0) Note: - the peripheral must be set as the source of the transfer (DREQ = 0). Note: - data packing to a wider destination width is not supported (if destination width source data width, GPDMA_CxTR1.PAM\[1\]
    ...
    must be programmed as a multiple of the source (peripheral) burst size.*/
    #[inline(always)]
    pub fn pfreq(&mut self) -> PFREQ_W<C3TR2rs> {
        PFREQ_W::new(self, 12)
    }
    ///Bits 14:15 - trigger mode These bits define the transfer granularity for its conditioning by the trigger.
    #[inline(always)]
    pub fn trigm(&mut self) -> TRIGM_W<C3TR2rs> {
        TRIGM_W::new(self, 14)
    }
    /**Bits 16:21 - trigger event input selection These bits select the trigger event input of the GPDMA transfer (as per Section 14.3.7), with an active trigger event if TRIGPOL\[1:0\]
    different 00.*/
    #[inline(always)]
    pub fn trigsel(&mut self) -> TRIGSEL_W<C3TR2rs> {
        TRIGSEL_W::new(self, 16)
    }
    ///Bits 24:25 - trigger event polarity These bits define the polarity of the selected trigger event input defined by TRIGSEL\[5:0\].
    #[inline(always)]
    pub fn trigpol(&mut self) -> TRIGPOL_W<C3TR2rs> {
    ...
    =0 ), then the half transfer event is not generated, and the transfer complete event is generated when is completed the loading of the LLI 1 .*/
    #[inline(always)]
    pub fn tcem(&mut self) -> TCEM_W<C3TR2rs> {
        TCEM_W::new(self, 30)
    }
}
...
pub struct C3TR2rs;
impl crate::RegisterSpec for C3TR2rs {
    type Ux = u32;
}
///`read()` method returns [`c3tr2::R`](R) reader structure
impl crate::Readable for C3TR2rs {}
///`write(|w| ..)` method takes [`c3tr2::W`](W) writer structure
impl crate::Writable for C3TR2rs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets C3TR2 to value 0
impl crate::Resettable for C3TR2rs {
