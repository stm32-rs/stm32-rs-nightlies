...
///Field `IDMALA` reader - Word aligned linked list item address offset
pub type IDMALA_R = crate::FieldReader<u16>;
///Field `IDMALA` writer - Word aligned linked list item address offset
...
///Field `ULS` reader - Update SDMMC_IDMABSIZE from the next linked list when in linked list mode (SDMMC_IDMACTRLR.
pub type ULS_R = crate::BitReader;
///Field `ULS` writer - Update SDMMC_IDMABSIZE from the next linked list when in linked list mode (SDMMC_IDMACTRLR.
pub type ULS_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ULA` reader - Update SDMMC_IDMALAR from linked list when in linked list mode (SDMMC_IDMACTRLR.
pub type ULA_R = crate::BitReader;
///Field `ULA` writer - Update SDMMC_IDMALAR from linked list when in linked list mode (SDMMC_IDMACTRLR.
...
impl W {
    ///Bits 2:15 - Word aligned linked list item address offset
    #[inline(always)]
    pub fn idmala(&mut self) -> IDMALA_W<'_, IDMALARrs> {
        IDMALA_W::new(self, 2)
    }
    ///Bit 29 - Acknowledge linked list buffer ready
    #[inline(always)]
    pub fn abr(&mut self) -> ABR_W<'_, IDMALARrs> {
        ABR_W::new(self, 29)
    }
    ///Bit 30 - Update SDMMC_IDMABSIZE from the next linked list when in linked list mode (SDMMC_IDMACTRLR.
    #[inline(always)]
    pub fn uls(&mut self) -> ULS_W<'_, IDMALARrs> {
        ULS_W::new(self, 30)
    }
    ///Bit 31 - Update SDMMC_IDMALAR from linked list when in linked list mode (SDMMC_IDMACTRLR.
    #[inline(always)]
    pub fn ula(&mut self) -> ULA_W<'_, IDMALARrs> {
        ULA_W::new(self, 31)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H523.html#SDMMC1:IDMALAR)*/
