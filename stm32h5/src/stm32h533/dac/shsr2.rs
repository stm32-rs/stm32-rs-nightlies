...
///Field `TSAMPLE2` reader - DAC channel2 sample time (only valid in Sample and hold mode)
pub type TSAMPLE2_R = crate::FieldReader<u16>;
///Field `TSAMPLE2` writer - DAC channel2 sample time (only valid in Sample and hold mode)
pub type TSAMPLE2_W<'a, REG> = crate::FieldWriter<'a, REG, 10, u16>;
impl R {
    ///Bits 0:9 - DAC channel2 sample time (only valid in Sample and hold mode)
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H533.html#DAC:SHSR2)*/
pub struct SHSR2rs;
