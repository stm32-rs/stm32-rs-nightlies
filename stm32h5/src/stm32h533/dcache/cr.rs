...
///Field `CACHECMD` reader - cache command maintenance operation (cleans and/or invalidates an address range)
pub type CACHECMD_R = crate::FieldReader;
///Field `CACHECMD` writer - cache command maintenance operation (cleans and/or invalidates an address range)
pub type CACHECMD_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
///Field `STARTCMD` writer - starts maintenance command (maintenance operation defined in CACHECMD).
...
///Field `HBURST` reader - output burst type for cache master port read accesses
pub type HBURST_R = crate::BitReader;
///Field `HBURST` writer - output burst type for cache master port read accesses
