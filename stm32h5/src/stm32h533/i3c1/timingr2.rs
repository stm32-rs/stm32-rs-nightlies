...
impl W {
    ///Bit 0 - Controller clock stall enable on T-bit phase of data (and on the ACK/NACK phase of data byte of a legacy Iless thansup>2less than/sup>C read)
    #[inline(always)]
    pub fn stallt(&mut self) -> STALLT_W<TIMINGR2rs> {
        STALLT_W::new(self, 0)
    }
    ///Bit 1 - Controller clock stall enable on PAR phase of Data
    #[inline(always)]
    pub fn stalld(&mut self) -> STALLD_W<TIMINGR2rs> {
        STALLD_W::new(self, 1)
    }
    ///Bit 2 - Controller clock stall enable on PAR phase of CCC
    #[inline(always)]
    pub fn stallc(&mut self) -> STALLC_W<TIMINGR2rs> {
        STALLC_W::new(self, 2)
    }
    ///Bit 3 - Controller clock stall enable on ACK phase
    #[inline(always)]
    pub fn stalla(&mut self) -> STALLA_W<TIMINGR2rs> {
        STALLA_W::new(self, 3)
    }
    ///Bits 8:15 - Controller clock stall time, in number of kernel clock cycles
    #[inline(always)]
    pub fn stall(&mut self) -> STALL_W<TIMINGR2rs> {
        STALL_W::new(self, 8)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H533.html#I3C1:TIMINGR2)*/
