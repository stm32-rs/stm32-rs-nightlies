...
///Field `XONEIE` reader - Execute-only execute-never error interrupt enable
pub type XONEIE_R = crate::BitReader;
///Field `XONEIE` writer - Execute-only execute-never error interrupt enable
...
impl R {
    ...
    ///Bit 1 - Execute-only execute-never error interrupt enable
    #[inline(always)]
...
impl W {
    ///Bit 0 - Security error interrupt enable
    #[inline(always)]
    pub fn seie(&mut self) -> SEIE_W<'_, IERrs> {
        SEIE_W::new(self, 0)
    }
    ///Bit 1 - Execute-only execute-never error interrupt enable
    #[inline(always)]
    pub fn xoneie(&mut self) -> XONEIE_W<'_, IERrs> {
        XONEIE_W::new(self, 1)
    }
    ///Bit 2 - Key error interrupt enable
    #[inline(always)]
