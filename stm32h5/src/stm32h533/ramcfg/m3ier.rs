...
impl W {
    ///Bit 0 - ECC single error interrupt enable
    #[inline(always)]
    pub fn seie(&mut self) -> SEIE_W<'_, M3IERrs> {
        SEIE_W::new(self, 0)
    }
    ///Bit 1 - ECC double error interrupt enable
    #[inline(always)]
    pub fn deie(&mut self) -> DEIE_W<'_, M3IERrs> {
        DEIE_W::new(self, 1)
    }
    ///Bit 3 - Double error NMI
    #[inline(always)]
    pub fn eccnmi(&mut self) -> ECCNMI_W<'_, M3IERrs> {
        ECCNMI_W::new(self, 3)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H533.html#RAMCFG:M3IER)*/
pub struct M3IERrs;
