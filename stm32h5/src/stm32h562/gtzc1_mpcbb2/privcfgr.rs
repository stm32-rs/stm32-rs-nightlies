...
///Field `PRIV(0-31)` reader - Privileged configuration for block y, belonging to super-block x (y = 31 to 0). Non-secure write to this bit is ignored if SECy bit is set in GTZC1_MPCBBz_SECCFGRx. Writes are ignored if SPLCKx bit is set in GTZC1_MPCBBz_CFGLOCK.
pub type PRIV_R = crate::BitReader;
///Field `PRIV(0-31)` writer - Privileged configuration for block y, belonging to super-block x (y = 31 to 0). Non-secure write to this bit is ignored if SECy bit is set in GTZC1_MPCBBz_SECCFGRx. Writes are ignored if SPLCKx bit is set in GTZC1_MPCBBz_CFGLOCK.
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H562.html#GTZC1_MPCBB2:PRIVCFGR[0])*/
