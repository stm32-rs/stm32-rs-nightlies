...
///Field `IDMABASE` reader - Buffer memory base address bits \[31:2\], must be word aligned (bit \[1:0\] are always 0 and read only) This register can be written by firmware when DPSM is inactive (DPSMACT = 0), and can dynamically be written by firmware when DPSM active (DPSMACT = 1).
pub type IDMABASE_R = crate::FieldReader<u32>;
///Field `IDMABASE` writer - Buffer memory base address bits \[31:2\], must be word aligned (bit \[1:0\] are always 0 and read only) This register can be written by firmware when DPSM is inactive (DPSMACT = 0), and can dynamically be written by firmware when DPSM active (DPSMACT = 1).
...
impl W {
    ///Bits 0:31 - Buffer memory base address bits \[31:2\], must be word aligned (bit \[1:0\] are always 0 and read only) This register can be written by firmware when DPSM is inactive (DPSMACT = 0), and can dynamically be written by firmware when DPSM active (DPSMACT = 1).
    #[inline(always)]
    pub fn idmabase(&mut self) -> IDMABASE_W<'_, IDMABASERrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H562.html#SDMMC1:IDMABASER)*/
