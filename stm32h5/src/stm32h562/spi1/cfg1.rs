...
///Field `DSIZE` reader - number of bits in at single SPI data frame ..... Note: Maximum data size can be limited up to 16-bits at some instances. At instances with limited set of features, DSIZE2:0\] bits are reserved and must be kept at reset state. DSIZE\[4:3\] bits then control next settings of data size: 00xxx: 8-bits 01xxx: 16-bits 10xxx: 24-bits 11xxx: 32-bits.
pub type DSIZE_R = crate::FieldReader;
///Field `DSIZE` writer - number of bits in at single SPI data frame ..... Note: Maximum data size can be limited up to 16-bits at some instances. At instances with limited set of features, DSIZE2:0\] bits are reserved and must be kept at reset state. DSIZE\[4:3\] bits then control next settings of data size: 00xxx: 8-bits 01xxx: 16-bits 10xxx: 24-bits 11xxx: 32-bits.
