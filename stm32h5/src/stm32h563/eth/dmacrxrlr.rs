...
///Field `ARBS` reader - Alternate Receive Buffer Size Indicates size in bytes for Buffer 1 when ARBS\[7:0\] is programmed to a non-zero value. When ARBS\[7:0\]=0, Rx Buffer1 and Rx Buffer2 sizes are based on RBSZ\[13:0\] field of Channel receive control register (ETH_DMACRXCR).
pub type ARBS_R = crate::FieldReader;
///Field `ARBS` writer - Alternate Receive Buffer Size Indicates size in bytes for Buffer 1 when ARBS\[7:0\] is programmed to a non-zero value. When ARBS\[7:0\]=0, Rx Buffer1 and Rx Buffer2 sizes are based on RBSZ\[13:0\] field of Channel receive control register (ETH_DMACRXCR).
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H563.html#ETH:DMACRXRLR)*/
