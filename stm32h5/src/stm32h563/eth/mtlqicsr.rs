...
///Field `TXUNFIS` reader - Transmit Queue Underflow Interrupt Status This bit indicates that the Transmit Queue had an underflow while transmitting the packet. Transmission is suspended and an Underflow Error TDES3\[2\] is set. This bit is cleared when the application writes 1 to this bit.
pub type TXUNFIS_R = crate::BitReader;
///Field `TXUNFIS` writer - Transmit Queue Underflow Interrupt Status This bit indicates that the Transmit Queue had an underflow while transmitting the packet. Transmission is suspended and an Underflow Error TDES3\[2\] is set. This bit is cleared when the application writes 1 to this bit.
