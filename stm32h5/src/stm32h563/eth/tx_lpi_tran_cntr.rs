...
///Field `TXLPITRC` reader - Tx LPI Transition counter This field indicates the number of times Tx LPI Entry has occurred. Even if Tx LPI Entry occurs in Automate mode (because of LPITXA bit set in the LPI control and status register (ETH_MACLCSR)), the counter is incremented.
pub type TXLPITRC_R = crate::FieldReader<u32>;
impl R {
    ///Bits 0:31 - Tx LPI Transition counter This field indicates the number of times Tx LPI Entry has occurred. Even if Tx LPI Entry occurs in Automate mode (because of LPITXA bit set in the LPI control and status register (ETH_MACLCSR)), the counter is incremented.
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H563.html#ETH:TX_LPI_TRAN_CNTR)*/
