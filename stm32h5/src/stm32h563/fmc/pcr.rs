...
impl R {
    ...
    ///Bits 13:16 - ALE to RE delay Sets time from ALE low to RE low in number of AHB clock cycles (HCLK). Time is: t_ar = (TAR + SET + 2) THCLK where THCLK is the HCLK clock period Note: SET is MEMSET or ATTSET according to the addressed space.
    #[inline(always)]
...
impl W {
    ...
    ///Bit 3 - Memory type Defines the type of device attached to the corresponding memory bank:
    #[inline(always)]
    pub fn ptyp(&mut self) -> PTYP_W<'_, PCRrs> {
        PTYP_W::new(self, 3)
    }
    ///Bits 4:5 - Data bus width Defines the external memory device width.
    #[inline(always)]
    ...
    ///Bits 17:19 - ECC page size Defines the page size for the extended ECC:
    #[inline(always)]
    pub fn eccps(&mut self) -> ECCPS_W<'_, PCRrs> {
        ECCPS_W::new(self, 17)
    }
}
