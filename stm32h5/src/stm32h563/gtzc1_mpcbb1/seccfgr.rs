...
///Field `SEC(0-31)` reader - Security configuration for block y Unprivileged write to this bit is ignored if PRIVy bit is set in GTZC1_MPCBBz_PRIVCFGRx. Writes are ignored if SPLCKx bit is set in GTZC1_MPCBBz_CFGLOCK.
pub type SEC_R = crate::BitReader;
///Field `SEC(0-31)` writer - Security configuration for block y Unprivileged write to this bit is ignored if PRIVy bit is set in GTZC1_MPCBBz_PRIVCFGRx. Writes are ignored if SPLCKx bit is set in GTZC1_MPCBBz_CFGLOCK.
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H563.html#GTZC1_MPCBB1:SECCFGR[0])*/
