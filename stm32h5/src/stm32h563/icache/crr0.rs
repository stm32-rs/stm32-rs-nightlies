...
impl W {
    ///Bits 0:7 - base address for region x This alias address is replaced by REMAPADDR field. The only useful bits are \[28:RI\], where 21 less than or equal RI less than or equal 27 is the number of bits of RSIZE (see ). If the programmed value has more LSBs, the useless bits are ignored.
    #[inline(always)]
    pub fn baseaddr(&mut self) -> BASEADDR_W<'_, CRR0rs> {
        BASEADDR_W::new(self, 0)
    }
    ///Bits 9:11 - size for region x
    #[inline(always)]
    pub fn rsize(&mut self) -> RSIZE_W<'_, CRR0rs> {
        RSIZE_W::new(self, 9)
    }
    ///Bit 15 - enable for region x
    #[inline(always)]
    pub fn ren(&mut self) -> REN_W<'_, CRR0rs> {
        REN_W::new(self, 15)
    }
    ///Bits 16:26 - remapped address for region x This field replaces the alias address defined by BASEADDR field. The only useful bits are \[31:RI\], where 21 less than or equal RI less than or equal 27 is the number of bits of RSIZE (see ). If the programmed value has more LSBs, the useless bits are ignored.
    #[inline(always)]
    pub fn remapaddr(&mut self) -> REMAPADDR_W<'_, CRR0rs> {
        REMAPADDR_W::new(self, 16)
    }
    ///Bit 28 - AHB cache master selection for region x
    #[inline(always)]
    pub fn mstsel(&mut self) -> MSTSEL_W<'_, CRR0rs> {
        MSTSEL_W::new(self, 28)
    }
    ///Bit 31 - output burst type for region x
    #[inline(always)]
    pub fn hburst(&mut self) -> HBURST_W<'_, CRR0rs> {
