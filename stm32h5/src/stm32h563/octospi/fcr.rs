...
///Field `CTEF` writer - Clear transfer error flag Writing 1 clears the TEF flag in the OCTOSPI_SR register.
...
///Field `CTCF` writer - Clear transfer complete flag Writing 1 clears the TCF flag in the OCTOSPI_SR register.
...
///Field `CSMF` writer - Clear status match flag Writing 1 clears the SMF flag in the OCTOSPI_SR register.
...
///Field `CTOF` writer - Clear timeout flag Writing 1 clears the TOF flag in the OCTOSPI_SR register.
...
impl W {
    ///Bit 0 - Clear transfer error flag Writing 1 clears the TEF flag in the OCTOSPI_SR register.
    #[inline(always)]
    pub fn ctef(&mut self) -> CTEF_W<'_, FCRrs> {
        CTEF_W::new(self, 0)
    }
    ///Bit 1 - Clear transfer complete flag Writing 1 clears the TCF flag in the OCTOSPI_SR register.
    #[inline(always)]
    pub fn ctcf(&mut self) -> CTCF_W<'_, FCRrs> {
        CTCF_W::new(self, 1)
    }
    ///Bit 3 - Clear status match flag Writing 1 clears the SMF flag in the OCTOSPI_SR register.
    #[inline(always)]
    pub fn csmf(&mut self) -> CSMF_W<'_, FCRrs> {
        CSMF_W::new(self, 3)
    }
    ///Bit 4 - Clear timeout flag Writing 1 clears the TOF flag in the OCTOSPI_SR register.
    #[inline(always)]
