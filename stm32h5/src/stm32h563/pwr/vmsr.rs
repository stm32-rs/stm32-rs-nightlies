...
/**analog voltage detector output on Vsub DDA/sub This bit is set and cleared by hardware. It is valid only if AVD on VDDA is enabled by the AVDEN bit. Note: Since the AVD is disabled in Standby mode, this bit is equal to 0 after standby or reset until the AVDEN bit is set.
...
///Field `AVDO` reader - analog voltage detector output on Vsub DDA/sub This bit is set and cleared by hardware. It is valid only if AVD on VDDA is enabled by the AVDEN bit. Note: Since the AVD is disabled in Standby mode, this bit is equal to 0 after standby or reset until the AVDEN bit is set.
...
/**voltage detector output on Vsub DDIO2/sub This bit is set and cleared by hardware.
...
///Field `VDDIO2RDY` reader - voltage detector output on Vsub DDIO2/sub This bit is set and cleared by hardware.
...
///Field `USB33RDY` reader - Vsub DDUSB/sub ready
pub type USB33RDY_R = crate::BitReader;
impl R {
    ///Bit 19 - analog voltage detector output on Vsub DDA/sub This bit is set and cleared by hardware. It is valid only if AVD on VDDA is enabled by the AVDEN bit. Note: Since the AVD is disabled in Standby mode, this bit is equal to 0 after standby or reset until the AVDEN bit is set.
    #[inline(always)]
    pub fn avdo(&self) -> AVDO_R {
        AVDO_R::new(((self.bits >> 19) & 1) != 0)
    }
    ///Bit 20 - voltage detector output on Vsub DDIO2/sub This bit is set and cleared by hardware.
    #[inline(always)]
