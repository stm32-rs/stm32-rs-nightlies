...
///Field `PW` reader - Pulse width This bitfield defines the pulse duration, as following: tsub PW/sub = PW\[7:0\] x tsub PWG/sub
pub type PW_R = crate::FieldReader;
///Field `PW` writer - Pulse width This bitfield defines the pulse duration, as following: tsub PW/sub = PW\[7:0\] x tsub PWG/sub
pub type PW_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `PWPRSC` reader - Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: tsub PWG/sub = (2sup (PWPRSC\[2:0\])/sup ) x tsub tim_ker_ck/sub
pub type PWPRSC_R = crate::FieldReader;
///Field `PWPRSC` writer - Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: tsub PWG/sub = (2sup (PWPRSC\[2:0\])/sup ) x tsub tim_ker_ck/sub
...
impl R {
    ...
    ///Bits 16:23 - Pulse width This bitfield defines the pulse duration, as following: tsub PW/sub = PW\[7:0\] x tsub PWG/sub
    #[inline(always)]
    pub fn pw(&self) -> PW_R {
        PW_R::new(((self.bits >> 16) & 0xff) as u8)
    }
    ///Bits 24:26 - Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: tsub PWG/sub = (2sup (PWPRSC\[2:0\])/sup ) x tsub tim_ker_ck/sub
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H563.html#TIM1:ECR)*/
pub struct ECRrs;
