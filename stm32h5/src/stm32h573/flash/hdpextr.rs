...
///Field `HDP1_EXT` reader - HDP area extension in 8Kbytes sectors in Bank1. Extension is added after the HDP1_END sector (included).
pub type HDP1_EXT_R = crate::FieldReader;
///Field `HDP1_EXT` writer - HDP area extension in 8Kbytes sectors in Bank1. Extension is added after the HDP1_END sector (included).
pub type HDP1_EXT_W<'a, REG> = crate::FieldWriter<'a, REG, 7>;
///Field `HDP2_EXT` reader - HDP area extension in 8Kbytes sectors in bank 2. Extension is added after the HDP2_END sector (included).
pub type HDP2_EXT_R = crate::FieldReader;
///Field `HDP2_EXT` writer - HDP area extension in 8Kbytes sectors in bank 2. Extension is added after the HDP2_END sector (included).
pub type HDP2_EXT_W<'a, REG> = crate::FieldWriter<'a, REG, 7>;
impl R {
    ///Bits 0:6 - HDP area extension in 8Kbytes sectors in Bank1. Extension is added after the HDP1_END sector (included).
    #[inline(always)]
    pub fn hdp1_ext(&self) -> HDP1_EXT_R {
        HDP1_EXT_R::new((self.bits & 0x7f) as u8)
    }
    ///Bits 16:22 - HDP area extension in 8Kbytes sectors in bank 2. Extension is added after the HDP2_END sector (included).
    #[inline(always)]
