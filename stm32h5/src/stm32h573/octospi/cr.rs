...
///Field `TCEN` reader - Timeout counter enable This bit is valid only when the Memory-mapped mode (FMODE\[1:0\] = 11) is selected. This bit enables the timeout counter.
...
///Field `TCEN` writer - Timeout counter enable This bit is valid only when the Memory-mapped mode (FMODE\[1:0\] = 11) is selected. This bit enables the timeout counter.
...
impl R {
    ...
    ///Bit 3 - Timeout counter enable This bit is valid only when the Memory-mapped mode (FMODE\[1:0\] = 11) is selected. This bit enables the timeout counter.
    #[inline(always)]
    ...
    ///Bits 8:12 - FIFO threshold level This field defines, in Indirect mode, the threshold number of bytes in the FIFO that causes the FIFO threshold flag FTF in OCTOSPI_SR, to be set. ... Note: If DMAEN = 1, the DMA controller for the corresponding channel must be disabled before changing the FTHRES\[4:0\] value.
    #[inline(always)]
