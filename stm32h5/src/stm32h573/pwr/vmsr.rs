...
pub enum AVDOR {
    /**0: VDDA is equal or higher than the AVD threshold selected with the ALS\[2:0\]
    bits*/
    AboveThreshold = 0,
    /**1: VDDA is lower than the AVD threshold selected with the ALS\[2:0\]
    bits*/
    BelowThreshold = 1,
}
...
impl AVDO_R {
    ...
    /**VDDA is equal or higher than the AVD threshold selected with the ALS\[2:0\]
    bits*/
    #[inline(always)]
    pub fn is_above_threshold(&self) -> bool {
        *self == AVDOR::AboveThreshold
    }
    /**VDDA is lower than the AVD threshold selected with the ALS\[2:0\]
    bits*/
    #[inline(always)]
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PVDOR {
    /**0: VDD is equal or higher than the PVD threshold selected through the PLS\[2:0\]
    bits.*/
    AboveThreshold = 0,
    /**1: VDD is lower than the PVD threshold selected through the PLS\[2:0\]
    bits*/
    BelowThreshold = 1,
}
...
impl PVDO_R {
    ...
    /**VDD is equal or higher than the PVD threshold selected through the PLS\[2:0\]
    bits.*/
    #[inline(always)]
    pub fn is_above_threshold(&self) -> bool {
        *self == PVDOR::AboveThreshold
    }
    /**VDD is lower than the PVD threshold selected through the PLS\[2:0\]
    bits*/
    #[inline(always)]
    pub fn is_below_threshold(&self) -> bool {
