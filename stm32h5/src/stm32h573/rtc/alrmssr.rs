...
/**Field `SS` reader - Sub seconds value This value is compared with the contents of the synchronous prescaler counter to determine if alarm A is to be activated. Only bits 0 up MASKSS-1 are compared. This field is the mirror of SS\[14:0\]
in the RTC_ALRMABINR, and so can also be read or written through RTC_ALRMABINR.*/
pub type SS_R = crate::FieldReader<u16>;
/**Field `SS` writer - Sub seconds value This value is compared with the contents of the synchronous prescaler counter to determine if alarm A is to be activated. Only bits 0 up MASKSS-1 are compared. This field is the mirror of SS\[14:0\]
in the RTC_ALRMABINR, and so can also be read or written through RTC_ALRMABINR.*/
pub type SS_W<'a, REG> = crate::FieldWriter<'a, REG, 15, u16>;
/**Field `MASKSS` reader - Mask the most-significant bits starting at this bit 2: SS\[31:2\]
...
/**Field `MASKSS` writer - Mask the most-significant bits starting at this bit 2: SS\[31:2\]
are don’t care in Alarm A comparison. Only SS\[1:0\]
are compared. ... 31: SS\[31\]
is don’t care in Alarm A comparison. Only SS\[30:0\]
are compared. From 32 to 63: All 32 SS bits are compared and must match to activate alarm. Note: In BCD mode (BIN=00) the overflow bits of the synchronous counter (bits 31:15) are never compared. These bits can be different from 0 only after a shift operation.*/
...
impl R {
    /**Bits 0:14 - Sub seconds value This value is compared with the contents of the synchronous prescaler counter to determine if alarm A is to be activated. Only bits 0 up MASKSS-1 are compared. This field is the mirror of SS\[14:0\]
    in the RTC_ALRMABINR, and so can also be read or written through RTC_ALRMABINR.*/
    #[inline(always)]
    pub fn ss(&self) -> SS_R {
        SS_R::new((self.bits & 0x7fff) as u16)
    }
    /**Bits 24:29 - Mask the most-significant bits starting at this bit 2: SS\[31:2\]
    are don’t care in Alarm A comparison. Only SS\[1:0\]
    are compared. ... 31: SS\[31\]
    is don’t care in Alarm A comparison. Only SS\[30:0\]
    are compared. From 32 to 63: All 32 SS bits are compared and must match to activate alarm. Note: In BCD mode (BIN=00) the overflow bits of the synchronous counter (bits 31:15) are never compared. These bits can be different from 0 only after a shift operation.*/
