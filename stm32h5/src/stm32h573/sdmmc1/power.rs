...
///Field `PWRCTRL` reader - SDMMC state control bits These bits can only be written when the SDMMC is not in the power-on state (PWRCTRL different 11). These bits are used to define the functional state of the SDMMC signals: stopped, SDMMC_D\[7:0\], SDMMC_CMD and SDMMC_CK are driven high. Any further write is ignored, PWRCTRL value keeps 11.
pub type PWRCTRL_R = crate::FieldReader;
///Field `PWRCTRL` writer - SDMMC state control bits These bits can only be written when the SDMMC is not in the power-on state (PWRCTRL different 11). These bits are used to define the functional state of the SDMMC signals: stopped, SDMMC_D\[7:0\], SDMMC_CMD and SDMMC_CK are driven high. Any further write is ignored, PWRCTRL value keeps 11.
...
impl R {
    ///Bits 0:1 - SDMMC state control bits These bits can only be written when the SDMMC is not in the power-on state (PWRCTRL different 11). These bits are used to define the functional state of the SDMMC signals: stopped, SDMMC_D\[7:0\], SDMMC_CMD and SDMMC_CK are driven high. Any further write is ignored, PWRCTRL value keeps 11.
    #[inline(always)]
...
impl W {
    ...
    ///Bit 2 - Voltage switch sequence start This bit is used to start the timing critical section of the voltage switch sequence:
    #[inline(always)]
    pub fn vswitch(&mut self) -> VSWITCH_W<'_, POWERrs> {
        VSWITCH_W::new(self, 2)
    }
    ///Bit 3 - Voltage switch procedure enable This bit can only be written by firmware when CPSM is disabled (CPSMEN = 0). This bit is used to stop the SDMMC_CK after the voltage switch command response:
    #[inline(always)]
    pub fn vswitchen(&mut self) -> VSWITCHEN_W<'_, POWERrs> {
        VSWITCHEN_W::new(self, 3)
    }
    ///Bit 4 - Data and command direction signals polarity selection This bit can only be written when the SDMMC is in the power-off state (PWRCTRL = 00).
    #[inline(always)]
    pub fn dirpol(&mut self) -> DIRPOL_W<'_, POWERrs> {
        DIRPOL_W::new(self, 4)
    }
}
/**SDMMC_POWER
