...
impl R {
    ///Bits 0:1 - SDMMC state control bits These bits can only be written when the SDMMC is not in the power-on state (PWRCTRL ≠ 11). These bits are used to define the functional state of the SDMMC signals: stopped, SDMMC_D\[7:0\], SDMMC_CMD and SDMMC_CK are driven high. Any further write is ignored, PWRCTRL value keeps 11.
    #[inline(always)]
