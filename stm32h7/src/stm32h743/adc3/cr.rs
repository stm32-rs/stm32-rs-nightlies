...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum BOOST {
    ///0: Boost mode off. Used when ADC clock < 20MHz
    Off = 0,
    ///1: Boost mode on. Used when ADC clock > 20MHz
    On = 1,
}
...
impl BOOST_R {
    ...
    ///Boost mode off. Used when ADC clock < 20MHz
    #[inline(always)]
    pub fn is_off(&self) -> bool {
        *self == BOOST::Off
    }
    ///Boost mode on. Used when ADC clock > 20MHz
    #[inline(always)]
