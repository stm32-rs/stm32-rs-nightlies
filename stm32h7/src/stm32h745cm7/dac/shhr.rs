...
impl W {
    ...
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `THOLD1` field.</div>
    #[inline(always)]
    pub fn thold(&mut self, n: u8) -> THOLD_W<'_, SHHRrs> {
        #[allow(clippy::no_effect)]
        [(); 2][n as usize];
    ...
    ///Bits 0:9 - DAC channel1 hold time (only valid in Sample and hold mode)
    #[inline(always)]
    pub fn thold1(&mut self) -> THOLD_W<'_, SHHRrs> {
        THOLD_W::new(self, 0)
    }
    ///Bits 16:25 - DAC channel2 hold time (only valid in Sample and hold mode)
    #[inline(always)]
    pub fn thold2(&mut self) -> THOLD_W<'_, SHHRrs> {
        THOLD_W::new(self, 16)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H745_CM7.html#DAC:SHHR)*/
pub struct SHHRrs;
