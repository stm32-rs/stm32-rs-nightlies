...
///Field `INTERVAL` reader - Polling interval Number of CLK cycles between to read during automatic polling phases. This field can be written only when BUSY = 0.
pub type INTERVAL_R = crate::FieldReader<u16>;
///Field `INTERVAL` writer - Polling interval Number of CLK cycles between to read during automatic polling phases. This field can be written only when BUSY = 0.
...
impl W {
    ///Bits 0:15 - Polling interval Number of CLK cycles between to read during automatic polling phases. This field can be written only when BUSY = 0.
    #[inline(always)]
    pub fn interval(&mut self) -> INTERVAL_W<'_, PIRrs> {
