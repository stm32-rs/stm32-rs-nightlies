...
impl W {
    ///Bit 2 - ECC single error interrupt enable
    #[inline(always)]
    pub fn eccseie(&mut self) -> ECCSEIE_W<M3CRrs> {
        ECCSEIE_W::new(self, 2)
    }
    ///Bit 3 - ECC double error interrupt enable
    #[inline(always)]
    pub fn eccdeie(&mut self) -> ECCDEIE_W<M3CRrs> {
        ECCDEIE_W::new(self, 3)
    }
    ///Bit 4 - ECC double error on byte write interrupt enable
    #[inline(always)]
    pub fn eccdebwie(&mut self) -> ECCDEBWIE_W<M3CRrs> {
        ECCDEBWIE_W::new(self, 4)
    }
    ///Bit 5 - ECC error context latching enable
    #[inline(always)]
    pub fn eccelen(&mut self) -> ECCELEN_W<M3CRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H747_CM4.html#RAMECC1:M3CR)*/
pub struct M3CRrs;
