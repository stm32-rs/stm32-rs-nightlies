#[doc = "Reader of register DMACCATxBR"]
pub type R = crate::R<u32, super::DMACCATXBR>;
#[doc = "Writer for register DMACCATxBR"]
pub type W = crate::W<u32, super::DMACCATXBR>;
#[doc = "Register DMACCATxBR `reset()`'s with value 0"]
impl crate::ResetValue for super::DMACCATXBR {
    type Type = u32;
    #[inline(always)]
    fn reset_value() -> Self::Type {
        0
    }
}
#[doc = "Reader of field `CURTBUFAPTR`"]
pub type CURTBUFAPTR_R = crate::R<u32, u32>;
#[doc = "Write proxy for field `CURTBUFAPTR`"]
pub struct CURTBUFAPTR_W<'a> {
    w: &'a mut W,
}
impl<'a> CURTBUFAPTR_W<'a> {
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub unsafe fn bits(self, value: u32) -> &'a mut W {
        self.w.bits = (self.w.bits & !0xffff_ffff) | ((value as u32) & 0xffff_ffff);
        self.w
    }
}
impl R {
    #[doc = "Bits 0:31 - Application Transmit Buffer Address Pointer"]
    #[inline(always)]
    pub fn curtbufaptr(&self) -> CURTBUFAPTR_R {
        CURTBUFAPTR_R::new((self.bits & 0xffff_ffff) as u32)
    }
}
impl W {
    #[doc = "Bits 0:31 - Application Transmit Buffer Address Pointer"]
    #[inline(always)]
    pub fn curtbufaptr(&mut self) -> CURTBUFAPTR_W {
        CURTBUFAPTR_W { w: self }
    }
}
