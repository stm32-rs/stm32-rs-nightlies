...
/**Field `ADDRESS` reader - 31 0\]: Address Address to be send to the external Flash memory Writes to this field are ignored when BUSY = 0 or when FMODE = 11 (memory-mapped mode). In dual flash mode, ADDRESS\[0\]
is automatically stuck to 0 as the address should always be even*/
pub type ADDRESS_R = crate::FieldReader<u32>;
/**Field `ADDRESS` writer - 31 0\]: Address Address to be send to the external Flash memory Writes to this field are ignored when BUSY = 0 or when FMODE = 11 (memory-mapped mode). In dual flash mode, ADDRESS\[0\]
is automatically stuck to 0 as the address should always be even*/
pub type ADDRESS_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32, crate::Safe>;
impl R {
    /**Bits 0:31 - 31 0\]: Address Address to be send to the external Flash memory Writes to this field are ignored when BUSY = 0 or when FMODE = 11 (memory-mapped mode). In dual flash mode, ADDRESS\[0\]
    is automatically stuck to 0 as the address should always be even*/
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H757_CM7.html#QUADSPI:AR)*/
pub struct ARrs;
