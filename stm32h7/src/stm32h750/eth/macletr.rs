...
/**Field `LPIET` reader - LPI Entry Timer This field specifies the time in microseconds the MAC will wait to enter LPI mode, after it has transmitted all the frames. This field is valid and used only when LPITE and LPITXA are set to 1. Bits \[2:0\]
are read-only so that the granularity of this timer is in steps of 8 micro-seconds.*/
pub type LPIET_R = crate::FieldReader<u32>;
/**Field `LPIET` writer - LPI Entry Timer This field specifies the time in microseconds the MAC will wait to enter LPI mode, after it has transmitted all the frames. This field is valid and used only when LPITE and LPITXA are set to 1. Bits \[2:0\]
are read-only so that the granularity of this timer is in steps of 8 micro-seconds.*/
pub type LPIET_W<'a, REG> = crate::FieldWriter<'a, REG, 20, u32>;
impl R {
    /**Bits 0:19 - LPI Entry Timer This field specifies the time in microseconds the MAC will wait to enter LPI mode, after it has transmitted all the frames. This field is valid and used only when LPITE and LPITXA are set to 1. Bits \[2:0\]
    are read-only so that the granularity of this timer is in steps of 8 micro-seconds.*/
    #[inline(always)]
