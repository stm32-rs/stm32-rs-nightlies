...
/**CTL (rw) register accessor: OTG device endpoint-0 control register
...
pub mod ctl;
/**INT (rw) register accessor: OTG device endpoint-0 interrupt register
...
pub mod int;
/**TSIZ (rw) register accessor: OTG_HS device IN endpoint 0 transfer size register
...
pub mod dma;
/**TXFSTS (r) register accessor: OTG_HS device IN endpoint transmit FIFO status register
