...
///Field `ALRBF` writer - Alarm B flag This flag is set by hardware when the time/date registers (RTC_TR and RTC_DR) match the Alarm B register (RTC_ALRMBR). This flag is cleared by software by writing 0.
...
impl From<ITSFR> for bool {
    #[inline(always)]
    fn from(variant: ITSFR) -> Self {
        variant as u8 != 0
    }
}
///Field `ITSF` reader - Internal tTime-stamp flag
pub type ITSF_R = crate::BitReader<ITSFR>;
impl ITSF_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> Option<ITSFR> {
        match self.bits {
            true => Some(ITSFR::Match),
            _ => None,
        }
    }
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ITSFW {
    ///0: This flag is cleared by software by writing 0, and must be cleared together with TSF bit by writing 0 in both bits
    Clear = 0,
}
impl From<ITSFW> for bool {
    #[inline(always)]
    fn from(variant: ITSFW) -> Self {
        variant as u8 != 0
    }
}
///Field `ITSF` writer - Internal tTime-stamp flag
pub type ITSF_W<'a, REG> = crate::BitWriter0C<'a, REG, ITSFW>;
impl<'a, REG> ITSF_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H753.html#RTC:ISR)*/
pub struct ISRrs;
...
impl crate::Writable for ISRrs {
    ...
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0x0002_ff20;
