...
///Field `EWIF` writer - Early wakeup interrupt flag This bit is set by hardware when the counter has reached the value 0x40. It must be cleared by software by writing 0. A write of 1 has no effect. This bit is also set if the interrupt is not enabled.
...
impl W {
    ///Bit 0 - Early wakeup interrupt flag This bit is set by hardware when the counter has reached the value 0x40. It must be cleared by software by writing 0. A write of 1 has no effect. This bit is also set if the interrupt is not enabled.
    #[inline(always)]
    pub fn ewif(&mut self) -> EWIF_W<'_, SRrs> {
        EWIF_W::new(self, 0)
    }
}
/**Status register
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H755_CM4.html#WWDG1:SR)*/
pub struct SRrs;
