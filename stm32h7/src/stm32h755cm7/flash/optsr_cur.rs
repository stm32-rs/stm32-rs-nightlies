...
impl W {
    ///Bit 0 - Option byte change ongoing flag
    #[inline(always)]
    pub fn opt_busy(&mut self) -> OPT_BUSY_W<'_, OPTSR_CURrs> {
        OPT_BUSY_W::new(self, 0)
    }
    ///Bits 2:3 - Brownout level option status bit
    #[inline(always)]
    pub fn bor_lev(&mut self) -> BOR_LEV_W<'_, OPTSR_CURrs> {
        BOR_LEV_W::new(self, 2)
    }
    ///Bit 4 - IWDG control mode option status bit
    #[inline(always)]
    pub fn iwdg_sw(&mut self) -> IWDG_SW_W<'_, OPTSR_CURrs> {
        IWDG_SW_W::new(self, 4)
    }
    ///Bit 5 - IWDG2 control mode option status bit
    #[inline(always)]
    pub fn iwdg2_sw(&mut self) -> IWDG2_SW_W<'_, OPTSR_CURrs> {
        IWDG2_SW_W::new(self, 5)
    }
    ///Bit 6 - D1 domain DStop entry reset option status bit
    #[inline(always)]
    pub fn nrst_stop_d1(&mut self) -> NRST_STOP_D1_W<'_, OPTSR_CURrs> {
        NRST_STOP_D1_W::new(self, 6)
    }
    ///Bit 7 - D1 domain DStandby entry reset option status bit
    #[inline(always)]
    pub fn rst_stdy_d1(&mut self) -> RST_STDY_D1_W<'_, OPTSR_CURrs> {
        RST_STDY_D1_W::new(self, 7)
    }
    ///Bits 8:15 - Readout protection level option status byte
    #[inline(always)]
    pub fn rdp(&mut self) -> RDP_W<'_, OPTSR_CURrs> {
        RDP_W::new(self, 8)
    }
    ///Bit 17 - IWDG Stop mode freeze option status bit
    #[inline(always)]
    pub fn iwdg_fz_stop(&mut self) -> IWDG_FZ_STOP_W<'_, OPTSR_CURrs> {
        IWDG_FZ_STOP_W::new(self, 17)
    }
    ///Bit 18 - IWDG Standby mode freeze option status bit
    #[inline(always)]
    pub fn iwdg_fz_sdby(&mut self) -> IWDG_FZ_SDBY_W<'_, OPTSR_CURrs> {
        IWDG_FZ_SDBY_W::new(self, 18)
    }
    ///Bits 19:20 - ST RAM size option status
    #[inline(always)]
    pub fn st_ram_size(&mut self) -> ST_RAM_SIZE_W<'_, OPTSR_CURrs> {
        ST_RAM_SIZE_W::new(self, 19)
    }
    ///Bit 21 - Security enable option status bit
    #[inline(always)]
    pub fn security(&mut self) -> SECURITY_W<'_, OPTSR_CURrs> {
        SECURITY_W::new(self, 21)
    }
    ///Bit 22 - Arm Cortex
    #[inline(always)]
    pub fn boot_cm4(&mut self) -> BOOT_CM4_W<'_, OPTSR_CURrs> {
        BOOT_CM4_W::new(self, 22)
    }
    ///Bit 23 - Arm Cortex
    #[inline(always)]
    pub fn boot_cm7(&mut self) -> BOOT_CM7_W<'_, OPTSR_CURrs> {
        BOOT_CM7_W::new(self, 23)
    }
    ///Bit 24 - D2 domain DStop entry reset option status bit
    #[inline(always)]
    pub fn nrst_stop_d2(&mut self) -> NRST_STOP_D2_W<'_, OPTSR_CURrs> {
        NRST_STOP_D2_W::new(self, 24)
    }
    ///Bit 25 - D2 domain DStandby entry reset option status bit
    #[inline(always)]
    pub fn nrst_stby_d2(&mut self) -> NRST_STBY_D2_W<'_, OPTSR_CURrs> {
        NRST_STBY_D2_W::new(self, 25)
    }
    ///Bit 29 - I
    #[inline(always)]
    pub fn io_hslv(&mut self) -> IO_HSLV_W<'_, OPTSR_CURrs> {
        IO_HSLV_W::new(self, 29)
    }
    ///Bit 30 - Option byte change error flag
    #[inline(always)]
    pub fn optchangeerr(&mut self) -> OPTCHANGEERR_W<'_, OPTSR_CURrs> {
        OPTCHANGEERR_W::new(self, 30)
    }
    ///Bit 31 - Bank swapping option status bit
    #[inline(always)]
    pub fn swap_bank_opt(&mut self) -> SWAP_BANK_OPT_W<'_, OPTSR_CURrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H755_CM7.html#FLASH:OPTSR_CUR)*/
