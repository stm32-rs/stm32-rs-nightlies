...
///Field `MASK` reader - Status mask Mask to be applied to the status bytes received in polling mode. For bit n: This field can be written only when BUSY = 0.
pub type MASK_R = crate::FieldReader<u32>;
///Field `MASK` writer - Status mask Mask to be applied to the status bytes received in polling mode. For bit n: This field can be written only when BUSY = 0.
...
impl W {
    ///Bits 0:31 - Status mask Mask to be applied to the status bytes received in polling mode. For bit n: This field can be written only when BUSY = 0.
    #[inline(always)]
    pub fn mask(&mut self) -> MASK_W<'_, PSMKRrs> {
        MASK_W::new(self, 0)
    }
}
