...
impl W {
    ///Bits 0:11 - Bank 1 PCROP area start status bits
    #[inline(always)]
    pub fn prot_area_start(&mut self) -> PROT_AREA_START_W<'_, PRAR_CURrs> {
        PROT_AREA_START_W::new(self, 0)
    }
    ///Bits 16:27 - Bank 1 PCROP area end status bits
    #[inline(always)]
    pub fn prot_area_end(&mut self) -> PROT_AREA_END_W<'_, PRAR_CURrs> {
        PROT_AREA_END_W::new(self, 16)
    }
    ///Bit 31 - Bank 1 PCROP protected erase enable option status bit
    #[inline(always)]
    pub fn dmep(&mut self) -> DMEP_W<'_, PRAR_CURrs> {
        DMEP_W::new(self, 31)
    }
}
