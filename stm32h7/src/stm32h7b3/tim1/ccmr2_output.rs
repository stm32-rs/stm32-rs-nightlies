...
pub type OCM_W<'a, REG> = crate::FieldWriter<'a, REG, 3, OC3M, crate::Safe>;
...
///Field `OCM_3(3-3)` reader - Output compare %s mode, bit 3
...
///Field `OCM_3(3-3)` writer - Output compare %s mode, bit 3
...
///Field `OC4M_4` reader - Output Compare 4 mode - bit 3
pub type OC4M_4_R = crate::BitReader;
///Field `OC4M_4` writer - Output Compare 4 mode - bit 3
pub type OC4M_4_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ...
    pub fn ocm_3(&self, n: u8) -> OCM_3_R {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        OCM_3_R::new(((self.bits >> (n * 0 + 16)) & 1) != 0)
    }
    ///Iterator for array of:
    ///Output compare (3-3) mode, bit 3
    #[inline(always)]
    pub fn ocm_3_iter(&self) -> impl Iterator<Item = OCM_3_R> + '_ {
        (0..1).map(move |n| OCM_3_R::new(((self.bits >> (n * 0 + 16)) & 1) != 0))
    }
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn oc4m_4(&mut self) -> OC4M_4_W<CCMR2_OUTPUTrs> {
        OC4M_4_W::new(self, 24)
    }
}
