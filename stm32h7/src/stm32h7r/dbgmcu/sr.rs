...
///Field `AP_PRESENT` reader - Bit n identifies whether access port AP n is open (can be accessed via the debug port) or locked (debug access to the AP is blocked) Bit n = 0: APn absent Bit n = 1: APn present
pub type AP_PRESENT_R = crate::FieldReader<u16>;
///Field `AP_ENABLED` reader - Bit n identifies whether access port AP n is open (can be accessed via the debug port) or locked (debug access to the AP is blocked) Bit n = 0: APn locked Bit n = 1: APn enabled
