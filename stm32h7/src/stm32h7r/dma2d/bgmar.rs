...
///Field `MA` reader - Memory address, address of the data used for the background image The address alignment must match the image format selected: a 32-bit per pixel format must be 32-bit aligned, a 16-bit per pixel format must be 16-bit aligned and a 4-bit per pixel format must be 8-bit aligned.
pub type MA_R = crate::FieldReader<u32>;
///Field `MA` writer - Memory address, address of the data used for the background image The address alignment must match the image format selected: a 32-bit per pixel format must be 32-bit aligned, a 16-bit per pixel format must be 16-bit aligned and a 4-bit per pixel format must be 8-bit aligned.
...
impl W {
    ///Bits 0:31 - Memory address, address of the data used for the background image The address alignment must match the image format selected: a 32-bit per pixel format must be 32-bit aligned, a 16-bit per pixel format must be 16-bit aligned and a 4-bit per pixel format must be 8-bit aligned.
    #[inline(always)]
    pub fn ma(&mut self) -> MA_W<'_, BGMARrs> {
        MA_W::new(self, 0)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7R.html#DMA2D:BGMAR)*/
pub struct BGMARrs;
