...
/**Read pipe These bits define the delay, in fmc_ker_ck clock cycles, for reading data after CAS latency. Note: The corresponding bits in the FMC_SDCR2 register is read only.
...
///Field `RPIPE` reader - Read pipe These bits define the delay, in fmc_ker_ck clock cycles, for reading data after CAS latency. Note: The corresponding bits in the FMC_SDCR2 register is read only.
...
///Field `RPIPE` writer - Read pipe These bits define the delay, in fmc_ker_ck clock cycles, for reading data after CAS latency. Note: The corresponding bits in the FMC_SDCR2 register is read only.
...
impl W {
    ...
    ///Bits 13:14 - Read pipe These bits define the delay, in fmc_ker_ck clock cycles, for reading data after CAS latency. Note: The corresponding bits in the FMC_SDCR2 register is read only.
    #[inline(always)]
    pub fn rpipe(&mut self) -> RPIPE_W<'_, SDCR2rs> {
        RPIPE_W::new(self, 13)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7R.html#FMC:SDCR2)*/
pub struct SDCR2rs;
