...
/**Field `BRSAO` reader - Block repeated source address offset For a channel with 2D addressing capability, this field is used to update (by addition or subtraction depending on HPDMA_CxBR1.BRSDEC) the current source address (HPDMA_CxSAR) at the end of a block transfer. A block repeated source address offset must be aligned with the programmed data width of a source burst (BRSAO\[2:0\]
versus HPDMA_CxTR1.SDW_LOG2\[1:0\]). Else a user setting error is reported and no transfer is issued. Note: BRSAO\[15:0\]
must be set to 0 in peripheral flow-control mode (if HPDMA_CxTR2.PFREQ=1).*/
...
versus HPDMA_CxTR1.DDW_LOG2\[1:0\]). Else a user setting error is reported and no transfer is issued. Note: BRDAO\[15:0\]
must be set to 0 in peripheral flow-control mode (if HPDMA_CxTR2.PFREQ=1).*/
...
impl W {
    ...
    must be set to 0 in peripheral flow-control mode (if HPDMA_CxTR2.PFREQ=1).*/
    #[inline(always)]
    pub fn brsao(&mut self) -> BRSAO_W<C15BR2rs> {
        BRSAO_W::new(self, 0)
    }
    /**Bits 16:31 - Block repeated destination address offset For a channel with 2D addressing capability, this field is used to update (by addition or subtraction depending on HPDMA_CxBR1.BRDDEC) the current destination address (HPDMA_CxDAR) at the end of a block transfer. A block repeated destination address offset must be aligned with the programmed data width of a destination burst (BRDAO\[2:0\]
    versus HPDMA_CxTR1.DDW_LOG2\[1:0\]). Else a user setting error is reported and no transfer is issued. Note: BRDAO\[15:0\]
    must be set to 0 in peripheral flow-control mode (if HPDMA_CxTR2.PFREQ=1).*/
    #[inline(always)]
    pub fn brdao(&mut self) -> BRDAO_W<C15BR2rs> {
        BRDAO_W::new(self, 16)
    }
}
