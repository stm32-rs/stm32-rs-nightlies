...
///Field `OA2MSK` reader - Own address 2 masks Note: These bits can be written only when OA2EN=0. Note: As soon as OA2MSK is not equal to 0, the reserved I2C addresses (0b0000xxx and 0b1111xxx) are not acknowledged even if the comparison matches.
pub type OA2MSK_R = crate::FieldReader;
///Field `OA2MSK` writer - Own address 2 masks Note: These bits can be written only when OA2EN=0. Note: As soon as OA2MSK is not equal to 0, the reserved I2C addresses (0b0000xxx and 0b1111xxx) are not acknowledged even if the comparison matches.
...
impl W {
    ///Bits 1:7 - Interface address 7-bit addressing mode: 7-bit address Note: These bits can be written only when OA2EN=0.
    #[inline(always)]
    pub fn oa2(&mut self) -> OA2_W<'_, OAR2rs> {
        OA2_W::new(self, 1)
    }
    ///Bits 8:10 - Own address 2 masks Note: These bits can be written only when OA2EN=0. Note: As soon as OA2MSK is not equal to 0, the reserved I2C addresses (0b0000xxx and 0b1111xxx) are not acknowledged even if the comparison matches.
    #[inline(always)]
    pub fn oa2msk(&mut self) -> OA2MSK_W<'_, OAR2rs> {
        OA2MSK_W::new(self, 8)
    }
    ///Bit 15 - Own address 2 enable
    #[inline(always)]
    pub fn oa2en(&mut self) -> OA2EN_W<'_, OAR2rs> {
        OA2EN_W::new(self, 15)
    }
}
