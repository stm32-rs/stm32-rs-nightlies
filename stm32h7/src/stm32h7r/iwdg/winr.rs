...
/**Field `WIN` reader - Watchdog counter window value These bits are write access protected, see Section 48.4.6.They contain the high limit of the window value to be compared with the downcounter. To prevent a reset, the IWDCNT downcounter must be reloaded when its value is lower than WIN\[11:0\]
...
/**Field `WIN` writer - Watchdog counter window value These bits are write access protected, see Section 48.4.6.They contain the high limit of the window value to be compared with the downcounter. To prevent a reset, the IWDCNT downcounter must be reloaded when its value is lower than WIN\[11:0\]
+ 1 and greater than 1. The WVU bit in the IWDG status register (IWDG_SR) must be reset to be able to change the reload value. Note: Reading this register returns the reload value from the V&lt;sub>DD&lt;/sub> voltage domain. This value may not be valid if a write operation to this register is ongoing. For this reason the value read from this register is valid only when the WVU bit in the IWDG status register (IWDG_SR) is reset.*/
pub type WIN_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
impl R {
    /**Bits 0:11 - Watchdog counter window value These bits are write access protected, see Section 48.4.6.They contain the high limit of the window value to be compared with the downcounter. To prevent a reset, the IWDCNT downcounter must be reloaded when its value is lower than WIN\[11:0\]
    + 1 and greater than 1. The WVU bit in the IWDG status register (IWDG_SR) must be reset to be able to change the reload value. Note: Reading this register returns the reload value from the V&lt;sub>DD&lt;/sub> voltage domain. This value may not be valid if a write operation to this register is ongoing. For this reason the value read from this register is valid only when the WVU bit in the IWDG status register (IWDG_SR) is reset.*/
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7R.html#IWDG:WINR)*/
pub struct WINRrs;
