...
///Field `WHSTPOS` reader - window horizontal start position These bits configure the first visible pixel of a line of the layer window. WHSTPOS\[11:0\] must be UNDER OR EQUAL AAW\[11:0\] bits (programmed in LTDC_AWCR register).
pub type WHSTPOS_R = crate::FieldReader<u16>;
///Field `WHSTPOS` writer - window horizontal start position These bits configure the first visible pixel of a line of the layer window. WHSTPOS\[11:0\] must be UNDER OR EQUAL AAW\[11:0\] bits (programmed in LTDC_AWCR register).
pub type WHSTPOS_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
///Field `WHSPPOS` reader - window horizontal stop position These bits configure the last visible pixel of a line of the layer window. WHSPPOS\[11:0\] must be more or equal to AHBP\[11:0\] bits + 1 (programmed in LTDC_BPCR register).
pub type WHSPPOS_R = crate::FieldReader<u16>;
///Field `WHSPPOS` writer - window horizontal stop position These bits configure the last visible pixel of a line of the layer window. WHSPPOS\[11:0\] must be more or equal to AHBP\[11:0\] bits + 1 (programmed in LTDC_BPCR register).
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7R.html#LTDC:L1WHPCR)*/
