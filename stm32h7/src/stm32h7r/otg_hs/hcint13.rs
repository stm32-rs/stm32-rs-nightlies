...
impl W {
    ///Bit 0 - Transfer completed. Transfer completed normally without any errors.
    #[inline(always)]
    pub fn xfrc(&mut self) -> XFRC_W<HCINT13rs> {
        XFRC_W::new(self, 0)
    }
    ///Bit 1 - Channel halted. Indicates the transfer completed abnormally either because of any USB transaction error or in response to disable request by the application.
    #[inline(always)]
    pub fn chh(&mut self) -> CHH_W<HCINT13rs> {
        CHH_W::new(self, 1)
    }
    ///Bit 2 - AHB error This error is generated only in Internal DMA mode when an AHB error occurs during an AHB read/write operation. The application can read the corresponding DMA channel address register to get the error address.
    #[inline(always)]
    pub fn ahberr(&mut self) -> AHBERR_W<HCINT13rs> {
        AHBERR_W::new(self, 2)
    }
    ///Bit 3 - STALL response received interrupt.
    #[inline(always)]
    pub fn stall(&mut self) -> STALL_W<HCINT13rs> {
        STALL_W::new(self, 3)
    }
    ///Bit 4 - NAK response received interrupt.
    #[inline(always)]
    pub fn nak(&mut self) -> NAK_W<HCINT13rs> {
        NAK_W::new(self, 4)
    }
    ///Bit 5 - ACK response received/transmitted interrupt.
    #[inline(always)]
    pub fn ack(&mut self) -> ACK_W<HCINT13rs> {
        ACK_W::new(self, 5)
    }
    ///Bit 6 - Not yet ready response received interrupt.
    #[inline(always)]
    pub fn nyet(&mut self) -> NYET_W<HCINT13rs> {
        NYET_W::new(self, 6)
    }
    ///Bit 7 - Transaction error. Indicates one of the following errors occurred on the USB. CRC check failure Timeout Bit stuff error False EOP
    #[inline(always)]
    pub fn txerr(&mut self) -> TXERR_W<HCINT13rs> {
        TXERR_W::new(self, 7)
    }
    ///Bit 8 - Babble error.
    #[inline(always)]
    pub fn bberr(&mut self) -> BBERR_W<HCINT13rs> {
        BBERR_W::new(self, 8)
    }
    ///Bit 9 - Frame overrun.
    #[inline(always)]
    pub fn frmor(&mut self) -> FRMOR_W<HCINT13rs> {
        FRMOR_W::new(self, 9)
    }
    ///Bit 10 - Data toggle error.
    #[inline(always)]
    pub fn dterr(&mut self) -> DTERR_W<HCINT13rs> {
        DTERR_W::new(self, 10)
    }
}
