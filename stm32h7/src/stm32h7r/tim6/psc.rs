...
///Field `PSC` reader - Prescaler value The counter clock frequency f<sub>tim_cnt_ck</sub> is equal to f<sub>tim_psc_ck</sub> / (PSC\[15:0\] + 1). PSC contains the value to be loaded into the active prescaler register at each update event. (including when the counter is cleared through UG bit of TIMx_EGR register.
pub type PSC_R = crate::FieldReader<u16>;
///Field `PSC` writer - Prescaler value The counter clock frequency f<sub>tim_cnt_ck</sub> is equal to f<sub>tim_psc_ck</sub> / (PSC\[15:0\] + 1). PSC contains the value to be loaded into the active prescaler register at each update event. (including when the counter is cleared through UG bit of TIMx_EGR register.
...
impl W {
    ///Bits 0:15 - Prescaler value The counter clock frequency f<sub>tim_cnt_ck</sub> is equal to f<sub>tim_psc_ck</sub> / (PSC\[15:0\] + 1). PSC contains the value to be loaded into the active prescaler register at each update event. (including when the counter is cleared through UG bit of TIMx_EGR register.
    #[inline(always)]
    pub fn psc(&mut self) -> PSC_W<'_, PSCrs> {
        PSC_W::new(self, 0)
    }
}
/**TIM6 prescaler
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7R.html#TIM6:PSC)*/
pub struct PSCrs;
