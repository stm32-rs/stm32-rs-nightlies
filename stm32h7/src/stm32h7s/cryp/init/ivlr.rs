...
/**Field `IVI` reader - Initialization vector bit x This bitfield stores the initialization vector bits \[127:96\]
...
/**Field `IVI` writer - Initialization vector bit x This bitfield stores the initialization vector bits \[127:96\]
for AES chaining modes other than ECB. The value stored in CRYP_IVxR/LR registers is updated by hardware after each computation round (when applicable). Write to this register is ignored when CRYP is busy (BUSY bit set).*/
pub type IVI_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    /**Bits 0:31 - Initialization vector bit x This bitfield stores the initialization vector bits \[127:96\]
    for AES chaining modes other than ECB. The value stored in CRYP_IVxR/LR registers is updated by hardware after each computation round (when applicable). Write to this register is ignored when CRYP is busy (BUSY bit set).*/
    #[inline(always)]
...
impl W {
    ...
    for AES chaining modes other than ECB. The value stored in CRYP_IVxR/LR registers is updated by hardware after each computation round (when applicable). Write to this register is ignored when CRYP is busy (BUSY bit set).*/
    #[inline(always)]
    pub fn ivi(&mut self) -> IVI_W<IVLRrs> {
        IVI_W::new(self, 0)
    }
}
