...
/**Field `K` writer - Key bit x This write-only bitfield contains the bits \[223:192\]
of the AES encryption or decryption key, depending on the operating mode. Refer to the CRYP_K0LR register for information relative to writing CRYP_KxR/LR registers.*/
...
impl W {
    /**Bits 0:31 - Key bit x This write-only bitfield contains the bits \[223:192\]
    of the AES encryption or decryption key, depending on the operating mode. Refer to the CRYP_K0LR register for information relative to writing CRYP_KxR/LR registers.*/
    #[inline(always)]
    #[must_use]
    pub fn k(&mut self) -> K_W<KRRrs> {
        K_W::new(self, 0)
    }
}
