...
/**Field `FRMCNT` reader - Frame counter, read-only, loops around. Incremented following VSYNC detection mapped to the pipe configured into bits PSFC\[1:0\]
of the DCMIPP_CMCR register. The counter is cleared using the CRC bit in the DCMIPP_CMCR register.*/
pub type FRMCNT_R = crate::FieldReader<u32>;
impl R {
    /**Bits 0:31 - Frame counter, read-only, loops around. Incremented following VSYNC detection mapped to the pipe configured into bits PSFC\[1:0\]
    of the DCMIPP_CMCR register. The counter is cleared using the CRC bit in the DCMIPP_CMCR register.*/
    #[inline(always)]
