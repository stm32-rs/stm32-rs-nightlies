...
///Field `HSIZE` reader - Horizontal size, from 0 to 4094 word wide (data 32-bit) If the value is maintained at 0 when enabling the crop by means of ENABLE bit, the crop operation is not performed on horizontal direction.
pub type HSIZE_R = crate::FieldReader<u16>;
///Field `HSIZE` writer - Horizontal size, from 0 to 4094 word wide (data 32-bit) If the value is maintained at 0 when enabling the crop by means of ENABLE bit, the crop operation is not performed on horizontal direction.
pub type HSIZE_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
///Field `VSIZE` reader - Vertical size, from 0 to 4094 pixels high If the value is maintained at 0 when enabling the crop by means of ENABLE bit, the crop operation is not performed on vertical direction.
pub type VSIZE_R = crate::FieldReader<u16>;
///Field `VSIZE` writer - Vertical size, from 0 to 4094 pixels high If the value is maintained at 0 when enabling the crop by means of ENABLE bit, the crop operation is not performed on vertical direction.
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn hsize(&mut self) -> HSIZE_W<P0SCSZRrs> {
        HSIZE_W::new(self, 0)
    }
    ///Bits 16:27 - Vertical size, from 0 to 4094 pixels high If the value is maintained at 0 when enabling the crop by means of ENABLE bit, the crop operation is not performed on vertical direction.
    #[inline(always)]
    #[must_use]
    pub fn vsize(&mut self) -> VSIZE_W<P0SCSZRrs> {
        VSIZE_W::new(self, 16)
    }
    ///Bit 30 - This bit is set and cleared by software. It has a meaning only if ENABLE bit is set.
    #[inline(always)]
    #[must_use]
    pub fn posneg(&mut self) -> POSNEG_W<P0SCSZRrs> {
        POSNEG_W::new(self, 30)
    }
    ///Bit 31 - This bit is set and cleared by software. if POSNEG = 0, the data inside the rectangle area are transmitted (it can correspond to a statistical data removal, or as a crop feature in a data valid image area). if POSNEG = 1, the data outside of the rectangle area are transmitted (it can correspond to a statistical data extraction, rejecting all data inside the window). This bit must be kept cleared if the input format is JPEG, to avoid unpredictable behavior of the pipe.
    #[inline(always)]
    #[must_use]
    pub fn enable(&mut self) -> ENABLE_W<P0SCSZRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7S.html#DCMIPP:P0SCSZR)*/
