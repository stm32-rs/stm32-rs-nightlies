...
///Field `SEL` reader - Phase for the output clock. These bits can only be written when SEN = 1. Output clock phase = input clock + SEL\[3:0\] x unit delay
pub type SEL_R = crate::FieldReader;
///Field `SEL` writer - Phase for the output clock. These bits can only be written when SEN = 1. Output clock phase = input clock + SEL\[3:0\] x unit delay
pub type SEL_W<'a, REG> = crate::FieldWriter<'a, REG, 4>;
///Field `UNIT` reader - Delay of a unit delay cell. These bits can only be written when SEN = 1. Unit delay = initial delay + UNIT\[6:0\] x delay step
pub type UNIT_R = crate::FieldReader;
///Field `UNIT` writer - Delay of a unit delay cell. These bits can only be written when SEN = 1. Unit delay = initial delay + UNIT\[6:0\] x delay step
...
///Field `LNGF` reader - Length valid flag This flag indicates when the delay line length value contained in LNG\[11:0\] is valid after UNIT\[6:0\] bits changed.
...
impl W {
    ///Bits 0:3 - Phase for the output clock. These bits can only be written when SEN = 1. Output clock phase = input clock + SEL\[3:0\] x unit delay
    #[inline(always)]
    pub fn sel(&mut self) -> SEL_W<'_, CFGRrs> {
        SEL_W::new(self, 0)
    }
    ///Bits 8:14 - Delay of a unit delay cell. These bits can only be written when SEN = 1. Unit delay = initial delay + UNIT\[6:0\] x delay step
    #[inline(always)]
    pub fn unit(&mut self) -> UNIT_W<'_, CFGRrs> {
        UNIT_W::new(self, 8)
    }
}
