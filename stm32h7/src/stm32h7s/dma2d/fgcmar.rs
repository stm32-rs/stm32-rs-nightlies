...
///Field `MA` reader - Memory address Address of the data used for the CLUT address dedicated to the foreground image. If the foreground CLUT format is 32-bit, the address must be 32-bit aligned.
pub type MA_R = crate::FieldReader<u32>;
///Field `MA` writer - Memory address Address of the data used for the CLUT address dedicated to the foreground image. If the foreground CLUT format is 32-bit, the address must be 32-bit aligned.
...
impl W {
    ///Bits 0:31 - Memory address Address of the data used for the CLUT address dedicated to the foreground image. If the foreground CLUT format is 32-bit, the address must be 32-bit aligned.
    #[inline(always)]
    pub fn ma(&mut self) -> MA_W<'_, FGCMARrs> {
        MA_W::new(self, 0)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7S.html#DMA2D:FGCMAR)*/
pub struct FGCMARrs;
