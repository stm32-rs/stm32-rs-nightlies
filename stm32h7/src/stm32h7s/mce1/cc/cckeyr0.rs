...
This register is used by the block or stream cipher of MCE when CTXID = z in encrypted region configuration register. KEY\[127:0\]
must be correctly initialize before CCEN bit is set in MCE_CCzCFGR register.*/
...
impl W {
    ...
    must be correctly initialize before CCEN bit is set in MCE_CCzCFGR register.*/
    #[inline(always)]
    pub fn key(&mut self) -> KEY_W<CCKEYR0rs> {
        KEY_W::new(self, 0)
    }
}
