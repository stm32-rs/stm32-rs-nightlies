...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn tx0fsa(&mut self) -> TX0FSA_W<HNPTXFSIZ_DEVICErs> {
        TX0FSA_W::new(self, 0)
    }
    ///Bits 16:31 - Endpoint 0 Tx FIFO depth This value is in terms of 32-bit words. Minimum value is 16 Programmed values must respect the available FIFO memory allocation and must not exceed the power-on value.
    #[inline(always)]
    #[must_use]
    pub fn tx0fd(&mut self) -> TX0FD_W<HNPTXFSIZ_DEVICErs> {
        TX0FD_W::new(self, 16)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7S.html#OTG_HS:HNPTXFSIZ_DEVICE)*/
