...
///Field `NPTXFSA` reader - Non-periodic transmit RAM start address This field configures the memory start address for non-periodic transmit FIFO RAM.
pub type NPTXFSA_R = crate::FieldReader<u16>;
///Field `NPTXFSA` writer - Non-periodic transmit RAM start address This field configures the memory start address for non-periodic transmit FIFO RAM.
pub type NPTXFSA_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `NPTXFD` reader - Non-periodic Tx FIFO depth This value is in terms of 32-bit words. Minimum value is 16 Programmed values must respect the available FIFO memory allocation and must not exceed the power-on value.
pub type NPTXFD_R = crate::FieldReader<u16>;
///Field `NPTXFD` writer - Non-periodic Tx FIFO depth This value is in terms of 32-bit words. Minimum value is 16 Programmed values must respect the available FIFO memory allocation and must not exceed the power-on value.
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7S.html#OTG_HS:HNPTXFSIZ_HOST)*/
...
///`reset()` method sets HNPTXFSIZ_HOST to value 0x0200_0200
