...
/**Field `PVD_BL` reader - PVD break lock This bit is set by SW and cleared only by a system reset. it can be used to enable and lock the connection to TIM1/8/15/16/17Break input as well as the PVDE and PLS\[2:0\]
bitfields in the PWR_CR1 register. Once set, this bit is cleared only by a system reset.*/
pub type PVD_BL_R = crate::BitReader;
/**Field `PVD_BL` writer - PVD break lock This bit is set by SW and cleared only by a system reset. it can be used to enable and lock the connection to TIM1/8/15/16/17Break input as well as the PVDE and PLS\[2:0\]
bitfields in the PWR_CR1 register. Once set, this bit is cleared only by a system reset.*/
...
impl R {
    /**Bit 2 - PVD break lock This bit is set by SW and cleared only by a system reset. it can be used to enable and lock the connection to TIM1/8/15/16/17Break input as well as the PVDE and PLS\[2:0\]
    bitfields in the PWR_CR1 register. Once set, this bit is cleared only by a system reset.*/
    #[inline(always)]
