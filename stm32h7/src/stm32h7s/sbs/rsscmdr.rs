...
///Field `RSSCMD` reader - RSS command The application can use this bitfield to pass on a command to the RSS, executed at the next reset.
pub type RSSCMD_R = crate::FieldReader<u16>;
///Field `RSSCMD` writer - RSS command The application can use this bitfield to pass on a command to the RSS, executed at the next reset.
