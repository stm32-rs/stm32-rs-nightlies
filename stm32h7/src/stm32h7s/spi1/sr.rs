...
/**Field `RXP` reader - Rx-packet available The flag is changed by hardware. It monitors the total number of data currently available at RxFIFO if SPI is enabled. RXP value depends on the FIFO threshold (FTHLV\[3:0\]), data frame size (DSIZE\[4:0\]
in SPI mode and DATLEN\[1:0\]
in I2S/PCM mode), and actual communication flow. If the data packet is read by performing consecutive read operations from SPI_RXDR, RXP flag must be checked again once a complete data packet is read out from RxFIFO.*/
pub type RXP_R = crate::BitReader;
/**Field `TXP` reader - Tx-packet space available TXP flag can be changed only by hardware. Its value depends on the physical size of the FIFO and its threshold (FTHLV\[3:0\]), data frame size (DSIZE\[4:0\]
in SPI mode and respective DATLEN\[1:0\]
in I2S/PCM mode), and actual communication flow. If the data packet is stored by performing consecutive write operations to SPI_TXDR, TXP flag must be checked again once a complete data packet is stored at TxFIFO. TXP is set despite SPI TxFIFO becomes inaccessible when SPI is reset or disabled.*/
...
/**Field `CTSIZE` reader - number of data frames remaining in current TSIZE session The value is not quite reliable when traffic is ongoing on bus . Note: CTSIZE\[15:0\]
bits are not available in instances with limited set of features.*/
pub type CTSIZE_R = crate::FieldReader<u16>;
impl R {
    /**Bit 0 - Rx-packet available The flag is changed by hardware. It monitors the total number of data currently available at RxFIFO if SPI is enabled. RXP value depends on the FIFO threshold (FTHLV\[3:0\]), data frame size (DSIZE\[4:0\]
    in SPI mode and DATLEN\[1:0\]
    in I2S/PCM mode), and actual communication flow. If the data packet is read by performing consecutive read operations from SPI_RXDR, RXP flag must be checked again once a complete data packet is read out from RxFIFO.*/
    #[inline(always)]
    ...
    /**Bit 1 - Tx-packet space available TXP flag can be changed only by hardware. Its value depends on the physical size of the FIFO and its threshold (FTHLV\[3:0\]), data frame size (DSIZE\[4:0\]
    in SPI mode and respective DATLEN\[1:0\]
    in I2S/PCM mode), and actual communication flow. If the data packet is stored by performing consecutive write operations to SPI_TXDR, TXP flag must be checked again once a complete data packet is stored at TxFIFO. TXP is set despite SPI TxFIFO becomes inaccessible when SPI is reset or disabled.*/
    #[inline(always)]
    ...
    /**Bits 16:31 - number of data frames remaining in current TSIZE session The value is not quite reliable when traffic is ongoing on bus . Note: CTSIZE\[15:0\]
    bits are not available in instances with limited set of features.*/
    #[inline(always)]
