...
/**Field `PSC` reader - Prescaler value The counter clock frequency (tim_cnt_ck) is equal to f&lt;sub>tim_psc_ck&lt;/sub> / (PSC\[15:0\]
...
/**Field `PSC` writer - Prescaler value The counter clock frequency (tim_cnt_ck) is equal to f&lt;sub>tim_psc_ck&lt;/sub> / (PSC\[15:0\]
+ 1). PSC contains the value to be loaded in the active prescaler register at each update event (including when the counter is cleared through UG bit of TIMx_EGR register or through trigger controller when configured in reset mode).*/
pub type PSC_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    /**Bits 0:15 - Prescaler value The counter clock frequency (tim_cnt_ck) is equal to f&lt;sub>tim_psc_ck&lt;/sub> / (PSC\[15:0\]
    + 1). PSC contains the value to be loaded in the active prescaler register at each update event (including when the counter is cleared through UG bit of TIMx_EGR register or through trigger controller when configured in reset mode).*/
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32H7S.html#TIM16:PSC)*/
pub struct PSCrs;
