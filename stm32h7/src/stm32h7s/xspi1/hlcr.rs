...
///Field `LM` reader - Latency mode This bit selects the Latency mode. Note: This bit must be set when using the dual-octal HyperBus configuration.
pub type LM_R = crate::BitReader;
///Field `LM` writer - Latency mode This bit selects the Latency mode. Note: This bit must be set when using the dual-octal HyperBus configuration.
...
///Field `TACC` reader - Access time Device access time expressed in number of communication clock cycles
pub type TACC_R = crate::FieldReader;
///Field `TACC` writer - Access time Device access time expressed in number of communication clock cycles
...
impl W {
    ///Bit 0 - Latency mode This bit selects the Latency mode. Note: This bit must be set when using the dual-octal HyperBus configuration.
    #[inline(always)]
    pub fn lm(&mut self) -> LM_W<'_, HLCRrs> {
        LM_W::new(self, 0)
    }
    ///Bit 1 - Write zero latency This bit enables zero latency on write operations.
    #[inline(always)]
    pub fn wzl(&mut self) -> WZL_W<'_, HLCRrs> {
        WZL_W::new(self, 1)
    }
    ///Bits 8:15 - Access time Device access time expressed in number of communication clock cycles
    #[inline(always)]
    pub fn tacc(&mut self) -> TACC_W<'_, HLCRrs> {
        TACC_W::new(self, 8)
    }
    ///Bits 16:23 - Read write recovery time Device read write recovery time expressed in number of communication clock cycles
    #[inline(always)]
    pub fn trwr(&mut self) -> TRWR_W<'_, HLCRrs> {
        TRWR_W::new(self, 16)
    }
}
