...
///Field `MASK` reader - Status mask Mask to be applied to the status bytes received in automatic status-polling mode For bit n:
pub type MASK_R = crate::FieldReader<u32>;
///Field `MASK` writer - Status mask Mask to be applied to the status bytes received in automatic status-polling mode For bit n:
