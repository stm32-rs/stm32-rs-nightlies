...
///Field `ALTERNATE` reader - Alternate bytes Optional data to be sent to the external SPI device right after the address
pub type ALTERNATE_R = crate::FieldReader<u32>;
///Field `ALTERNATE` writer - Alternate bytes Optional data to be sent to the external SPI device right after the address
...
impl W {
    ///Bits 0:31 - Alternate bytes Optional data to be sent to the external SPI device right after the address
    #[inline(always)]
    pub fn alternate(&mut self) -> ALTERNATE_W<'_, WPABRrs> {
