///Register `SQR2` reader
pub type R = crate::R<SQR2rs>;
///Register `SQR2` writer
pub type W = crate::W<SQR2rs>;
///Field `SQ(19-24)` reader - %s conversion in regular sequence
pub type SQ_R = crate::FieldReader;
///Field `SQ(19-24)` writer - %s conversion in regular sequence
pub type SQ_W<'a, REG> = crate::FieldWriter<'a, REG, 5>;
impl R {
    ///(19-24) conversion in regular sequence
    ///
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `SQ19` field.</div>
    #[inline(always)]
    pub fn sq(&self, n: u8) -> SQ_R {
        #[allow(clippy::no_effect)]
        [(); 6][n as usize];
        SQ_R::new(((self.bits >> (n * 5)) & 0x1f) as u8)
    }
    ///Iterator for array of:
    ///(19-24) conversion in regular sequence
    #[inline(always)]
    pub fn sq_iter(&self) -> impl Iterator<Item = SQ_R> + '_ {
        (0..6).map(move |n| SQ_R::new(((self.bits >> (n * 5)) & 0x1f) as u8))
    }
    ///Bits 0:4 - 19 conversion in regular sequence
    #[inline(always)]
    pub fn sq19(&self) -> SQ_R {
        SQ_R::new((self.bits & 0x1f) as u8)
    }
    ///Bits 5:9 - 20 conversion in regular sequence
    #[inline(always)]
    pub fn sq20(&self) -> SQ_R {
        SQ_R::new(((self.bits >> 5) & 0x1f) as u8)
    }
    ///Bits 10:14 - 21 conversion in regular sequence
    #[inline(always)]
    pub fn sq21(&self) -> SQ_R {
        SQ_R::new(((self.bits >> 10) & 0x1f) as u8)
    }
    ///Bits 15:19 - 22 conversion in regular sequence
    #[inline(always)]
    pub fn sq22(&self) -> SQ_R {
        SQ_R::new(((self.bits >> 15) & 0x1f) as u8)
    }
    ///Bits 20:24 - 23 conversion in regular sequence
    #[inline(always)]
    pub fn sq23(&self) -> SQ_R {
        SQ_R::new(((self.bits >> 20) & 0x1f) as u8)
    }
    ///Bits 25:29 - 24 conversion in regular sequence
    #[inline(always)]
    pub fn sq24(&self) -> SQ_R {
        SQ_R::new(((self.bits >> 25) & 0x1f) as u8)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("SQR2")
            .field("sq19", &self.sq19())
            .field("sq20", &self.sq20())
            .field("sq21", &self.sq21())
            .field("sq22", &self.sq22())
            .field("sq23", &self.sq23())
            .field("sq24", &self.sq24())
            .finish()
    }
}
impl W {
    ///(19-24) conversion in regular sequence
    ///
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `SQ19` field.</div>
    #[inline(always)]
    pub fn sq(&mut self, n: u8) -> SQ_W<SQR2rs> {
        #[allow(clippy::no_effect)]
        [(); 6][n as usize];
        SQ_W::new(self, n * 5)
    }
    ///Bits 0:4 - 19 conversion in regular sequence
    #[inline(always)]
    pub fn sq19(&mut self) -> SQ_W<SQR2rs> {
        SQ_W::new(self, 0)
    }
    ///Bits 5:9 - 20 conversion in regular sequence
    #[inline(always)]
    pub fn sq20(&mut self) -> SQ_W<SQR2rs> {
        SQ_W::new(self, 5)
    }
    ///Bits 10:14 - 21 conversion in regular sequence
    #[inline(always)]
    pub fn sq21(&mut self) -> SQ_W<SQR2rs> {
        SQ_W::new(self, 10)
    }
    ///Bits 15:19 - 22 conversion in regular sequence
    #[inline(always)]
    pub fn sq22(&mut self) -> SQ_W<SQR2rs> {
        SQ_W::new(self, 15)
    }
    ///Bits 20:24 - 23 conversion in regular sequence
    #[inline(always)]
    pub fn sq23(&mut self) -> SQ_W<SQR2rs> {
        SQ_W::new(self, 20)
    }
    ///Bits 25:29 - 24 conversion in regular sequence
    #[inline(always)]
    pub fn sq24(&mut self) -> SQ_W<SQR2rs> {
        SQ_W::new(self, 25)
    }
}
/**regular sequence register 2

You can [`read`](crate::Reg::read) this register and get [`sqr2::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`sqr2::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32L162.html#ADC:SQR2)*/
pub struct SQR2rs;
impl crate::RegisterSpec for SQR2rs {
    type Ux = u32;
}
///`read()` method returns [`sqr2::R`](R) reader structure
impl crate::Readable for SQR2rs {}
///`write(|w| ..)` method takes [`sqr2::W`](W) writer structure
impl crate::Writable for SQR2rs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets SQR2 to value 0
impl crate::Resettable for SQR2rs {
    const RESET_VALUE: u32 = 0;
}
