...
impl From<UCESM> for bool {
    #[inline(always)]
    fn from(variant: UCESM) -> Self {
        variant as u8 != 0
    }
}
///Field `UCESM` reader - USART Clock Enable in Stop mode
pub type UCESM_R = crate::BitReader<UCESM>;
impl UCESM_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> UCESM {
        match self.bits {
            false => UCESM::Disabled,
            true => UCESM::Enabled,
        }
    }
    ///USART clock is disabled in STOP mode
    #[inline(always)]
    pub fn is_disabled(&self) -> bool {
        *self == UCESM::Disabled
    }
    ///USART clock is enabled in STOP mode
    #[inline(always)]
    pub fn is_enabled(&self) -> bool {
        *self == UCESM::Enabled
    }
}
///Field `UCESM` writer - USART Clock Enable in Stop mode
pub type UCESM_W<'a, REG> = crate::BitWriter<'a, REG, UCESM>;
impl<'a, REG> UCESM_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///USART clock is disabled in STOP mode
    #[inline(always)]
    pub fn disabled(self) -> &'a mut crate::W<REG> {
        self.variant(UCESM::Disabled)
    }
    ///USART clock is enabled in STOP mode
    #[inline(always)]
    pub fn enabled(self) -> &'a mut crate::W<REG> {
        self.variant(UCESM::Enabled)
    }
}
...
pub enum TCBGTIE {
    ///0: Interrupt is inhibited
    Disabled = 0,
    ///1: An USART interrupt is generated whenever TCBGT=1 in the ISR register
    Enabled = 1,
}
...
impl TCBGTIE_R {
    ...
    ///An USART interrupt is generated whenever TCBGT=1 in the ISR register
    #[inline(always)]
    pub fn is_enabled(&self) -> bool {
...
impl W {
    ...
    ///Bit 23 - USART Clock Enable in Stop mode
    #[inline(always)]
    pub fn ucesm(&mut self) -> UCESM_W<'_, CR3rs> {
        UCESM_W::new(self, 23)
    }
    ///Bit 24 - Transmission complete before guard time interrupt enable
    #[inline(always)]
