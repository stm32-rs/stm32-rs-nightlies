...
///Field `SSCLR` reader - Clear synchronous counter on alarm
pub type SSCLR_R = crate::BitReader;
///Field `SSCLR` writer - Clear synchronous counter on alarm
...
impl W {
    ///Bits 0:14 - Sub seconds value
    #[inline(always)]
    pub fn ss(&mut self) -> SS_W<'_, ALRMBSSRrs> {
        SS_W::new(self, 0)
    }
    ///Bits 24:29 - Mask the most-significant bits starting at this bit
    #[inline(always)]
    pub fn maskss(&mut self) -> MASKSS_W<'_, ALRMBSSRrs> {
        MASKSS_W::new(self, 24)
    }
    ///Bit 31 - Clear synchronous counter on alarm
    #[inline(always)]
    pub fn ssclr(&mut self) -> SSCLR_W<'_, ALRMBSSRrs> {
        SSCLR_W::new(self, 31)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32L4P5.html#RTC:ALRMBSSR)*/
