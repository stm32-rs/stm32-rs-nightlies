...
impl From<UIFREMA> for bool {
    #[inline(always)]
    fn from(variant: UIFREMA) -> Self {
        variant as u8 != 0
    }
}
///Field `UIFREMA` reader - UIF status bit remapping
pub type UIFREMA_R = crate::BitReader<UIFREMA>;
impl UIFREMA_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> UIFREMA {
        match self.bits {
            false => UIFREMA::Disabled,
            true => UIFREMA::Enabled,
        }
    }
    ///No remapping. UIF status bit is not copied to TIMx_CNT register bit 31
    #[inline(always)]
    pub fn is_disabled(&self) -> bool {
        *self == UIFREMA::Disabled
    }
    ///Remapping enabled. UIF status bit is copied to TIMx_CNT register bit 31
    #[inline(always)]
    pub fn is_enabled(&self) -> bool {
        *self == UIFREMA::Enabled
    }
}
///Field `UIFREMA` writer - UIF status bit remapping
pub type UIFREMA_W<'a, REG> = crate::BitWriter<'a, REG, UIFREMA>;
impl<'a, REG> UIFREMA_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///No remapping. UIF status bit is not copied to TIMx_CNT register bit 31
    #[inline(always)]
    pub fn disabled(self) -> &'a mut crate::W<REG> {
        self.variant(UIFREMA::Disabled)
    }
    ///Remapping enabled. UIF status bit is copied to TIMx_CNT register bit 31
    #[inline(always)]
    pub fn enabled(self) -> &'a mut crate::W<REG> {
        self.variant(UIFREMA::Enabled)
    }
}
impl R {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32L4P5.html#TIM2:CR1)*/
pub struct CR1rs;
