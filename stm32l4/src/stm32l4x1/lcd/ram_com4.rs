...
///Field `SEGS` reader - Segment states, one bit per segment, LSB: S00, MSB: S43
pub type SEGS_R = crate::FieldReader<u64>;
///Field `SEGS` writer - Segment states, one bit per segment, LSB: S00, MSB: S43
pub type SEGS_W<'a, REG> = crate::FieldWriter<'a, REG, 44, u64>;
impl R {
    ///Bits 0:43 - Segment states, one bit per segment, LSB: S00, MSB: S43
    #[inline(always)]
    pub fn segs(&self) -> SEGS_R {
        SEGS_R::new(self.bits & 0x0fff_ffff_ffff)
    }
}
...
impl W {
    ///Bits 0:43 - Segment states, one bit per segment, LSB: S00, MSB: S43
    #[inline(always)]
    pub fn segs(&mut self) -> SEGS_W<RAM_COM4rs> {
        SEGS_W::new(self, 0)
    }
}
/**LCD display memory
