#[doc = "Register `VCTR3` reader"]
pub type R = crate::R<VCTR3rs>;
#[doc = "Register `VCTR3` writer"]
pub type W = crate::W<VCTR3rs>;
#[doc = "Field `B96` reader - B96"]
pub type B96_R = crate::BitReader;
#[doc = "Field `B96` writer - B96"]
pub type B96_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B97` reader - B97"]
pub type B97_R = crate::BitReader;
#[doc = "Field `B97` writer - B97"]
pub type B97_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B98` reader - B98"]
pub type B98_R = crate::BitReader;
#[doc = "Field `B98` writer - B98"]
pub type B98_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B99` reader - B99"]
pub type B99_R = crate::BitReader;
#[doc = "Field `B99` writer - B99"]
pub type B99_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B100` reader - B100"]
pub type B100_R = crate::BitReader;
#[doc = "Field `B100` writer - B100"]
pub type B100_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B101` reader - B101"]
pub type B101_R = crate::BitReader;
#[doc = "Field `B101` writer - B101"]
pub type B101_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B102` reader - B102"]
pub type B102_R = crate::BitReader;
#[doc = "Field `B102` writer - B102"]
pub type B102_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B103` reader - B103"]
pub type B103_R = crate::BitReader;
#[doc = "Field `B103` writer - B103"]
pub type B103_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B104` reader - B104"]
pub type B104_R = crate::BitReader;
#[doc = "Field `B104` writer - B104"]
pub type B104_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B105` reader - B105"]
pub type B105_R = crate::BitReader;
#[doc = "Field `B105` writer - B105"]
pub type B105_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B106` reader - B106"]
pub type B106_R = crate::BitReader;
#[doc = "Field `B106` writer - B106"]
pub type B106_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B107` reader - B107"]
pub type B107_R = crate::BitReader;
#[doc = "Field `B107` writer - B107"]
pub type B107_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B108` reader - B108"]
pub type B108_R = crate::BitReader;
#[doc = "Field `B108` writer - B108"]
pub type B108_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B109` reader - B109"]
pub type B109_R = crate::BitReader;
#[doc = "Field `B109` writer - B109"]
pub type B109_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B110` reader - B110"]
pub type B110_R = crate::BitReader;
#[doc = "Field `B110` writer - B110"]
pub type B110_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B111` reader - B111"]
pub type B111_R = crate::BitReader;
#[doc = "Field `B111` writer - B111"]
pub type B111_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B112` reader - B112"]
pub type B112_R = crate::BitReader;
#[doc = "Field `B112` writer - B112"]
pub type B112_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B113` reader - B113"]
pub type B113_R = crate::BitReader;
#[doc = "Field `B113` writer - B113"]
pub type B113_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B114` reader - B114"]
pub type B114_R = crate::BitReader;
#[doc = "Field `B114` writer - B114"]
pub type B114_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B115` reader - B115"]
pub type B115_R = crate::BitReader;
#[doc = "Field `B115` writer - B115"]
pub type B115_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B116` reader - B116"]
pub type B116_R = crate::BitReader;
#[doc = "Field `B116` writer - B116"]
pub type B116_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B117` reader - B117"]
pub type B117_R = crate::BitReader;
#[doc = "Field `B117` writer - B117"]
pub type B117_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B118` reader - B118"]
pub type B118_R = crate::BitReader;
#[doc = "Field `B118` writer - B118"]
pub type B118_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B119` reader - B119"]
pub type B119_R = crate::BitReader;
#[doc = "Field `B119` writer - B119"]
pub type B119_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B120` reader - B120"]
pub type B120_R = crate::BitReader;
#[doc = "Field `B120` writer - B120"]
pub type B120_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B121` reader - B121"]
pub type B121_R = crate::BitReader;
#[doc = "Field `B121` writer - B121"]
pub type B121_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B122` reader - B122"]
pub type B122_R = crate::BitReader;
#[doc = "Field `B122` writer - B122"]
pub type B122_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B123` reader - B123"]
pub type B123_R = crate::BitReader;
#[doc = "Field `B123` writer - B123"]
pub type B123_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B124` reader - B124"]
pub type B124_R = crate::BitReader;
#[doc = "Field `B124` writer - B124"]
pub type B124_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B125` reader - B125"]
pub type B125_R = crate::BitReader;
#[doc = "Field `B125` writer - B125"]
pub type B125_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B126` reader - B126"]
pub type B126_R = crate::BitReader;
#[doc = "Field `B126` writer - B126"]
pub type B126_W<'a, REG> = crate::BitWriter<'a, REG>;
#[doc = "Field `B127` reader - B127"]
pub type B127_R = crate::BitReader;
#[doc = "Field `B127` writer - B127"]
pub type B127_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    #[doc = "Bit 0 - B96"]
    #[inline(always)]
    pub fn b96(&self) -> B96_R {
        B96_R::new((self.bits & 1) != 0)
    }
    #[doc = "Bit 1 - B97"]
    #[inline(always)]
    pub fn b97(&self) -> B97_R {
        B97_R::new(((self.bits >> 1) & 1) != 0)
    }
    #[doc = "Bit 2 - B98"]
    #[inline(always)]
    pub fn b98(&self) -> B98_R {
        B98_R::new(((self.bits >> 2) & 1) != 0)
    }
    #[doc = "Bit 3 - B99"]
    #[inline(always)]
    pub fn b99(&self) -> B99_R {
        B99_R::new(((self.bits >> 3) & 1) != 0)
    }
    #[doc = "Bit 4 - B100"]
    #[inline(always)]
    pub fn b100(&self) -> B100_R {
        B100_R::new(((self.bits >> 4) & 1) != 0)
    }
    #[doc = "Bit 5 - B101"]
    #[inline(always)]
    pub fn b101(&self) -> B101_R {
        B101_R::new(((self.bits >> 5) & 1) != 0)
    }
    #[doc = "Bit 6 - B102"]
    #[inline(always)]
    pub fn b102(&self) -> B102_R {
        B102_R::new(((self.bits >> 6) & 1) != 0)
    }
    #[doc = "Bit 7 - B103"]
    #[inline(always)]
    pub fn b103(&self) -> B103_R {
        B103_R::new(((self.bits >> 7) & 1) != 0)
    }
    #[doc = "Bit 8 - B104"]
    #[inline(always)]
    pub fn b104(&self) -> B104_R {
        B104_R::new(((self.bits >> 8) & 1) != 0)
    }
    #[doc = "Bit 9 - B105"]
    #[inline(always)]
    pub fn b105(&self) -> B105_R {
        B105_R::new(((self.bits >> 9) & 1) != 0)
    }
    #[doc = "Bit 10 - B106"]
    #[inline(always)]
    pub fn b106(&self) -> B106_R {
        B106_R::new(((self.bits >> 10) & 1) != 0)
    }
    #[doc = "Bit 11 - B107"]
    #[inline(always)]
    pub fn b107(&self) -> B107_R {
        B107_R::new(((self.bits >> 11) & 1) != 0)
    }
    #[doc = "Bit 12 - B108"]
    #[inline(always)]
    pub fn b108(&self) -> B108_R {
        B108_R::new(((self.bits >> 12) & 1) != 0)
    }
    #[doc = "Bit 13 - B109"]
    #[inline(always)]
    pub fn b109(&self) -> B109_R {
        B109_R::new(((self.bits >> 13) & 1) != 0)
    }
    #[doc = "Bit 14 - B110"]
    #[inline(always)]
    pub fn b110(&self) -> B110_R {
        B110_R::new(((self.bits >> 14) & 1) != 0)
    }
    #[doc = "Bit 15 - B111"]
    #[inline(always)]
    pub fn b111(&self) -> B111_R {
        B111_R::new(((self.bits >> 15) & 1) != 0)
    }
    #[doc = "Bit 16 - B112"]
    #[inline(always)]
    pub fn b112(&self) -> B112_R {
        B112_R::new(((self.bits >> 16) & 1) != 0)
    }
    #[doc = "Bit 17 - B113"]
    #[inline(always)]
    pub fn b113(&self) -> B113_R {
        B113_R::new(((self.bits >> 17) & 1) != 0)
    }
    #[doc = "Bit 18 - B114"]
    #[inline(always)]
    pub fn b114(&self) -> B114_R {
        B114_R::new(((self.bits >> 18) & 1) != 0)
    }
    #[doc = "Bit 19 - B115"]
    #[inline(always)]
    pub fn b115(&self) -> B115_R {
        B115_R::new(((self.bits >> 19) & 1) != 0)
    }
    #[doc = "Bit 20 - B116"]
    #[inline(always)]
    pub fn b116(&self) -> B116_R {
        B116_R::new(((self.bits >> 20) & 1) != 0)
    }
    #[doc = "Bit 21 - B117"]
    #[inline(always)]
    pub fn b117(&self) -> B117_R {
        B117_R::new(((self.bits >> 21) & 1) != 0)
    }
    #[doc = "Bit 22 - B118"]
    #[inline(always)]
    pub fn b118(&self) -> B118_R {
        B118_R::new(((self.bits >> 22) & 1) != 0)
    }
    #[doc = "Bit 23 - B119"]
    #[inline(always)]
    pub fn b119(&self) -> B119_R {
        B119_R::new(((self.bits >> 23) & 1) != 0)
    }
    #[doc = "Bit 24 - B120"]
    #[inline(always)]
    pub fn b120(&self) -> B120_R {
        B120_R::new(((self.bits >> 24) & 1) != 0)
    }
    #[doc = "Bit 25 - B121"]
    #[inline(always)]
    pub fn b121(&self) -> B121_R {
        B121_R::new(((self.bits >> 25) & 1) != 0)
    }
    #[doc = "Bit 26 - B122"]
    #[inline(always)]
    pub fn b122(&self) -> B122_R {
        B122_R::new(((self.bits >> 26) & 1) != 0)
    }
    #[doc = "Bit 27 - B123"]
    #[inline(always)]
    pub fn b123(&self) -> B123_R {
        B123_R::new(((self.bits >> 27) & 1) != 0)
    }
    #[doc = "Bit 28 - B124"]
    #[inline(always)]
    pub fn b124(&self) -> B124_R {
        B124_R::new(((self.bits >> 28) & 1) != 0)
    }
    #[doc = "Bit 29 - B125"]
    #[inline(always)]
    pub fn b125(&self) -> B125_R {
        B125_R::new(((self.bits >> 29) & 1) != 0)
    }
    #[doc = "Bit 30 - B126"]
    #[inline(always)]
    pub fn b126(&self) -> B126_R {
        B126_R::new(((self.bits >> 30) & 1) != 0)
    }
    #[doc = "Bit 31 - B127"]
    #[inline(always)]
    pub fn b127(&self) -> B127_R {
        B127_R::new(((self.bits >> 31) & 1) != 0)
    }
}
impl W {
    #[doc = "Bit 0 - B96"]
    #[inline(always)]
    #[must_use]
    pub fn b96(&mut self) -> B96_W<VCTR3rs> {
        B96_W::new(self, 0)
    }
    #[doc = "Bit 1 - B97"]
    #[inline(always)]
    #[must_use]
    pub fn b97(&mut self) -> B97_W<VCTR3rs> {
        B97_W::new(self, 1)
    }
    #[doc = "Bit 2 - B98"]
    #[inline(always)]
    #[must_use]
    pub fn b98(&mut self) -> B98_W<VCTR3rs> {
        B98_W::new(self, 2)
    }
    #[doc = "Bit 3 - B99"]
    #[inline(always)]
    #[must_use]
    pub fn b99(&mut self) -> B99_W<VCTR3rs> {
        B99_W::new(self, 3)
    }
    #[doc = "Bit 4 - B100"]
    #[inline(always)]
    #[must_use]
    pub fn b100(&mut self) -> B100_W<VCTR3rs> {
        B100_W::new(self, 4)
    }
    #[doc = "Bit 5 - B101"]
    #[inline(always)]
    #[must_use]
    pub fn b101(&mut self) -> B101_W<VCTR3rs> {
        B101_W::new(self, 5)
    }
    #[doc = "Bit 6 - B102"]
    #[inline(always)]
    #[must_use]
    pub fn b102(&mut self) -> B102_W<VCTR3rs> {
        B102_W::new(self, 6)
    }
    #[doc = "Bit 7 - B103"]
    #[inline(always)]
    #[must_use]
    pub fn b103(&mut self) -> B103_W<VCTR3rs> {
        B103_W::new(self, 7)
    }
    #[doc = "Bit 8 - B104"]
    #[inline(always)]
    #[must_use]
    pub fn b104(&mut self) -> B104_W<VCTR3rs> {
        B104_W::new(self, 8)
    }
    #[doc = "Bit 9 - B105"]
    #[inline(always)]
    #[must_use]
    pub fn b105(&mut self) -> B105_W<VCTR3rs> {
        B105_W::new(self, 9)
    }
    #[doc = "Bit 10 - B106"]
    #[inline(always)]
    #[must_use]
    pub fn b106(&mut self) -> B106_W<VCTR3rs> {
        B106_W::new(self, 10)
    }
    #[doc = "Bit 11 - B107"]
    #[inline(always)]
    #[must_use]
    pub fn b107(&mut self) -> B107_W<VCTR3rs> {
        B107_W::new(self, 11)
    }
    #[doc = "Bit 12 - B108"]
    #[inline(always)]
    #[must_use]
    pub fn b108(&mut self) -> B108_W<VCTR3rs> {
        B108_W::new(self, 12)
    }
    #[doc = "Bit 13 - B109"]
    #[inline(always)]
    #[must_use]
    pub fn b109(&mut self) -> B109_W<VCTR3rs> {
        B109_W::new(self, 13)
    }
    #[doc = "Bit 14 - B110"]
    #[inline(always)]
    #[must_use]
    pub fn b110(&mut self) -> B110_W<VCTR3rs> {
        B110_W::new(self, 14)
    }
    #[doc = "Bit 15 - B111"]
    #[inline(always)]
    #[must_use]
    pub fn b111(&mut self) -> B111_W<VCTR3rs> {
        B111_W::new(self, 15)
    }
    #[doc = "Bit 16 - B112"]
    #[inline(always)]
    #[must_use]
    pub fn b112(&mut self) -> B112_W<VCTR3rs> {
        B112_W::new(self, 16)
    }
    #[doc = "Bit 17 - B113"]
    #[inline(always)]
    #[must_use]
    pub fn b113(&mut self) -> B113_W<VCTR3rs> {
        B113_W::new(self, 17)
    }
    #[doc = "Bit 18 - B114"]
    #[inline(always)]
    #[must_use]
    pub fn b114(&mut self) -> B114_W<VCTR3rs> {
        B114_W::new(self, 18)
    }
    #[doc = "Bit 19 - B115"]
    #[inline(always)]
    #[must_use]
    pub fn b115(&mut self) -> B115_W<VCTR3rs> {
        B115_W::new(self, 19)
    }
    #[doc = "Bit 20 - B116"]
    #[inline(always)]
    #[must_use]
    pub fn b116(&mut self) -> B116_W<VCTR3rs> {
        B116_W::new(self, 20)
    }
    #[doc = "Bit 21 - B117"]
    #[inline(always)]
    #[must_use]
    pub fn b117(&mut self) -> B117_W<VCTR3rs> {
        B117_W::new(self, 21)
    }
    #[doc = "Bit 22 - B118"]
    #[inline(always)]
    #[must_use]
    pub fn b118(&mut self) -> B118_W<VCTR3rs> {
        B118_W::new(self, 22)
    }
    #[doc = "Bit 23 - B119"]
    #[inline(always)]
    #[must_use]
    pub fn b119(&mut self) -> B119_W<VCTR3rs> {
        B119_W::new(self, 23)
    }
    #[doc = "Bit 24 - B120"]
    #[inline(always)]
    #[must_use]
    pub fn b120(&mut self) -> B120_W<VCTR3rs> {
        B120_W::new(self, 24)
    }
    #[doc = "Bit 25 - B121"]
    #[inline(always)]
    #[must_use]
    pub fn b121(&mut self) -> B121_W<VCTR3rs> {
        B121_W::new(self, 25)
    }
    #[doc = "Bit 26 - B122"]
    #[inline(always)]
    #[must_use]
    pub fn b122(&mut self) -> B122_W<VCTR3rs> {
        B122_W::new(self, 26)
    }
    #[doc = "Bit 27 - B123"]
    #[inline(always)]
    #[must_use]
    pub fn b123(&mut self) -> B123_W<VCTR3rs> {
        B123_W::new(self, 27)
    }
    #[doc = "Bit 28 - B124"]
    #[inline(always)]
    #[must_use]
    pub fn b124(&mut self) -> B124_W<VCTR3rs> {
        B124_W::new(self, 28)
    }
    #[doc = "Bit 29 - B125"]
    #[inline(always)]
    #[must_use]
    pub fn b125(&mut self) -> B125_W<VCTR3rs> {
        B125_W::new(self, 29)
    }
    #[doc = "Bit 30 - B126"]
    #[inline(always)]
    #[must_use]
    pub fn b126(&mut self) -> B126_W<VCTR3rs> {
        B126_W::new(self, 30)
    }
    #[doc = "Bit 31 - B127"]
    #[inline(always)]
    #[must_use]
    pub fn b127(&mut self) -> B127_W<VCTR3rs> {
        B127_W::new(self, 31)
    }
}
#[doc = "MPCBBx vector register\n\nYou can [`read`](crate::generic::Reg::read) this register and get [`vctr3::R`](R).  You can [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`write_with_zero`](crate::generic::Reg::write_with_zero) this register using [`vctr3::W`](W). You can also [`modify`](crate::generic::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct VCTR3rs;
impl crate::RegisterSpec for VCTR3rs {
    type Ux = u32;
}
#[doc = "`read()` method returns [`vctr3::R`](R) reader structure"]
impl crate::Readable for VCTR3rs {}
#[doc = "`write(|w| ..)` method takes [`vctr3::W`](W) writer structure"]
impl crate::Writable for VCTR3rs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
#[doc = "`reset()` method sets VCTR3 to value 0xffff_ffff"]
impl crate::Resettable for VCTR3rs {
    const RESET_VALUE: u32 = 0xffff_ffff;
}
