...
///Field `SWI21` reader - Software interrupt on event x
pub type SWI21_R = crate::BitReader;
///Field `SWI21` writer - Software interrupt on event x
pub type SWI21_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `SWI22` reader - Software interrupt on event x
pub type SWI22_R = crate::BitReader;
///Field `SWI22` writer - Software interrupt on event x
...
impl W {
    ...
    ///Bit 22 - Software interrupt on event x
    #[inline(always)]
    pub fn swi22(&mut self) -> SWI22_W<'_, SWIER1rs> {
        SWI22_W::new(self, 22)
    }
}
