///Register `ETH_MTLRxQ1CR` reader
pub type R = crate::R<ETH_MTLRX_Q1CRrs>;
///Register `ETH_MTLRxQ1CR` writer
pub type W = crate::W<ETH_MTLRX_Q1CRrs>;
///Field `RXQ_WEGT` reader - RXQ_WEGT
pub type RXQ_WEGT_R = crate::FieldReader;
///Field `RXQ_FRM_ARBIT` reader - RXQ_FRM_ARBIT
pub type RXQ_FRM_ARBIT_R = crate::BitReader;
impl R {
    ///Bits 0:2 - RXQ_WEGT
    #[inline(always)]
    pub fn rxq_wegt(&self) -> RXQ_WEGT_R {
        RXQ_WEGT_R::new((self.bits & 7) as u8)
    }
    ///Bit 3 - RXQ_FRM_ARBIT
    #[inline(always)]
    pub fn rxq_frm_arbit(&self) -> RXQ_FRM_ARBIT_R {
        RXQ_FRM_ARBIT_R::new(((self.bits >> 3) & 1) != 0)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("ETH_MTLRxQ1CR")
            .field("rxq_wegt", &self.rxq_wegt())
            .field("rxq_frm_arbit", &self.rxq_frm_arbit())
            .finish()
    }
}
impl W {}
/**Rx queue 1 control register

You can [`read`](crate::Reg::read) this register and get [`eth_mtlrx_q1cr::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`eth_mtlrx_q1cr::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32MP153.html#ETH_MTL:ETH_MTLRxQ1CR)*/
pub struct ETH_MTLRX_Q1CRrs;
impl crate::RegisterSpec for ETH_MTLRX_Q1CRrs {
    type Ux = u32;
}
///`read()` method returns [`eth_mtlrx_q1cr::R`](R) reader structure
impl crate::Readable for ETH_MTLRX_Q1CRrs {}
///`write(|w| ..)` method takes [`eth_mtlrx_q1cr::W`](W) writer structure
impl crate::Writable for ETH_MTLRX_Q1CRrs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets ETH_MTLRxQ1CR to value 0
impl crate::Resettable for ETH_MTLRX_Q1CRrs {
    const RESET_VALUE: u32 = 0;
}
