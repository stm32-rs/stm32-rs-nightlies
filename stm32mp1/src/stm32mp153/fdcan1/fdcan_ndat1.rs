///Register `FDCAN_NDAT1` reader
pub type R = crate::R<FDCAN_NDAT1rs>;
///Register `FDCAN_NDAT1` writer
pub type W = crate::W<FDCAN_NDAT1rs>;
///Field `ND0` reader - ND0
pub type ND0_R = crate::BitReader;
///Field `ND0` writer - ND0
pub type ND0_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND1` reader - ND1
pub type ND1_R = crate::BitReader;
///Field `ND1` writer - ND1
pub type ND1_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND2` reader - ND2
pub type ND2_R = crate::BitReader;
///Field `ND2` writer - ND2
pub type ND2_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND3` reader - ND3
pub type ND3_R = crate::BitReader;
///Field `ND3` writer - ND3
pub type ND3_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND4` reader - ND4
pub type ND4_R = crate::BitReader;
///Field `ND4` writer - ND4
pub type ND4_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND5` reader - ND5
pub type ND5_R = crate::BitReader;
///Field `ND5` writer - ND5
pub type ND5_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND6` reader - ND6
pub type ND6_R = crate::BitReader;
///Field `ND6` writer - ND6
pub type ND6_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND7` reader - ND7
pub type ND7_R = crate::BitReader;
///Field `ND7` writer - ND7
pub type ND7_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND8` reader - ND8
pub type ND8_R = crate::BitReader;
///Field `ND8` writer - ND8
pub type ND8_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND9` reader - ND9
pub type ND9_R = crate::BitReader;
///Field `ND9` writer - ND9
pub type ND9_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND10` reader - ND10
pub type ND10_R = crate::BitReader;
///Field `ND10` writer - ND10
pub type ND10_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND11` reader - ND11
pub type ND11_R = crate::BitReader;
///Field `ND11` writer - ND11
pub type ND11_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND12` reader - ND12
pub type ND12_R = crate::BitReader;
///Field `ND12` writer - ND12
pub type ND12_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND13` reader - ND13
pub type ND13_R = crate::BitReader;
///Field `ND13` writer - ND13
pub type ND13_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND14` reader - ND14
pub type ND14_R = crate::BitReader;
///Field `ND14` writer - ND14
pub type ND14_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND15` reader - ND15
pub type ND15_R = crate::BitReader;
///Field `ND15` writer - ND15
pub type ND15_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND16` reader - ND16
pub type ND16_R = crate::BitReader;
///Field `ND16` writer - ND16
pub type ND16_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND17` reader - ND17
pub type ND17_R = crate::BitReader;
///Field `ND17` writer - ND17
pub type ND17_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND18` reader - ND18
pub type ND18_R = crate::BitReader;
///Field `ND18` writer - ND18
pub type ND18_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND19` reader - ND19
pub type ND19_R = crate::BitReader;
///Field `ND19` writer - ND19
pub type ND19_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND20` reader - ND20
pub type ND20_R = crate::BitReader;
///Field `ND20` writer - ND20
pub type ND20_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND21` reader - ND21
pub type ND21_R = crate::BitReader;
///Field `ND21` writer - ND21
pub type ND21_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND22` reader - ND22
pub type ND22_R = crate::BitReader;
///Field `ND22` writer - ND22
pub type ND22_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND23` reader - ND23
pub type ND23_R = crate::BitReader;
///Field `ND23` writer - ND23
pub type ND23_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND24` reader - ND24
pub type ND24_R = crate::BitReader;
///Field `ND24` writer - ND24
pub type ND24_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND25` reader - ND25
pub type ND25_R = crate::BitReader;
///Field `ND25` writer - ND25
pub type ND25_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND26` reader - ND26
pub type ND26_R = crate::BitReader;
///Field `ND26` writer - ND26
pub type ND26_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND27` reader - ND27
pub type ND27_R = crate::BitReader;
///Field `ND27` writer - ND27
pub type ND27_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND28` reader - ND28
pub type ND28_R = crate::BitReader;
///Field `ND28` writer - ND28
pub type ND28_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND29` reader - ND29
pub type ND29_R = crate::BitReader;
///Field `ND29` writer - ND29
pub type ND29_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND30` reader - ND30
pub type ND30_R = crate::BitReader;
///Field `ND30` writer - ND30
pub type ND30_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ND31` reader - ND31
pub type ND31_R = crate::BitReader;
///Field `ND31` writer - ND31
pub type ND31_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0 - ND0
    #[inline(always)]
    pub fn nd0(&self) -> ND0_R {
        ND0_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - ND1
    #[inline(always)]
    pub fn nd1(&self) -> ND1_R {
        ND1_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - ND2
    #[inline(always)]
    pub fn nd2(&self) -> ND2_R {
        ND2_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - ND3
    #[inline(always)]
    pub fn nd3(&self) -> ND3_R {
        ND3_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 4 - ND4
    #[inline(always)]
    pub fn nd4(&self) -> ND4_R {
        ND4_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - ND5
    #[inline(always)]
    pub fn nd5(&self) -> ND5_R {
        ND5_R::new(((self.bits >> 5) & 1) != 0)
    }
    ///Bit 6 - ND6
    #[inline(always)]
    pub fn nd6(&self) -> ND6_R {
        ND6_R::new(((self.bits >> 6) & 1) != 0)
    }
    ///Bit 7 - ND7
    #[inline(always)]
    pub fn nd7(&self) -> ND7_R {
        ND7_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8 - ND8
    #[inline(always)]
    pub fn nd8(&self) -> ND8_R {
        ND8_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9 - ND9
    #[inline(always)]
    pub fn nd9(&self) -> ND9_R {
        ND9_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10 - ND10
    #[inline(always)]
    pub fn nd10(&self) -> ND10_R {
        ND10_R::new(((self.bits >> 10) & 1) != 0)
    }
    ///Bit 11 - ND11
    #[inline(always)]
    pub fn nd11(&self) -> ND11_R {
        ND11_R::new(((self.bits >> 11) & 1) != 0)
    }
    ///Bit 12 - ND12
    #[inline(always)]
    pub fn nd12(&self) -> ND12_R {
        ND12_R::new(((self.bits >> 12) & 1) != 0)
    }
    ///Bit 13 - ND13
    #[inline(always)]
    pub fn nd13(&self) -> ND13_R {
        ND13_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14 - ND14
    #[inline(always)]
    pub fn nd14(&self) -> ND14_R {
        ND14_R::new(((self.bits >> 14) & 1) != 0)
    }
    ///Bit 15 - ND15
    #[inline(always)]
    pub fn nd15(&self) -> ND15_R {
        ND15_R::new(((self.bits >> 15) & 1) != 0)
    }
    ///Bit 16 - ND16
    #[inline(always)]
    pub fn nd16(&self) -> ND16_R {
        ND16_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17 - ND17
    #[inline(always)]
    pub fn nd17(&self) -> ND17_R {
        ND17_R::new(((self.bits >> 17) & 1) != 0)
    }
    ///Bit 18 - ND18
    #[inline(always)]
    pub fn nd18(&self) -> ND18_R {
        ND18_R::new(((self.bits >> 18) & 1) != 0)
    }
    ///Bit 19 - ND19
    #[inline(always)]
    pub fn nd19(&self) -> ND19_R {
        ND19_R::new(((self.bits >> 19) & 1) != 0)
    }
    ///Bit 20 - ND20
    #[inline(always)]
    pub fn nd20(&self) -> ND20_R {
        ND20_R::new(((self.bits >> 20) & 1) != 0)
    }
    ///Bit 21 - ND21
    #[inline(always)]
    pub fn nd21(&self) -> ND21_R {
        ND21_R::new(((self.bits >> 21) & 1) != 0)
    }
    ///Bit 22 - ND22
    #[inline(always)]
    pub fn nd22(&self) -> ND22_R {
        ND22_R::new(((self.bits >> 22) & 1) != 0)
    }
    ///Bit 23 - ND23
    #[inline(always)]
    pub fn nd23(&self) -> ND23_R {
        ND23_R::new(((self.bits >> 23) & 1) != 0)
    }
    ///Bit 24 - ND24
    #[inline(always)]
    pub fn nd24(&self) -> ND24_R {
        ND24_R::new(((self.bits >> 24) & 1) != 0)
    }
    ///Bit 25 - ND25
    #[inline(always)]
    pub fn nd25(&self) -> ND25_R {
        ND25_R::new(((self.bits >> 25) & 1) != 0)
    }
    ///Bit 26 - ND26
    #[inline(always)]
    pub fn nd26(&self) -> ND26_R {
        ND26_R::new(((self.bits >> 26) & 1) != 0)
    }
    ///Bit 27 - ND27
    #[inline(always)]
    pub fn nd27(&self) -> ND27_R {
        ND27_R::new(((self.bits >> 27) & 1) != 0)
    }
    ///Bit 28 - ND28
    #[inline(always)]
    pub fn nd28(&self) -> ND28_R {
        ND28_R::new(((self.bits >> 28) & 1) != 0)
    }
    ///Bit 29 - ND29
    #[inline(always)]
    pub fn nd29(&self) -> ND29_R {
        ND29_R::new(((self.bits >> 29) & 1) != 0)
    }
    ///Bit 30 - ND30
    #[inline(always)]
    pub fn nd30(&self) -> ND30_R {
        ND30_R::new(((self.bits >> 30) & 1) != 0)
    }
    ///Bit 31 - ND31
    #[inline(always)]
    pub fn nd31(&self) -> ND31_R {
        ND31_R::new(((self.bits >> 31) & 1) != 0)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("FDCAN_NDAT1")
            .field("nd0", &self.nd0())
            .field("nd1", &self.nd1())
            .field("nd2", &self.nd2())
            .field("nd3", &self.nd3())
            .field("nd4", &self.nd4())
            .field("nd5", &self.nd5())
            .field("nd6", &self.nd6())
            .field("nd7", &self.nd7())
            .field("nd8", &self.nd8())
            .field("nd9", &self.nd9())
            .field("nd10", &self.nd10())
            .field("nd11", &self.nd11())
            .field("nd12", &self.nd12())
            .field("nd13", &self.nd13())
            .field("nd14", &self.nd14())
            .field("nd15", &self.nd15())
            .field("nd16", &self.nd16())
            .field("nd17", &self.nd17())
            .field("nd18", &self.nd18())
            .field("nd19", &self.nd19())
            .field("nd20", &self.nd20())
            .field("nd21", &self.nd21())
            .field("nd22", &self.nd22())
            .field("nd23", &self.nd23())
            .field("nd24", &self.nd24())
            .field("nd25", &self.nd25())
            .field("nd26", &self.nd26())
            .field("nd27", &self.nd27())
            .field("nd28", &self.nd28())
            .field("nd29", &self.nd29())
            .field("nd30", &self.nd30())
            .field("nd31", &self.nd31())
            .finish()
    }
}
impl W {
    ///Bit 0 - ND0
    #[inline(always)]
    #[must_use]
    pub fn nd0(&mut self) -> ND0_W<FDCAN_NDAT1rs> {
        ND0_W::new(self, 0)
    }
    ///Bit 1 - ND1
    #[inline(always)]
    #[must_use]
    pub fn nd1(&mut self) -> ND1_W<FDCAN_NDAT1rs> {
        ND1_W::new(self, 1)
    }
    ///Bit 2 - ND2
    #[inline(always)]
    #[must_use]
    pub fn nd2(&mut self) -> ND2_W<FDCAN_NDAT1rs> {
        ND2_W::new(self, 2)
    }
    ///Bit 3 - ND3
    #[inline(always)]
    #[must_use]
    pub fn nd3(&mut self) -> ND3_W<FDCAN_NDAT1rs> {
        ND3_W::new(self, 3)
    }
    ///Bit 4 - ND4
    #[inline(always)]
    #[must_use]
    pub fn nd4(&mut self) -> ND4_W<FDCAN_NDAT1rs> {
        ND4_W::new(self, 4)
    }
    ///Bit 5 - ND5
    #[inline(always)]
    #[must_use]
    pub fn nd5(&mut self) -> ND5_W<FDCAN_NDAT1rs> {
        ND5_W::new(self, 5)
    }
    ///Bit 6 - ND6
    #[inline(always)]
    #[must_use]
    pub fn nd6(&mut self) -> ND6_W<FDCAN_NDAT1rs> {
        ND6_W::new(self, 6)
    }
    ///Bit 7 - ND7
    #[inline(always)]
    #[must_use]
    pub fn nd7(&mut self) -> ND7_W<FDCAN_NDAT1rs> {
        ND7_W::new(self, 7)
    }
    ///Bit 8 - ND8
    #[inline(always)]
    #[must_use]
    pub fn nd8(&mut self) -> ND8_W<FDCAN_NDAT1rs> {
        ND8_W::new(self, 8)
    }
    ///Bit 9 - ND9
    #[inline(always)]
    #[must_use]
    pub fn nd9(&mut self) -> ND9_W<FDCAN_NDAT1rs> {
        ND9_W::new(self, 9)
    }
    ///Bit 10 - ND10
    #[inline(always)]
    #[must_use]
    pub fn nd10(&mut self) -> ND10_W<FDCAN_NDAT1rs> {
        ND10_W::new(self, 10)
    }
    ///Bit 11 - ND11
    #[inline(always)]
    #[must_use]
    pub fn nd11(&mut self) -> ND11_W<FDCAN_NDAT1rs> {
        ND11_W::new(self, 11)
    }
    ///Bit 12 - ND12
    #[inline(always)]
    #[must_use]
    pub fn nd12(&mut self) -> ND12_W<FDCAN_NDAT1rs> {
        ND12_W::new(self, 12)
    }
    ///Bit 13 - ND13
    #[inline(always)]
    #[must_use]
    pub fn nd13(&mut self) -> ND13_W<FDCAN_NDAT1rs> {
        ND13_W::new(self, 13)
    }
    ///Bit 14 - ND14
    #[inline(always)]
    #[must_use]
    pub fn nd14(&mut self) -> ND14_W<FDCAN_NDAT1rs> {
        ND14_W::new(self, 14)
    }
    ///Bit 15 - ND15
    #[inline(always)]
    #[must_use]
    pub fn nd15(&mut self) -> ND15_W<FDCAN_NDAT1rs> {
        ND15_W::new(self, 15)
    }
    ///Bit 16 - ND16
    #[inline(always)]
    #[must_use]
    pub fn nd16(&mut self) -> ND16_W<FDCAN_NDAT1rs> {
        ND16_W::new(self, 16)
    }
    ///Bit 17 - ND17
    #[inline(always)]
    #[must_use]
    pub fn nd17(&mut self) -> ND17_W<FDCAN_NDAT1rs> {
        ND17_W::new(self, 17)
    }
    ///Bit 18 - ND18
    #[inline(always)]
    #[must_use]
    pub fn nd18(&mut self) -> ND18_W<FDCAN_NDAT1rs> {
        ND18_W::new(self, 18)
    }
    ///Bit 19 - ND19
    #[inline(always)]
    #[must_use]
    pub fn nd19(&mut self) -> ND19_W<FDCAN_NDAT1rs> {
        ND19_W::new(self, 19)
    }
    ///Bit 20 - ND20
    #[inline(always)]
    #[must_use]
    pub fn nd20(&mut self) -> ND20_W<FDCAN_NDAT1rs> {
        ND20_W::new(self, 20)
    }
    ///Bit 21 - ND21
    #[inline(always)]
    #[must_use]
    pub fn nd21(&mut self) -> ND21_W<FDCAN_NDAT1rs> {
        ND21_W::new(self, 21)
    }
    ///Bit 22 - ND22
    #[inline(always)]
    #[must_use]
    pub fn nd22(&mut self) -> ND22_W<FDCAN_NDAT1rs> {
        ND22_W::new(self, 22)
    }
    ///Bit 23 - ND23
    #[inline(always)]
    #[must_use]
    pub fn nd23(&mut self) -> ND23_W<FDCAN_NDAT1rs> {
        ND23_W::new(self, 23)
    }
    ///Bit 24 - ND24
    #[inline(always)]
    #[must_use]
    pub fn nd24(&mut self) -> ND24_W<FDCAN_NDAT1rs> {
        ND24_W::new(self, 24)
    }
    ///Bit 25 - ND25
    #[inline(always)]
    #[must_use]
    pub fn nd25(&mut self) -> ND25_W<FDCAN_NDAT1rs> {
        ND25_W::new(self, 25)
    }
    ///Bit 26 - ND26
    #[inline(always)]
    #[must_use]
    pub fn nd26(&mut self) -> ND26_W<FDCAN_NDAT1rs> {
        ND26_W::new(self, 26)
    }
    ///Bit 27 - ND27
    #[inline(always)]
    #[must_use]
    pub fn nd27(&mut self) -> ND27_W<FDCAN_NDAT1rs> {
        ND27_W::new(self, 27)
    }
    ///Bit 28 - ND28
    #[inline(always)]
    #[must_use]
    pub fn nd28(&mut self) -> ND28_W<FDCAN_NDAT1rs> {
        ND28_W::new(self, 28)
    }
    ///Bit 29 - ND29
    #[inline(always)]
    #[must_use]
    pub fn nd29(&mut self) -> ND29_W<FDCAN_NDAT1rs> {
        ND29_W::new(self, 29)
    }
    ///Bit 30 - ND30
    #[inline(always)]
    #[must_use]
    pub fn nd30(&mut self) -> ND30_W<FDCAN_NDAT1rs> {
        ND30_W::new(self, 30)
    }
    ///Bit 31 - ND31
    #[inline(always)]
    #[must_use]
    pub fn nd31(&mut self) -> ND31_W<FDCAN_NDAT1rs> {
        ND31_W::new(self, 31)
    }
}
/**FDCAN new data 1 register

You can [`read`](crate::Reg::read) this register and get [`fdcan_ndat1::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`fdcan_ndat1::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32MP153.html#FDCAN1:FDCAN_NDAT1)*/
pub struct FDCAN_NDAT1rs;
impl crate::RegisterSpec for FDCAN_NDAT1rs {
    type Ux = u32;
}
///`read()` method returns [`fdcan_ndat1::R`](R) reader structure
impl crate::Readable for FDCAN_NDAT1rs {}
///`write(|w| ..)` method takes [`fdcan_ndat1::W`](W) writer structure
impl crate::Writable for FDCAN_NDAT1rs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets FDCAN_NDAT1 to value 0
impl crate::Resettable for FDCAN_NDAT1rs {
    const RESET_VALUE: u32 = 0;
}
