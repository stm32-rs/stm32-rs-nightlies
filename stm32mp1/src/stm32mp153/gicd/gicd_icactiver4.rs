///Register `GICD_ICACTIVER4` reader
pub type R = crate::R<GICD_ICACTIVER4rs>;
///Register `GICD_ICACTIVER4` writer
pub type W = crate::W<GICD_ICACTIVER4rs>;
///Field `ICACTIVER4` reader - ICACTIVER4
pub type ICACTIVER4_R = crate::FieldReader<u32>;
///Field `ICACTIVER4` writer - ICACTIVER4
pub type ICACTIVER4_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - ICACTIVER4
    #[inline(always)]
    pub fn icactiver4(&self) -> ICACTIVER4_R {
        ICACTIVER4_R::new(self.bits)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("GICD_ICACTIVER4")
            .field("icactiver4", &self.icactiver4())
            .finish()
    }
}
impl W {
    ///Bits 0:31 - ICACTIVER4
    #[inline(always)]
    #[must_use]
    pub fn icactiver4(&mut self) -> ICACTIVER4_W<GICD_ICACTIVER4rs> {
        ICACTIVER4_W::new(self, 0)
    }
}
/**For interrupts ID

You can [`read`](crate::Reg::read) this register and get [`gicd_icactiver4::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`gicd_icactiver4::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32MP153.html#GICD:GICD_ICACTIVER4)*/
pub struct GICD_ICACTIVER4rs;
impl crate::RegisterSpec for GICD_ICACTIVER4rs {
    type Ux = u32;
}
///`read()` method returns [`gicd_icactiver4::R`](R) reader structure
impl crate::Readable for GICD_ICACTIVER4rs {}
///`write(|w| ..)` method takes [`gicd_icactiver4::W`](W) writer structure
impl crate::Writable for GICD_ICACTIVER4rs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets GICD_ICACTIVER4 to value 0
impl crate::Resettable for GICD_ICACTIVER4rs {
    const RESET_VALUE: u32 = 0;
}
