///Register `GICD_ISACTIVER0` reader
pub type R = crate::R<GICD_ISACTIVER0rs>;
///Register `GICD_ISACTIVER0` writer
pub type W = crate::W<GICD_ISACTIVER0rs>;
///Field `ISACTIVER0` reader - ISACTIVER0
pub type ISACTIVER0_R = crate::FieldReader<u32>;
///Field `ISACTIVER0` writer - ISACTIVER0
pub type ISACTIVER0_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - ISACTIVER0
    #[inline(always)]
    pub fn isactiver0(&self) -> ISACTIVER0_R {
        ISACTIVER0_R::new(self.bits)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("GICD_ISACTIVER0")
            .field("isactiver0", &self.isactiver0())
            .finish()
    }
}
impl W {
    ///Bits 0:31 - ISACTIVER0
    #[inline(always)]
    #[must_use]
    pub fn isactiver0(&mut self) -> ISACTIVER0_W<GICD_ISACTIVER0rs> {
        ISACTIVER0_W::new(self, 0)
    }
}
/**For interrupts ID

You can [`read`](crate::Reg::read) this register and get [`gicd_isactiver0::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`gicd_isactiver0::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32MP153.html#GICD:GICD_ISACTIVER0)*/
pub struct GICD_ISACTIVER0rs;
impl crate::RegisterSpec for GICD_ISACTIVER0rs {
    type Ux = u32;
}
///`read()` method returns [`gicd_isactiver0::R`](R) reader structure
impl crate::Readable for GICD_ISACTIVER0rs {}
///`write(|w| ..)` method takes [`gicd_isactiver0::W`](W) writer structure
impl crate::Writable for GICD_ISACTIVER0rs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets GICD_ISACTIVER0 to value 0
impl crate::Resettable for GICD_ISACTIVER0rs {
    const RESET_VALUE: u32 = 0;
}
