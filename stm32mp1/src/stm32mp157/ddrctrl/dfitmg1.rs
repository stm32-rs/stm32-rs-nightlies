...
impl W {
    ///Bits 0:4 - DFI_T_DRAM_CLK_ENABLE
    #[inline(always)]
    pub fn dfi_t_dram_clk_enable(&mut self) -> DFI_T_DRAM_CLK_ENABLE_W<DFITMG1rs> {
        DFI_T_DRAM_CLK_ENABLE_W::new(self, 0)
    }
    ///Bits 8:12 - DFI_T_DRAM_CLK_DISABLE
    #[inline(always)]
    pub fn dfi_t_dram_clk_disable(&mut self) -> DFI_T_DRAM_CLK_DISABLE_W<DFITMG1rs> {
        DFI_T_DRAM_CLK_DISABLE_W::new(self, 8)
    }
    ///Bits 16:20 - DFI_T_WRDATA_DELAY
    #[inline(always)]
    pub fn dfi_t_wrdata_delay(&mut self) -> DFI_T_WRDATA_DELAY_W<DFITMG1rs> {
