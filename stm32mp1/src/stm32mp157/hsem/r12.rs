...
impl W {
    ///Bits 0:7 - PROCID
    #[inline(always)]
    pub fn procid(&mut self) -> PROCID_W<'_, R12rs> {
        PROCID_W::new(self, 0)
    }
    ///Bits 8:11 - COREID
    #[inline(always)]
    pub fn coreid(&mut self) -> COREID_W<'_, R12rs> {
        COREID_W::new(self, 8)
    }
    ///Bit 31 - LOCK
    #[inline(always)]
    pub fn lock(&mut self) -> LOCK_W<'_, R12rs> {
        LOCK_W::new(self, 31)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32MP157.html#HSEM:R12)*/
pub struct R12rs;
