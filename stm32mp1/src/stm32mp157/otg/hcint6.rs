...
impl W {
    ///Bit 0 - XFRC
    #[inline(always)]
    pub fn xfrc(&mut self) -> XFRC_W<'_, HCINT6rs> {
        XFRC_W::new(self, 0)
    }
    ///Bit 1 - CHH
    #[inline(always)]
    pub fn chh(&mut self) -> CHH_W<'_, HCINT6rs> {
        CHH_W::new(self, 1)
    }
    ///Bit 2 - AHBERR
    #[inline(always)]
    pub fn ahberr(&mut self) -> AHBERR_W<'_, HCINT6rs> {
        AHBERR_W::new(self, 2)
    }
    ///Bit 3 - STALL
    #[inline(always)]
    pub fn stall(&mut self) -> STALL_W<'_, HCINT6rs> {
        STALL_W::new(self, 3)
    }
    ///Bit 4 - NAK
    #[inline(always)]
    pub fn nak(&mut self) -> NAK_W<'_, HCINT6rs> {
        NAK_W::new(self, 4)
    }
    ///Bit 5 - ACK
    #[inline(always)]
    pub fn ack(&mut self) -> ACK_W<'_, HCINT6rs> {
        ACK_W::new(self, 5)
    }
    ///Bit 6 - NYET
    #[inline(always)]
    pub fn nyet(&mut self) -> NYET_W<'_, HCINT6rs> {
        NYET_W::new(self, 6)
    }
    ///Bit 7 - TXERR
    #[inline(always)]
    pub fn txerr(&mut self) -> TXERR_W<'_, HCINT6rs> {
        TXERR_W::new(self, 7)
    }
    ///Bit 8 - BBERR
    #[inline(always)]
    pub fn bberr(&mut self) -> BBERR_W<'_, HCINT6rs> {
        BBERR_W::new(self, 8)
    }
    ///Bit 9 - FRMOR
    #[inline(always)]
    pub fn frmor(&mut self) -> FRMOR_W<'_, HCINT6rs> {
        FRMOR_W::new(self, 9)
    }
    ///Bit 10 - DTERR
    #[inline(always)]
    pub fn dterr(&mut self) -> DTERR_W<'_, HCINT6rs> {
        DTERR_W::new(self, 10)
    }
    ///Bit 11 - BNA
    #[inline(always)]
    pub fn bna(&mut self) -> BNA_W<'_, HCINT6rs> {
        BNA_W::new(self, 11)
    }
    ///Bit 12 - XCSXACTERR
    #[inline(always)]
    pub fn xcsxacterr(&mut self) -> XCSXACTERR_W<'_, HCINT6rs> {
        XCSXACTERR_W::new(self, 12)
    }
    ///Bit 13 - DESCLSTROLL
    #[inline(always)]
    pub fn desclstroll(&mut self) -> DESCLSTROLL_W<'_, HCINT6rs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32MP157.html#OTG:HCINT6)*/
pub struct HCINT6rs;
impl crate::RegisterSpec for HCINT6rs {
    type Ux = u32;
}
///`read()` method returns [`hcint6::R`](R) reader structure
impl crate::Readable for HCINT6rs {}
///`write(|w| ..)` method takes [`hcint6::W`](W) writer structure
