...
impl W {
    ///Bit 0 - None
    #[inline(always)]
    pub fn enable(&mut self) -> ENABLE_W<P1YUVCRrs> {
        ENABLE_W::new(self, 0)
    }
    ///Bit 1 - Output samples type used while CLAMP is activated
    #[inline(always)]
    pub fn type_(&mut self) -> TYPE_W<P1YUVCRrs> {
        TYPE_W::new(self, 1)
    }
    ///Bit 2 - Clamp the output samples
    #[inline(always)]
    pub fn clamp(&mut self) -> CLAMP_W<P1YUVCRrs> {
        CLAMP_W::new(self, 2)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N645.html#DCMIPP:P1YUVCR)*/
