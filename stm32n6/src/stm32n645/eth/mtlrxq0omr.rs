...
impl W {
    ///Bits 0:1 - Receive Queue Threshold Control
    #[inline(always)]
    pub fn rtc(&mut self) -> RTC_W<MTLRXQ0OMRrs> {
        RTC_W::new(self, 0)
    }
    ///Bit 3 - Forward Undersized Good Packets
    #[inline(always)]
    pub fn fup(&mut self) -> FUP_W<MTLRXQ0OMRrs> {
        FUP_W::new(self, 3)
    }
    ///Bit 4 - Forward Error Packets
    #[inline(always)]
    pub fn fep(&mut self) -> FEP_W<MTLRXQ0OMRrs> {
        FEP_W::new(self, 4)
    }
    ///Bit 5 - Receive Queue Store and Forward
    #[inline(always)]
    pub fn rsf(&mut self) -> RSF_W<MTLRXQ0OMRrs> {
        RSF_W::new(self, 5)
    }
    ///Bit 6 - Disable Dropping of TCP/IP Checksum Error Packets
    #[inline(always)]
    pub fn dis_tcp_ef(&mut self) -> DIS_TCP_EF_W<MTLRXQ0OMRrs> {
        DIS_TCP_EF_W::new(self, 6)
    }
    ///Bit 7 - Enable Hardware Flow Control
    #[inline(always)]
    pub fn ehfc(&mut self) -> EHFC_W<MTLRXQ0OMRrs> {
        EHFC_W::new(self, 7)
    }
    ///Bits 8:10 - Threshold for Activating Flow Control (in Half-duplex and Full-duplex)
    #[inline(always)]
    pub fn rfa(&mut self) -> RFA_W<MTLRXQ0OMRrs> {
        RFA_W::new(self, 8)
    }
    ///Bits 14:16 - Threshold for Deactivating Flow Control (in Half-duplex and Full-duplex modes)
    #[inline(always)]
    pub fn rfd(&mut self) -> RFD_W<MTLRXQ0OMRrs> {
        RFD_W::new(self, 14)
    }
    ///Bits 20:23 - Receive Queue Size
    #[inline(always)]
    pub fn rqs(&mut self) -> RQS_W<MTLRXQ0OMRrs> {
        RQS_W::new(self, 20)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N645.html#ETH:MTLRXQ0OMR)*/
