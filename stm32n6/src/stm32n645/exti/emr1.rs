///Register `EMR1` reader
pub type R = crate::R<EMR1rs>;
///Register `EMR1` writer
pub type W = crate::W<EMR1rs>;
///Field `EM0` reader - CPU wake-up with interrupt mask on event input x
pub type EM0_R = crate::BitReader;
///Field `EM0` writer - CPU wake-up with interrupt mask on event input x
pub type EM0_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM1` reader - CPU wake-up with interrupt mask on event input x
pub type EM1_R = crate::BitReader;
///Field `EM1` writer - CPU wake-up with interrupt mask on event input x
pub type EM1_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM2` reader - CPU wake-up with interrupt mask on event input x
pub type EM2_R = crate::BitReader;
///Field `EM2` writer - CPU wake-up with interrupt mask on event input x
pub type EM2_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM3` reader - CPU wake-up with interrupt mask on event input x
pub type EM3_R = crate::BitReader;
///Field `EM3` writer - CPU wake-up with interrupt mask on event input x
pub type EM3_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM4` reader - CPU wake-up with interrupt mask on event input x
pub type EM4_R = crate::BitReader;
///Field `EM4` writer - CPU wake-up with interrupt mask on event input x
pub type EM4_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM5` reader - CPU wake-up with interrupt mask on event input x
pub type EM5_R = crate::BitReader;
///Field `EM5` writer - CPU wake-up with interrupt mask on event input x
pub type EM5_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM6` reader - CPU wake-up with interrupt mask on event input x
pub type EM6_R = crate::BitReader;
///Field `EM6` writer - CPU wake-up with interrupt mask on event input x
pub type EM6_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM7` reader - CPU wake-up with interrupt mask on event input x
pub type EM7_R = crate::BitReader;
///Field `EM7` writer - CPU wake-up with interrupt mask on event input x
pub type EM7_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM8` reader - CPU wake-up with interrupt mask on event input x
pub type EM8_R = crate::BitReader;
///Field `EM8` writer - CPU wake-up with interrupt mask on event input x
pub type EM8_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM9` reader - CPU wake-up with interrupt mask on event input x
pub type EM9_R = crate::BitReader;
///Field `EM9` writer - CPU wake-up with interrupt mask on event input x
pub type EM9_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM10` reader - CPU wake-up with interrupt mask on event input x
pub type EM10_R = crate::BitReader;
///Field `EM10` writer - CPU wake-up with interrupt mask on event input x
pub type EM10_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM11` reader - CPU wake-up with interrupt mask on event input x
pub type EM11_R = crate::BitReader;
///Field `EM11` writer - CPU wake-up with interrupt mask on event input x
pub type EM11_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM12` reader - CPU wake-up with interrupt mask on event input x
pub type EM12_R = crate::BitReader;
///Field `EM12` writer - CPU wake-up with interrupt mask on event input x
pub type EM12_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM13` reader - CPU wake-up with interrupt mask on event input x
pub type EM13_R = crate::BitReader;
///Field `EM13` writer - CPU wake-up with interrupt mask on event input x
pub type EM13_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM14` reader - CPU wake-up with interrupt mask on event input x
pub type EM14_R = crate::BitReader;
///Field `EM14` writer - CPU wake-up with interrupt mask on event input x
pub type EM14_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM15` reader - CPU wake-up with interrupt mask on event input x
pub type EM15_R = crate::BitReader;
///Field `EM15` writer - CPU wake-up with interrupt mask on event input x
pub type EM15_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM17` reader - CPU wake-up with event on event input x
pub type EM17_R = crate::BitReader;
///Field `EM17` writer - CPU wake-up with event on event input x
pub type EM17_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM18` reader - CPU wake-up with event on event input x
pub type EM18_R = crate::BitReader;
///Field `EM18` writer - CPU wake-up with event on event input x
pub type EM18_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM19` reader - CPU wake-up with event on event input x
pub type EM19_R = crate::BitReader;
///Field `EM19` writer - CPU wake-up with event on event input x
pub type EM19_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM20` reader - CPU wake-up with event on event input x
pub type EM20_R = crate::BitReader;
///Field `EM20` writer - CPU wake-up with event on event input x
pub type EM20_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM21` reader - CPU wake-up with event on event input x
pub type EM21_R = crate::BitReader;
///Field `EM21` writer - CPU wake-up with event on event input x
pub type EM21_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM22` reader - CPU wake-up with event on event input x
pub type EM22_R = crate::BitReader;
///Field `EM22` writer - CPU wake-up with event on event input x
pub type EM22_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM23` reader - CPU wake-up with event on event input x
pub type EM23_R = crate::BitReader;
///Field `EM23` writer - CPU wake-up with event on event input x
pub type EM23_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM24` reader - CPU wake-up with event on event input x
pub type EM24_R = crate::BitReader;
///Field `EM24` writer - CPU wake-up with event on event input x
pub type EM24_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM25` reader - CPU wake-up with event on event input x
pub type EM25_R = crate::BitReader;
///Field `EM25` writer - CPU wake-up with event on event input x
pub type EM25_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM26` reader - CPU wake-up with event on event input x
pub type EM26_R = crate::BitReader;
///Field `EM26` writer - CPU wake-up with event on event input x
pub type EM26_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM27` reader - CPU wake-up with event on event input x
pub type EM27_R = crate::BitReader;
///Field `EM27` writer - CPU wake-up with event on event input x
pub type EM27_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM28` reader - CPU wake-up with event on event input x
pub type EM28_R = crate::BitReader;
///Field `EM28` writer - CPU wake-up with event on event input x
pub type EM28_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM29` reader - CPU wake-up with event on event input x
pub type EM29_R = crate::BitReader;
///Field `EM29` writer - CPU wake-up with event on event input x
pub type EM29_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM30` reader - CPU wake-up with event on event input x
pub type EM30_R = crate::BitReader;
///Field `EM30` writer - CPU wake-up with event on event input x
pub type EM30_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `EM31` reader - CPU wake-up with event on event input x
pub type EM31_R = crate::BitReader;
///Field `EM31` writer - CPU wake-up with event on event input x
pub type EM31_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em0(&self) -> EM0_R {
        EM0_R::new((self.bits & 1) != 0)
    }
    ///Bit 1 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em1(&self) -> EM1_R {
        EM1_R::new(((self.bits >> 1) & 1) != 0)
    }
    ///Bit 2 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em2(&self) -> EM2_R {
        EM2_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em3(&self) -> EM3_R {
        EM3_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 4 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em4(&self) -> EM4_R {
        EM4_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em5(&self) -> EM5_R {
        EM5_R::new(((self.bits >> 5) & 1) != 0)
    }
    ///Bit 6 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em6(&self) -> EM6_R {
        EM6_R::new(((self.bits >> 6) & 1) != 0)
    }
    ///Bit 7 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em7(&self) -> EM7_R {
        EM7_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em8(&self) -> EM8_R {
        EM8_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em9(&self) -> EM9_R {
        EM9_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em10(&self) -> EM10_R {
        EM10_R::new(((self.bits >> 10) & 1) != 0)
    }
    ///Bit 11 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em11(&self) -> EM11_R {
        EM11_R::new(((self.bits >> 11) & 1) != 0)
    }
    ///Bit 12 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em12(&self) -> EM12_R {
        EM12_R::new(((self.bits >> 12) & 1) != 0)
    }
    ///Bit 13 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em13(&self) -> EM13_R {
        EM13_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em14(&self) -> EM14_R {
        EM14_R::new(((self.bits >> 14) & 1) != 0)
    }
    ///Bit 15 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em15(&self) -> EM15_R {
        EM15_R::new(((self.bits >> 15) & 1) != 0)
    }
    ///Bit 17 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em17(&self) -> EM17_R {
        EM17_R::new(((self.bits >> 17) & 1) != 0)
    }
    ///Bit 18 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em18(&self) -> EM18_R {
        EM18_R::new(((self.bits >> 18) & 1) != 0)
    }
    ///Bit 19 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em19(&self) -> EM19_R {
        EM19_R::new(((self.bits >> 19) & 1) != 0)
    }
    ///Bit 20 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em20(&self) -> EM20_R {
        EM20_R::new(((self.bits >> 20) & 1) != 0)
    }
    ///Bit 21 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em21(&self) -> EM21_R {
        EM21_R::new(((self.bits >> 21) & 1) != 0)
    }
    ///Bit 22 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em22(&self) -> EM22_R {
        EM22_R::new(((self.bits >> 22) & 1) != 0)
    }
    ///Bit 23 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em23(&self) -> EM23_R {
        EM23_R::new(((self.bits >> 23) & 1) != 0)
    }
    ///Bit 24 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em24(&self) -> EM24_R {
        EM24_R::new(((self.bits >> 24) & 1) != 0)
    }
    ///Bit 25 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em25(&self) -> EM25_R {
        EM25_R::new(((self.bits >> 25) & 1) != 0)
    }
    ///Bit 26 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em26(&self) -> EM26_R {
        EM26_R::new(((self.bits >> 26) & 1) != 0)
    }
    ///Bit 27 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em27(&self) -> EM27_R {
        EM27_R::new(((self.bits >> 27) & 1) != 0)
    }
    ///Bit 28 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em28(&self) -> EM28_R {
        EM28_R::new(((self.bits >> 28) & 1) != 0)
    }
    ///Bit 29 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em29(&self) -> EM29_R {
        EM29_R::new(((self.bits >> 29) & 1) != 0)
    }
    ///Bit 30 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em30(&self) -> EM30_R {
        EM30_R::new(((self.bits >> 30) & 1) != 0)
    }
    ///Bit 31 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em31(&self) -> EM31_R {
        EM31_R::new(((self.bits >> 31) & 1) != 0)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("EMR1")
            .field("em0", &self.em0())
            .field("em1", &self.em1())
            .field("em2", &self.em2())
            .field("em3", &self.em3())
            .field("em4", &self.em4())
            .field("em5", &self.em5())
            .field("em6", &self.em6())
            .field("em7", &self.em7())
            .field("em8", &self.em8())
            .field("em9", &self.em9())
            .field("em10", &self.em10())
            .field("em11", &self.em11())
            .field("em12", &self.em12())
            .field("em13", &self.em13())
            .field("em14", &self.em14())
            .field("em15", &self.em15())
            .field("em17", &self.em17())
            .field("em18", &self.em18())
            .field("em19", &self.em19())
            .field("em20", &self.em20())
            .field("em21", &self.em21())
            .field("em22", &self.em22())
            .field("em23", &self.em23())
            .field("em24", &self.em24())
            .field("em25", &self.em25())
            .field("em26", &self.em26())
            .field("em27", &self.em27())
            .field("em28", &self.em28())
            .field("em29", &self.em29())
            .field("em30", &self.em30())
            .field("em31", &self.em31())
            .finish()
    }
}
impl W {
    ///Bit 0 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em0(&mut self) -> EM0_W<'_, EMR1rs> {
        EM0_W::new(self, 0)
    }
    ///Bit 1 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em1(&mut self) -> EM1_W<'_, EMR1rs> {
        EM1_W::new(self, 1)
    }
    ///Bit 2 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em2(&mut self) -> EM2_W<'_, EMR1rs> {
        EM2_W::new(self, 2)
    }
    ///Bit 3 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em3(&mut self) -> EM3_W<'_, EMR1rs> {
        EM3_W::new(self, 3)
    }
    ///Bit 4 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em4(&mut self) -> EM4_W<'_, EMR1rs> {
        EM4_W::new(self, 4)
    }
    ///Bit 5 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em5(&mut self) -> EM5_W<'_, EMR1rs> {
        EM5_W::new(self, 5)
    }
    ///Bit 6 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em6(&mut self) -> EM6_W<'_, EMR1rs> {
        EM6_W::new(self, 6)
    }
    ///Bit 7 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em7(&mut self) -> EM7_W<'_, EMR1rs> {
        EM7_W::new(self, 7)
    }
    ///Bit 8 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em8(&mut self) -> EM8_W<'_, EMR1rs> {
        EM8_W::new(self, 8)
    }
    ///Bit 9 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em9(&mut self) -> EM9_W<'_, EMR1rs> {
        EM9_W::new(self, 9)
    }
    ///Bit 10 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em10(&mut self) -> EM10_W<'_, EMR1rs> {
        EM10_W::new(self, 10)
    }
    ///Bit 11 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em11(&mut self) -> EM11_W<'_, EMR1rs> {
        EM11_W::new(self, 11)
    }
    ///Bit 12 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em12(&mut self) -> EM12_W<'_, EMR1rs> {
        EM12_W::new(self, 12)
    }
    ///Bit 13 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em13(&mut self) -> EM13_W<'_, EMR1rs> {
        EM13_W::new(self, 13)
    }
    ///Bit 14 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em14(&mut self) -> EM14_W<'_, EMR1rs> {
        EM14_W::new(self, 14)
    }
    ///Bit 15 - CPU wake-up with interrupt mask on event input x
    #[inline(always)]
    pub fn em15(&mut self) -> EM15_W<'_, EMR1rs> {
        EM15_W::new(self, 15)
    }
    ///Bit 17 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em17(&mut self) -> EM17_W<'_, EMR1rs> {
        EM17_W::new(self, 17)
    }
    ///Bit 18 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em18(&mut self) -> EM18_W<'_, EMR1rs> {
        EM18_W::new(self, 18)
    }
    ///Bit 19 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em19(&mut self) -> EM19_W<'_, EMR1rs> {
        EM19_W::new(self, 19)
    }
    ///Bit 20 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em20(&mut self) -> EM20_W<'_, EMR1rs> {
        EM20_W::new(self, 20)
    }
    ///Bit 21 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em21(&mut self) -> EM21_W<'_, EMR1rs> {
        EM21_W::new(self, 21)
    }
    ///Bit 22 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em22(&mut self) -> EM22_W<'_, EMR1rs> {
        EM22_W::new(self, 22)
    }
    ///Bit 23 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em23(&mut self) -> EM23_W<'_, EMR1rs> {
        EM23_W::new(self, 23)
    }
    ///Bit 24 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em24(&mut self) -> EM24_W<'_, EMR1rs> {
        EM24_W::new(self, 24)
    }
    ///Bit 25 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em25(&mut self) -> EM25_W<'_, EMR1rs> {
        EM25_W::new(self, 25)
    }
    ///Bit 26 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em26(&mut self) -> EM26_W<'_, EMR1rs> {
        EM26_W::new(self, 26)
    }
    ///Bit 27 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em27(&mut self) -> EM27_W<'_, EMR1rs> {
        EM27_W::new(self, 27)
    }
    ///Bit 28 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em28(&mut self) -> EM28_W<'_, EMR1rs> {
        EM28_W::new(self, 28)
    }
    ///Bit 29 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em29(&mut self) -> EM29_W<'_, EMR1rs> {
        EM29_W::new(self, 29)
    }
    ///Bit 30 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em30(&mut self) -> EM30_W<'_, EMR1rs> {
        EM30_W::new(self, 30)
    }
    ///Bit 31 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em31(&mut self) -> EM31_W<'_, EMR1rs> {
        EM31_W::new(self, 31)
    }
}
/**EXTI CPU wake-up with event mask register 1

You can [`read`](crate::Reg::read) this register and get [`emr1::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`emr1::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N645.html#EXTI:EMR1)*/
pub struct EMR1rs;
impl crate::RegisterSpec for EMR1rs {
    type Ux = u32;
}
///`read()` method returns [`emr1::R`](R) reader structure
impl crate::Readable for EMR1rs {}
///`write(|w| ..)` method takes [`emr1::W`](W) writer structure
impl crate::Writable for EMR1rs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets EMR1 to value 0
impl crate::Resettable for EMR1rs {}
