...
impl W {
    ///Bits 1:7 - Assigned I3C dynamic address to target x (when the I3C acts as controller)
    #[inline(always)]
    pub fn da(&mut self) -> DA_W<DEVR2rs> {
        DA_W::new(self, 1)
    }
    ///Bit 16 - IBI request acknowledge (when the I3C acts as controller)
    #[inline(always)]
    pub fn ibiack(&mut self) -> IBIACK_W<DEVR2rs> {
        IBIACK_W::new(self, 16)
    }
    ///Bit 17 - Controller-role request acknowledge (when the I3C acts as controller)
    #[inline(always)]
    pub fn crack(&mut self) -> CRACK_W<DEVR2rs> {
        CRACK_W::new(self, 17)
    }
    ///Bit 18 - IBI data enable (when the I3C acts as controller)
    #[inline(always)]
    pub fn ibiden(&mut self) -> IBIDEN_W<DEVR2rs> {
        IBIDEN_W::new(self, 18)
    }
    ///Bit 19 - Suspend/stop I3C transfer on received IBI (when the I3C acts as controller)
    #[inline(always)]
    pub fn susp(&mut self) -> SUSP_W<DEVR2rs> {
        SUSP_W::new(self, 19)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N645.html#I3C1:DEVR2)*/
pub struct DEVR2rs;
