...
///Field `SCLL_OD` reader - SCL low duration in open-drain phases, used for legacy I less than sup>2 less than /sup>C messages and for I3C open-drain phases (address phase following a start, ACK phase during controller-initiated messages, and T bit phase during direct/private/IBI payload), in number of kernel clocks cycles:
pub type SCLL_OD_R = crate::FieldReader;
///Field `SCLL_OD` writer - SCL low duration in open-drain phases, used for legacy I less than sup>2 less than /sup>C messages and for I3C open-drain phases (address phase following a start, ACK phase during controller-initiated messages, and T bit phase during direct/private/IBI payload), in number of kernel clocks cycles:
pub type SCLL_OD_W<'a, REG> = crate::FieldWriter<'a, REG, 8>;
///Field `SCLH_I2C` reader - SCL high duration, used for legacy I less than sup>2 less than /sup>C messages, in number of kernel clocks cycles:
pub type SCLH_I2C_R = crate::FieldReader;
///Field `SCLH_I2C` writer - SCL high duration, used for legacy I less than sup>2 less than /sup>C messages, in number of kernel clocks cycles:
...
impl R {
    ...
    ///Bits 16:23 - SCL low duration in open-drain phases, used for legacy I less than sup>2 less than /sup>C messages and for I3C open-drain phases (address phase following a start, ACK phase during controller-initiated messages, and T bit phase during direct/private/IBI payload), in number of kernel clocks cycles:
    #[inline(always)]
    pub fn scll_od(&self) -> SCLL_OD_R {
        SCLL_OD_R::new(((self.bits >> 16) & 0xff) as u8)
    }
    ///Bits 24:31 - SCL high duration, used for legacy I less than sup>2 less than /sup>C messages, in number of kernel clocks cycles:
    #[inline(always)]
...
impl W {
    ///Bits 0:7 - SCL low duration in I3C push-pull phases, in number of kernel clocks cycles:
    #[inline(always)]
    pub fn scll_pp(&mut self) -> SCLL_PP_W<TIMINGR0rs> {
        SCLL_PP_W::new(self, 0)
    }
    ///Bits 8:15 - SCL high duration, used for I3C messages (both in push-pull and open-drain phases), in number of kernel clocks cycles:
    #[inline(always)]
    pub fn sclh_i3c(&mut self) -> SCLH_I3C_W<TIMINGR0rs> {
        SCLH_I3C_W::new(self, 8)
    }
    ///Bits 16:23 - SCL low duration in open-drain phases, used for legacy I less than sup>2 less than /sup>C messages and for I3C open-drain phases (address phase following a start, ACK phase during controller-initiated messages, and T bit phase during direct/private/IBI payload), in number of kernel clocks cycles:
    #[inline(always)]
    pub fn scll_od(&mut self) -> SCLL_OD_W<TIMINGR0rs> {
        SCLL_OD_W::new(self, 16)
    }
    ///Bits 24:31 - SCL high duration, used for legacy I less than sup>2 less than /sup>C messages, in number of kernel clocks cycles:
    #[inline(always)]
    pub fn sclh_i2c(&mut self) -> SCLH_I2C_W<TIMINGR0rs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N645.html#I3C1:TIMINGR0)*/
