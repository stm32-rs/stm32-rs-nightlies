...
///Field `CACHEINV` writer - cache invalidation
pub type CACHEINV_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `WAYSEL` reader - cache associativity mode selection
pub type WAYSEL_R = crate::BitReader;
///Field `WAYSEL` writer - cache associativity mode selection
...
impl W {
    ...
    ///Bit 2 - cache associativity mode selection
    #[inline(always)]
    pub fn waysel(&mut self) -> WAYSEL_W<'_, CRrs> {
        WAYSEL_W::new(self, 2)
    }
    ///Bit 16 - hit monitor enable
    #[inline(always)]
    pub fn hitmen(&mut self) -> HITMEN_W<'_, CRrs> {
        HITMEN_W::new(self, 16)
    }
    ///Bit 17 - miss monitor enable
    #[inline(always)]
    pub fn missmen(&mut self) -> MISSMEN_W<'_, CRrs> {
        MISSMEN_W::new(self, 17)
    }
    ///Bit 18 - hit monitor reset
    #[inline(always)]
    pub fn hitmrst(&mut self) -> HITMRST_W<'_, CRrs> {
        HITMRST_W::new(self, 18)
    }
    ///Bit 19 - miss monitor reset
    #[inline(always)]
    pub fn missmrst(&mut self) -> MISSMRST_W<'_, CRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N645.html#ICACHE:CR)*/
pub struct CRrs;
