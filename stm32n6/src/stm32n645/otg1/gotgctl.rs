...
///Field `VBVALOEN` reader - V less than sub>BUS less than /sub> valid override enable.
pub type VBVALOEN_R = crate::BitReader;
///Field `VBVALOEN` writer - V less than sub>BUS less than /sub> valid override enable.
pub type VBVALOEN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `VBVALOVAL` reader - V less than sub>BUS less than /sub> valid override value.
pub type VBVALOVAL_R = crate::BitReader;
///Field `VBVALOVAL` writer - V less than sub>BUS less than /sub> valid override value.
...
impl R {
    ///Bit 2 - V less than sub>BUS less than /sub> valid override enable.
    #[inline(always)]
    pub fn vbvaloen(&self) -> VBVALOEN_R {
        VBVALOEN_R::new(((self.bits >> 2) & 1) != 0)
    }
    ///Bit 3 - V less than sub>BUS less than /sub> valid override value.
    #[inline(always)]
