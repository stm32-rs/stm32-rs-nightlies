///Register `HCINT7` reader
pub type R = crate::R<HCINT7rs>;
///Register `HCINT7` writer
pub type W = crate::W<HCINT7rs>;
...
impl W {
    ///Bit 0 - Transfer completed.
    #[inline(always)]
    pub fn xfrc(&mut self) -> XFRC_W<'_, HCINT7rs> {
        XFRC_W::new(self, 0)
    }
    ///Bit 1 - Channel halted.
    #[inline(always)]
    pub fn chh(&mut self) -> CHH_W<'_, HCINT7rs> {
        CHH_W::new(self, 1)
    }
    ///Bit 2 - AHB error
    #[inline(always)]
    pub fn ahberr(&mut self) -> AHBERR_W<'_, HCINT7rs> {
        AHBERR_W::new(self, 2)
    }
    ///Bit 3 - STALL response received interrupt.
    #[inline(always)]
    pub fn stall(&mut self) -> STALL_W<'_, HCINT7rs> {
        STALL_W::new(self, 3)
    }
    ///Bit 4 - NAK response received interrupt.
    #[inline(always)]
    pub fn nak(&mut self) -> NAK_W<'_, HCINT7rs> {
        NAK_W::new(self, 4)
    }
    ///Bit 5 - ACK response received/transmitted interrupt.
    #[inline(always)]
    pub fn ack(&mut self) -> ACK_W<'_, HCINT7rs> {
        ACK_W::new(self, 5)
    }
    ///Bit 6 - Not yet ready response received interrupt.
    #[inline(always)]
    pub fn nyet(&mut self) -> NYET_W<'_, HCINT7rs> {
        NYET_W::new(self, 6)
    }
    ///Bit 7 - Transaction error.
    #[inline(always)]
    pub fn txerr(&mut self) -> TXERR_W<'_, HCINT7rs> {
        TXERR_W::new(self, 7)
    }
    ///Bit 8 - Babble error.
    #[inline(always)]
    pub fn bberr(&mut self) -> BBERR_W<'_, HCINT7rs> {
        BBERR_W::new(self, 8)
    }
    ///Bit 9 - Frame overrun.
    #[inline(always)]
    pub fn frmor(&mut self) -> FRMOR_W<'_, HCINT7rs> {
        FRMOR_W::new(self, 9)
    }
    ///Bit 10 - Data toggle error.
    #[inline(always)]
    pub fn dterr(&mut self) -> DTERR_W<'_, HCINT7rs> {
        DTERR_W::new(self, 10)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N645.html#OTG1:HCINT7)*/
pub struct HCINT7rs;
impl crate::RegisterSpec for HCINT7rs {
    type Ux = u32;
}
///`read()` method returns [`hcint7::R`](R) reader structure
impl crate::Readable for HCINT7rs {}
///`write(|w| ..)` method takes [`hcint7::W`](W) writer structure
impl crate::Writable for HCINT7rs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets HCINT7 to value 0
impl crate::Resettable for HCINT7rs {}
