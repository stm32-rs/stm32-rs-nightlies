...
///Field `PTXSA` reader - Host periodic Tx FIFO start address
pub type PTXSA_R = crate::FieldReader<u16>;
///Field `PTXSA` writer - Host periodic Tx FIFO start address
pub type PTXSA_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
///Field `PTXFSIZ` reader - Host periodic Tx FIFO depth
pub type PTXFSIZ_R = crate::FieldReader<u16>;
///Field `PTXFSIZ` writer - Host periodic Tx FIFO depth
pub type PTXFSIZ_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    ///Bits 0:15 - Host periodic Tx FIFO start address
    #[inline(always)]
    pub fn ptxsa(&self) -> PTXSA_R {
        PTXSA_R::new((self.bits & 0xffff) as u16)
    }
    ///Bits 16:31 - Host periodic Tx FIFO depth
    #[inline(always)]
