...
///Field `BKPRBSEN` reader - Backup RAM backup supply enable (used to maintain BKPRAM content in Standby and V less than sub>BAT less than /sub> modes).
pub type BKPRBSEN_R = crate::BitReader;
///Field `BKPRBSEN` writer - Backup RAM backup supply enable (used to maintain BKPRAM content in Standby and V less than sub>BAT less than /sub> modes).
pub type BKPRBSEN_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0 - Backup RAM backup supply enable (used to maintain BKPRAM content in Standby and V less than sub>BAT less than /sub> modes).
    #[inline(always)]
...
impl W {
    ///Bit 0 - Backup RAM backup supply enable (used to maintain BKPRAM content in Standby and V less than sub>BAT less than /sub> modes).
    #[inline(always)]
    pub fn bkprbsen(&mut self) -> BKPRBSEN_W<BDCR2rs> {
