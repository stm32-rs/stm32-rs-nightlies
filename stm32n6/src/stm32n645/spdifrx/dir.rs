...
///Field `THI` reader - threshold HIGH (THI = 2.5 x UI / T less than sub>spdifrx_ker_ck less than /sub>)
pub type THI_R = crate::FieldReader<u16>;
///Field `TLO` reader - threshold LOW (TLO = 1.5 x UI / T less than sub>spdifrx_ker_ck less than /sub>)
pub type TLO_R = crate::FieldReader<u16>;
impl R {
    ///Bits 0:12 - threshold HIGH (THI = 2.5 x UI / T less than sub>spdifrx_ker_ck less than /sub>)
    #[inline(always)]
    pub fn thi(&self) -> THI_R {
        THI_R::new((self.bits & 0x1fff) as u16)
    }
    ///Bits 16:28 - threshold LOW (TLO = 1.5 x UI / T less than sub>spdifrx_ker_ck less than /sub>)
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N645.html#SPDIFRX:DIR)*/
pub struct DIRrs;
