...
impl W {
    ///Bits 8:15 - any other value: debug not authorized (provided BSEC state is not OPEN)
    #[inline(always)]
    pub fn unlock(&mut self) -> UNLOCK_W<DBGCRrs> {
        UNLOCK_W::new(self, 8)
    }
    ///Bits 16:23 - level at which debug may be opened.
    #[inline(always)]
    pub fn auth_hdpl(&mut self) -> AUTH_HDPL_W<DBGCRrs> {
        AUTH_HDPL_W::new(self, 16)
    }
    ///Bits 24:31 - any other value: secure debug not authorized (provided BSEC state is not OPEN)
    #[inline(always)]
    pub fn auth_sec(&mut self) -> AUTH_SEC_W<DBGCRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N647.html#BSEC:DBGCR)*/
pub struct DBGCRrs;
