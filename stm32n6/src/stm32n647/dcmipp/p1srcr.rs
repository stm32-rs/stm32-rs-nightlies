...
impl W {
    ///Bits 0:11 - Amount of following lines to keep when CROPEN = 1. If LASTLINE = 0 all pixels after FIRSTLINEDEL are fed through.
    #[inline(always)]
    pub fn lastline(&mut self) -> LASTLINE_W<P1SRCRrs> {
        LASTLINE_W::new(self, 0)
    }
    ///Bits 12:14 - Amount of first lines to delete when CROPEN = 1
    #[inline(always)]
    pub fn firstlinedel(&mut self) -> FIRSTLINEDEL_W<P1SRCRrs> {
        FIRSTLINEDEL_W::new(self, 12)
    }
    ///Bit 15 - Crop line enable
    #[inline(always)]
    pub fn cropen(&mut self) -> CROPEN_W<P1SRCRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N647.html#DCMIPP:P1SRCR)*/
pub struct P1SRCRrs;
