...
impl W {
    ///Bits 0:15 - VLAN Tag ID
    #[inline(always)]
    pub fn vid(&mut self) -> VID_W<MACVTDRrs> {
        VID_W::new(self, 0)
    }
    ///Bit 16 - VLAN Tag Enable
    #[inline(always)]
    pub fn ven(&mut self) -> VEN_W<MACVTDRrs> {
        VEN_W::new(self, 16)
    }
    ///Bit 17 - 12-bit or 16-bit VLAN comparison
    #[inline(always)]
    pub fn etv(&mut self) -> ETV_W<MACVTDRrs> {
        ETV_W::new(self, 17)
    }
    ///Bit 18 - Disable VLAN Type Comparison
    #[inline(always)]
    pub fn dovltc(&mut self) -> DOVLTC_W<MACVTDRrs> {
        DOVLTC_W::new(self, 18)
    }
    ///Bit 19 - Enable S-VLAN Match for received Frames
    #[inline(always)]
    pub fn ersvlm(&mut self) -> ERSVLM_W<MACVTDRrs> {
        ERSVLM_W::new(self, 19)
    }
    ///Bit 20 - Enable Inner VLAN Tag Comparison
    #[inline(always)]
    pub fn erivlt(&mut self) -> ERIVLT_W<MACVTDRrs> {
        ERIVLT_W::new(self, 20)
    }
    ///Bit 24 - DMA Channel Number Enable
    #[inline(always)]
    pub fn dmachen(&mut self) -> DMACHEN_W<MACVTDRrs> {
        DMACHEN_W::new(self, 24)
    }
    ///Bit 25 - DMA Channel Number
    #[inline(always)]
    pub fn dmachn(&mut self) -> DMACHN_W<MACVTDRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N647.html#ETH:MACVTDR)*/
