...
///Field `OLDIE` reader - OLD0 interrupt enable
pub type OLDIE_R = crate::BitReader;
///Field `OLDIE` writer - OLD0 interrupt enable
...
///Field `SCDIE` reader - SCD0 interrupt enable
pub type SCDIE_R = crate::BitReader;
///Field `SCDIE` writer - SCD0 interrupt enable
...
impl W {
    ...
    ///Bit 2 - Snapshot data ready interrupt enable
    #[inline(always)]
    pub fn ssdrie(&mut self) -> SSDRIE_W<'_, DFLT0IERrs> {
        SSDRIE_W::new(self, 2)
    }
    ///Bit 4 - OLD0 interrupt enable
    #[inline(always)]
    pub fn oldie(&mut self) -> OLDIE_W<'_, DFLT0IERrs> {
        OLDIE_W::new(self, 4)
    }
    ///Bit 7 - Snapshot overrun interrupt enable
    #[inline(always)]
    pub fn ssovrie(&mut self) -> SSOVRIE_W<'_, DFLT0IERrs> {
        SSOVRIE_W::new(self, 7)
    }
    ///Bit 8 - SCD0 interrupt enable
    #[inline(always)]
    pub fn scdie(&mut self) -> SCDIE_W<'_, DFLT0IERrs> {
        SCDIE_W::new(self, 8)
    }
    ///Bit 9 - Saturation detection interrupt enable
    #[inline(always)]
