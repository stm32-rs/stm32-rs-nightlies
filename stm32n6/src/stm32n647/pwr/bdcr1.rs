...
///Field `MONEN` reader - Vless thansub>BATless than/sub> and temperature monitoring enable
pub type MONEN_R = crate::BitReader;
///Field `MONEN` writer - Vless thansub>BATless than/sub> and temperature monitoring enable
pub type MONEN_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `VBATL` reader - Vless thansub>BATless than/sub> level monitoring versus low threshold
pub type VBATL_R = crate::BitReader;
///Field `VBATH` reader - Vless thansub>BATless than/sub> level monitoring versus high threshold
...
impl R {
    ///Bit 0 - Vless thansub>BATless than/sub> and temperature monitoring enable
    #[inline(always)]
    pub fn monen(&self) -> MONEN_R {
        MONEN_R::new((self.bits & 1) != 0)
    }
    ///Bit 16 - Vless thansub>BATless than/sub> level monitoring versus low threshold
    #[inline(always)]
    pub fn vbatl(&self) -> VBATL_R {
        VBATL_R::new(((self.bits >> 16) & 1) != 0)
    }
    ///Bit 17 - Vless thansub>BATless than/sub> level monitoring versus high threshold
    #[inline(always)]
