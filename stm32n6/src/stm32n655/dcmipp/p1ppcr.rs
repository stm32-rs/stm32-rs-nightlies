...
impl W {
    ///Bits 0:3 - Memory format
    #[inline(always)]
    pub fn format(&mut self) -> FORMAT_W<P1PPCRrs> {
        FORMAT_W::new(self, 0)
    }
    ///Bit 4 - Swaps R-vs-B components if RGB, and U-vs-V components if YUV
    #[inline(always)]
    pub fn swaprb(&mut self) -> SWAPRB_W<P1PPCRrs> {
        SWAPRB_W::new(self, 4)
    }
    ///Bits 13:15 - Amount of capture completed lines for LINE Event and Interrupt
    #[inline(always)]
    pub fn linemult(&mut self) -> LINEMULT_W<P1PPCRrs> {
        LINEMULT_W::new(self, 13)
    }
    ///Bit 16 - Double buffer mode
    #[inline(always)]
    pub fn dbm(&mut self) -> DBM_W<P1PPCRrs> {
        DBM_W::new(self, 16)
    }
    ///Bits 17:19 - Line multi address wrapping modulo.
    #[inline(always)]
    pub fn lmawm(&mut self) -> LMAWM_W<P1PPCRrs> {
        LMAWM_W::new(self, 17)
    }
    ///Bit 20 - Line multi address wrapping enable bit.
    #[inline(always)]
    pub fn lmawe(&mut self) -> LMAWE_W<P1PPCRrs> {
        LMAWE_W::new(self, 20)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N655.html#DCMIPP:P1PPCR)*/
pub struct P1PPCRrs;
