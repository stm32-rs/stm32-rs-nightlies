...
impl W {
    ///Bits 0:3 - Memory format (only coplanar formats are supported in Pipe2)
    #[inline(always)]
    pub fn format(&mut self) -> FORMAT_W<P2PPCRrs> {
        FORMAT_W::new(self, 0)
    }
    ///Bit 4 - Swaps R-vs-B components if RGB, and if YUV, swaps U-vs-V components
    #[inline(always)]
    pub fn swaprb(&mut self) -> SWAPRB_W<P2PPCRrs> {
        SWAPRB_W::new(self, 4)
    }
    ///Bits 13:15 - Amount of capture completed lines for LINE event and interrupt
    #[inline(always)]
    pub fn linemult(&mut self) -> LINEMULT_W<P2PPCRrs> {
        LINEMULT_W::new(self, 13)
    }
    ///Bit 16 - Double buffer mode
    #[inline(always)]
    pub fn dbm(&mut self) -> DBM_W<P2PPCRrs> {
        DBM_W::new(self, 16)
    }
    ///Bits 17:19 - Line multi address wrapping modulo
    #[inline(always)]
    pub fn lmawm(&mut self) -> LMAWM_W<P2PPCRrs> {
        LMAWM_W::new(self, 17)
    }
    ///Bit 20 - Line multi address wrapping enable bit
    #[inline(always)]
    pub fn lmawe(&mut self) -> LMAWE_W<P2PPCRrs> {
        LMAWE_W::new(self, 20)
    }
}
