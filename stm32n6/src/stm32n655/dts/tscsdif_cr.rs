...
impl W {
    ///Bits 0:23 - Serial interface write data
    #[inline(always)]
    pub fn sdif_wdata(&mut self) -> SDIF_WDATA_W<TSCSDIF_CRrs> {
        SDIF_WDATA_W::new(self, 0)
    }
    ///Bits 24:26 - Serial interface register address
    #[inline(always)]
    pub fn sdif_addr(&mut self) -> SDIF_ADDR_W<TSCSDIF_CRrs> {
        SDIF_ADDR_W::new(self, 24)
    }
    ///Bit 27 - Serial interface write/no read control bit
    #[inline(always)]
    pub fn sdif_wrn(&mut self) -> SDIF_WRN_W<TSCSDIF_CRrs> {
        SDIF_WRN_W::new(self, 27)
    }
    ///Bit 31 - Serial interface program request
    #[inline(always)]
    pub fn sdif_prog(&mut self) -> SDIF_PROG_W<TSCSDIF_CRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N655.html#DTS:TSCSDIF_CR)*/
