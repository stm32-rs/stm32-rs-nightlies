///Register `DOEPDMA3` reader
pub type R = crate::R<DOEPDMA3rs>;
///Register `DOEPDMA3` writer
pub type W = crate::W<DOEPDMA3rs>;
///Field `DMAADDR` reader - DMA Address
pub type DMAADDR_R = crate::FieldReader<u32>;
///Field `DMAADDR` writer - DMA Address
pub type DMAADDR_W<'a, REG> = crate::FieldWriter<'a, REG, 32, u32>;
impl R {
    ///Bits 0:31 - DMA Address
    #[inline(always)]
    pub fn dmaaddr(&self) -> DMAADDR_R {
        DMAADDR_R::new(self.bits)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("DOEPDMA3")
            .field("dmaaddr", &self.dmaaddr())
            .finish()
    }
}
impl W {
    ///Bits 0:31 - DMA Address
    #[inline(always)]
    pub fn dmaaddr(&mut self) -> DMAADDR_W<'_, DOEPDMA3rs> {
        DMAADDR_W::new(self, 0)
    }
}
/**OTG device OUT endpoint 3 DMA address register

You can [`read`](crate::Reg::read) this register and get [`doepdma3::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`doepdma3::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N655.html#OTG1:DOEPDMA3)*/
pub struct DOEPDMA3rs;
impl crate::RegisterSpec for DOEPDMA3rs {
    type Ux = u32;
}
///`read()` method returns [`doepdma3::R`](R) reader structure
impl crate::Readable for DOEPDMA3rs {}
///`write(|w| ..)` method takes [`doepdma3::W`](W) writer structure
impl crate::Writable for DOEPDMA3rs {
    type Safety = crate::Unsafe;
}
///`reset()` method sets DOEPDMA3 to value 0
impl crate::Resettable for DOEPDMA3rs {}
