...
///Field `CLIMITF` writer - limit interrupt status clear
...
impl W {
    ...
    #[inline(always)]
    pub fn clinef(&mut self) -> CLINEF_W<P0FCRrs> {
        CLINEF_W::new(self, 0)
    }
    ///Bit 1 - Frame capture complete interrupt status clear
    #[inline(always)]
    pub fn cframef(&mut self) -> CFRAMEF_W<P0FCRrs> {
        CFRAMEF_W::new(self, 1)
    }
    ///Bit 2 - Vertical synchronization interrupt status clear
    #[inline(always)]
    pub fn cvsyncf(&mut self) -> CVSYNCF_W<P0FCRrs> {
        CVSYNCF_W::new(self, 2)
    }
    ///Bit 6 - limit interrupt status clear
    #[inline(always)]
    pub fn climitf(&mut self) -> CLIMITF_W<P0FCRrs> {
        CLIMITF_W::new(self, 6)
    }
    ///Bit 7 - Overrun interrupt status clear
    #[inline(always)]
    pub fn covrf(&mut self) -> COVRF_W<P0FCRrs> {
        COVRF_W::new(self, 7)
    }
}
