...
impl W {
    ...
    ///Bit 28 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em60(&mut self) -> EM60_W<EMR2rs> {
        EM60_W::new(self, 28)
    }
    ///Bit 29 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em61(&mut self) -> EM61_W<EMR2rs> {
        EM61_W::new(self, 29)
    }
    ///Bit 30 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em62(&mut self) -> EM62_W<EMR2rs> {
        EM62_W::new(self, 30)
    }
    ///Bit 31 - CPU wake-up with event on event input x
    #[inline(always)]
    pub fn em63(&mut self) -> EM63_W<EMR2rs> {
        EM63_W::new(self, 31)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N657.html#EXTI:EMR2)*/
pub struct EMR2rs;
