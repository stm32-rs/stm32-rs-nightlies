...
impl W {
    ///Bits 0:7 - GPDMA hardware request selection
    #[inline(always)]
    pub fn reqsel(&mut self) -> REQSEL_W<'_, C0TR2rs> {
        REQSEL_W::new(self, 0)
    }
    ///Bit 9 - software request
    #[inline(always)]
    pub fn swreq(&mut self) -> SWREQ_W<'_, C0TR2rs> {
        SWREQ_W::new(self, 9)
    }
    ///Bit 10 - destination hardware request
    #[inline(always)]
    pub fn dreq(&mut self) -> DREQ_W<'_, C0TR2rs> {
        DREQ_W::new(self, 10)
    }
    ///Bit 11 - Block hardware request
    #[inline(always)]
    pub fn breq(&mut self) -> BREQ_W<'_, C0TR2rs> {
        BREQ_W::new(self, 11)
    }
    ///Bit 12 - Hardware request in peripheral flow control mode
    #[inline(always)]
    pub fn pfreq(&mut self) -> PFREQ_W<'_, C0TR2rs> {
        PFREQ_W::new(self, 12)
    }
    ///Bits 14:15 - trigger mode
    #[inline(always)]
    pub fn trigm(&mut self) -> TRIGM_W<'_, C0TR2rs> {
        TRIGM_W::new(self, 14)
    }
    ///Bits 16:22 - trigger event input selection
    #[inline(always)]
    pub fn trigsel(&mut self) -> TRIGSEL_W<'_, C0TR2rs> {
        TRIGSEL_W::new(self, 16)
    }
    ///Bits 24:25 - trigger event polarity
    #[inline(always)]
    pub fn trigpol(&mut self) -> TRIGPOL_W<'_, C0TR2rs> {
        TRIGPOL_W::new(self, 24)
    }
    ///Bits 30:31 - transfer complete event mode
    #[inline(always)]
    pub fn tcem(&mut self) -> TCEM_W<'_, C0TR2rs> {
        TCEM_W::new(self, 30)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N657.html#GPDMA:C0TR2)*/
pub struct C0TR2rs;
