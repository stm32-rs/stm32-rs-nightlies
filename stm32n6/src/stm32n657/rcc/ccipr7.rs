...
impl W {
    ///Bits 0:2 - Source selection for the PER kernel clock
    #[inline(always)]
    pub fn persel(&mut self) -> PERSEL_W<CCIPR7rs> {
        PERSEL_W::new(self, 0)
    }
    ///Bits 4:5 - Source selection for the PSSI kernel clock
    #[inline(always)]
    pub fn pssisel(&mut self) -> PSSISEL_W<CCIPR7rs> {
        PSSISEL_W::new(self, 4)
    }
    ///Bits 8:9 - Source selection for the RTC kernel clock
    #[inline(always)]
    pub fn rtcsel(&mut self) -> RTCSEL_W<CCIPR7rs> {
        RTCSEL_W::new(self, 8)
    }
    ///Bits 12:17 - RTC Prog clock divider selection (for clock ck_icn_p_risaf)
    #[inline(always)]
    pub fn rtcpre(&mut self) -> RTCPRE_W<CCIPR7rs> {
        RTCPRE_W::new(self, 12)
    }
    ///Bits 20:22 - Source selection for the SAI1 kernel clock
    #[inline(always)]
    pub fn sai1sel(&mut self) -> SAI1SEL_W<CCIPR7rs> {
        SAI1SEL_W::new(self, 20)
    }
    ///Bits 24:26 - Source selection for the SAI2 kernel clock
    #[inline(always)]
    pub fn sai2sel(&mut self) -> SAI2SEL_W<CCIPR7rs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32N657.html#RCC:CCIPR7)*/
pub struct CCIPR7rs;
