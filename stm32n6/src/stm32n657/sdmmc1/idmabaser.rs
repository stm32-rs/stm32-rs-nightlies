...
///Field `IDMABASE` reader - Buffer memory base address bits \[31:2\], must be word aligned (bit \[1:0\] are always 0 and read only)
pub type IDMABASE_R = crate::FieldReader<u32>;
///Field `IDMABASE` writer - Buffer memory base address bits \[31:2\], must be word aligned (bit \[1:0\] are always 0 and read only)
