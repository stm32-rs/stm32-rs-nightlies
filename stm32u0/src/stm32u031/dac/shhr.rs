...
///Field `THOLD(1-1)` reader - DAC channel%s hold time (only valid in Sample and hold mode)
pub type THOLD_R = crate::FieldReader<u16>;
///Field `THOLD(1-1)` writer - DAC channel%s hold time (only valid in Sample and hold mode)
pub type THOLD_W<'a, REG> = crate::FieldWriter<'a, REG, 10, u16, crate::Safe>;
impl R {
    ///DAC channel(1-1) hold time (only valid in Sample and hold mode)
    ///
    ...
    pub fn thold(&self, n: u8) -> THOLD_R {
        ...
        [(); 1][n as usize];
        THOLD_R::new(((self.bits >> (n * 0)) & 0x03ff) as u16)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) hold time (only valid in Sample and hold mode)
    #[inline(always)]
    pub fn thold_iter(&self) -> impl Iterator<Item = THOLD_R> + '_ {
        (0..1).map(move |n| THOLD_R::new(((self.bits >> (n * 0)) & 0x03ff) as u16))
    }
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U031.html#DAC:SHHR)*/
pub struct SHHRrs;
