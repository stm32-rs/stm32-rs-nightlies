...
///Field `SS` writer - Subseconds value This value is compared with the contents of the synchronous prescaler counter to determine if alarm A is to be activated. Only bits 0 up MASKSS-1 are compared. This field is the mirror of SS\[14:0\] in the RTC_ALRABINR, and so can also be read or written through RTC_ALRABINR.
...
impl R {
    ///Bits 0:14 - Subseconds value This value is compared with the contents of the synchronous prescaler counter to determine if alarm A is to be activated. Only bits 0 up MASKSS-1 are compared. This field is the mirror of SS\[14:0\] in the RTC_ALRABINR, and so can also be read or written through RTC_ALRABINR.
    #[inline(always)]
    pub fn ss(&self) -> SS_R {
