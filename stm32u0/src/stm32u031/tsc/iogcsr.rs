...
///Field `GE(1-7)` reader - Analog I/O group x enable These bits are set and cleared by software to enable/disable the acquisition (counter is counting) on the corresponding analog I/O group x.
...
///Field `GE(1-7)` writer - Analog I/O group x enable These bits are set and cleared by software to enable/disable the acquisition (counter is counting) on the corresponding analog I/O group x.
...
///Field `GS(1-7)` reader - Analog I/O group x status These bits are set by hardware when the acquisition on the corresponding enabled analog I/O group x is complete. They are cleared by hardware when a new acquisition is started. Note: When a max count error is detected the remaining GxS bits of the enabled analog I/O groups are not set.
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U031.html#TSC:IOGCSR)*/
pub struct IOGCSRrs;
