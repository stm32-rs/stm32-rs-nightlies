...
///Field `SEGS` reader - Segment states, one bit per segment, LSB: S00, MSB: S47
pub type SEGS_R = crate::FieldReader<u64>;
///Field `SEGS` writer - Segment states, one bit per segment, LSB: S00, MSB: S47
pub type SEGS_W<'a, REG> = crate::FieldWriter<'a, REG, 48, u64>;
impl R {
    ///Bits 0:47 - Segment states, one bit per segment, LSB: S00, MSB: S47
    #[inline(always)]
    pub fn segs(&self) -> SEGS_R {
        SEGS_R::new(self.bits & 0xffff_ffff_ffff)
    }
}
...
impl W {
    ///Bits 0:47 - Segment states, one bit per segment, LSB: S00, MSB: S47
    #[inline(always)]
    pub fn segs(&mut self) -> SEGS_W<RAM_COM0rs> {
        SEGS_W::new(self, 0)
    }
}
/**LCD display memory
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U073.html#LCD:RAM_COM0)*/
