///Register `LPUART_CR3` reader
pub type R = crate::R<LPUART_CR3rs>;
///Register `LPUART_CR3` writer
pub type W = crate::W<LPUART_CR3rs>;
///Field `EIE` reader - Error interrupt enable Error Interrupt Enable Bit is required to enable interrupt generation in case of a framing error, overrun error or noise flag (FE=1 or ORE=1 or NE=1 in the LPUART_ISR register).
pub type EIE_R = crate::BitReader;
///Field `EIE` writer - Error interrupt enable Error Interrupt Enable Bit is required to enable interrupt generation in case of a framing error, overrun error or noise flag (FE=1 or ORE=1 or NE=1 in the LPUART_ISR register).
pub type EIE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `HDSEL` reader - Half-duplex selection Selection of Single-wire Half-duplex mode This bit can only be written when the LPUART is disabled (UE=0).
pub type HDSEL_R = crate::BitReader;
///Field `HDSEL` writer - Half-duplex selection Selection of Single-wire Half-duplex mode This bit can only be written when the LPUART is disabled (UE=0).
pub type HDSEL_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `DMAR` reader - DMA enable receiver This bit is set/reset by software
pub type DMAR_R = crate::BitReader;
///Field `DMAR` writer - DMA enable receiver This bit is set/reset by software
pub type DMAR_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `DMAT` reader - DMA enable transmitter This bit is set/reset by software
pub type DMAT_R = crate::BitReader;
///Field `DMAT` writer - DMA enable transmitter This bit is set/reset by software
pub type DMAT_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `RTSE` reader - RTS enable This bit can only be written when the LPUART is disabled (UE=0).
pub type RTSE_R = crate::BitReader;
///Field `RTSE` writer - RTS enable This bit can only be written when the LPUART is disabled (UE=0).
pub type RTSE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CTSE` reader - CTS enable This bit can only be written when the LPUART is disabled (UE=0)
pub type CTSE_R = crate::BitReader;
///Field `CTSE` writer - CTS enable This bit can only be written when the LPUART is disabled (UE=0)
pub type CTSE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CTSIE` reader - CTS interrupt enable
pub type CTSIE_R = crate::BitReader;
///Field `CTSIE` writer - CTS interrupt enable
pub type CTSIE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `OVRDIS` reader - Overrun Disable This bit is used to disable the receive overrun detection. the ORE flag is not set and the new received data overwrites the previous content of the LPUART_RDR register. This bit can only be written when the LPUART is disabled (UE=0). Note: This control bit enables checking the communication flow w/o reading the data.
pub type OVRDIS_R = crate::BitReader;
///Field `OVRDIS` writer - Overrun Disable This bit is used to disable the receive overrun detection. the ORE flag is not set and the new received data overwrites the previous content of the LPUART_RDR register. This bit can only be written when the LPUART is disabled (UE=0). Note: This control bit enables checking the communication flow w/o reading the data.
pub type OVRDIS_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `DDRE` reader - DMA Disable on Reception Error This bit can only be written when the LPUART is disabled (UE=0). Note: The reception errors are: parity error, framing error or noise error.
pub type DDRE_R = crate::BitReader;
///Field `DDRE` writer - DMA Disable on Reception Error This bit can only be written when the LPUART is disabled (UE=0). Note: The reception errors are: parity error, framing error or noise error.
pub type DDRE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `DEM` reader - Driver enable mode This bit enables the user to activate the external transceiver control, through the DE signal. This bit can only be written when the LPUART is disabled (UE=0).
pub type DEM_R = crate::BitReader;
///Field `DEM` writer - Driver enable mode This bit enables the user to activate the external transceiver control, through the DE signal. This bit can only be written when the LPUART is disabled (UE=0).
pub type DEM_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `DEP` reader - Driver enable polarity selection This bit can only be written when the LPUART is disabled (UE=0).
pub type DEP_R = crate::BitReader;
///Field `DEP` writer - Driver enable polarity selection This bit can only be written when the LPUART is disabled (UE=0).
pub type DEP_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `WUS0` reader - Wake-up from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wake-up from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
pub type WUS0_R = crate::BitReader;
///Field `WUS0` writer - Wake-up from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wake-up from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
pub type WUS0_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `WUS1` reader - Wake-up from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wake-up from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
pub type WUS1_R = crate::BitReader;
///Field `WUS1` writer - Wake-up from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wake-up from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
pub type WUS1_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `WUFIE` reader - Wake-up from low-power mode interrupt enable This bit is set and cleared by software. Note: WUFIE must be set before entering in low-power mode. Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
pub type WUFIE_R = crate::BitReader;
///Field `WUFIE` writer - Wake-up from low-power mode interrupt enable This bit is set and cleared by software. Note: WUFIE must be set before entering in low-power mode. Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
pub type WUFIE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TXFTIE` reader - TXFIFO threshold interrupt enable This bit is set and cleared by software.
pub type TXFTIE_R = crate::BitReader;
///Field `TXFTIE` writer - TXFIFO threshold interrupt enable This bit is set and cleared by software.
pub type TXFTIE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `RXFTCFG` reader - Receive FIFO threshold configuration Remaining combinations: Reserved.
pub type RXFTCFG_R = crate::FieldReader;
///Field `RXFTCFG` writer - Receive FIFO threshold configuration Remaining combinations: Reserved.
pub type RXFTCFG_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
///Field `RXFTIE` reader - RXFIFO threshold interrupt enable This bit is set and cleared by software.
pub type RXFTIE_R = crate::BitReader;
///Field `RXFTIE` writer - RXFIFO threshold interrupt enable This bit is set and cleared by software.
pub type RXFTIE_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `TXFTCFG` reader - TXFIFO threshold configuration Remaining combinations: Reserved.
pub type TXFTCFG_R = crate::FieldReader;
///Field `TXFTCFG` writer - TXFIFO threshold configuration Remaining combinations: Reserved.
pub type TXFTCFG_W<'a, REG> = crate::FieldWriter<'a, REG, 3>;
impl R {
    ///Bit 0 - Error interrupt enable Error Interrupt Enable Bit is required to enable interrupt generation in case of a framing error, overrun error or noise flag (FE=1 or ORE=1 or NE=1 in the LPUART_ISR register).
    #[inline(always)]
    pub fn eie(&self) -> EIE_R {
        EIE_R::new((self.bits & 1) != 0)
    }
    ///Bit 3 - Half-duplex selection Selection of Single-wire Half-duplex mode This bit can only be written when the LPUART is disabled (UE=0).
    #[inline(always)]
    pub fn hdsel(&self) -> HDSEL_R {
        HDSEL_R::new(((self.bits >> 3) & 1) != 0)
    }
    ///Bit 6 - DMA enable receiver This bit is set/reset by software
    #[inline(always)]
    pub fn dmar(&self) -> DMAR_R {
        DMAR_R::new(((self.bits >> 6) & 1) != 0)
    }
    ///Bit 7 - DMA enable transmitter This bit is set/reset by software
    #[inline(always)]
    pub fn dmat(&self) -> DMAT_R {
        DMAT_R::new(((self.bits >> 7) & 1) != 0)
    }
    ///Bit 8 - RTS enable This bit can only be written when the LPUART is disabled (UE=0).
    #[inline(always)]
    pub fn rtse(&self) -> RTSE_R {
        RTSE_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9 - CTS enable This bit can only be written when the LPUART is disabled (UE=0)
    #[inline(always)]
    pub fn ctse(&self) -> CTSE_R {
        CTSE_R::new(((self.bits >> 9) & 1) != 0)
    }
    ///Bit 10 - CTS interrupt enable
    #[inline(always)]
    pub fn ctsie(&self) -> CTSIE_R {
        CTSIE_R::new(((self.bits >> 10) & 1) != 0)
    }
    ///Bit 12 - Overrun Disable This bit is used to disable the receive overrun detection. the ORE flag is not set and the new received data overwrites the previous content of the LPUART_RDR register. This bit can only be written when the LPUART is disabled (UE=0). Note: This control bit enables checking the communication flow w/o reading the data.
    #[inline(always)]
    pub fn ovrdis(&self) -> OVRDIS_R {
        OVRDIS_R::new(((self.bits >> 12) & 1) != 0)
    }
    ///Bit 13 - DMA Disable on Reception Error This bit can only be written when the LPUART is disabled (UE=0). Note: The reception errors are: parity error, framing error or noise error.
    #[inline(always)]
    pub fn ddre(&self) -> DDRE_R {
        DDRE_R::new(((self.bits >> 13) & 1) != 0)
    }
    ///Bit 14 - Driver enable mode This bit enables the user to activate the external transceiver control, through the DE signal. This bit can only be written when the LPUART is disabled (UE=0).
    #[inline(always)]
    pub fn dem(&self) -> DEM_R {
        DEM_R::new(((self.bits >> 14) & 1) != 0)
    }
    ///Bit 15 - Driver enable polarity selection This bit can only be written when the LPUART is disabled (UE=0).
    #[inline(always)]
    pub fn dep(&self) -> DEP_R {
        DEP_R::new(((self.bits >> 15) & 1) != 0)
    }
    ///Bit 20 - Wake-up from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wake-up from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
    #[inline(always)]
    pub fn wus0(&self) -> WUS0_R {
        WUS0_R::new(((self.bits >> 20) & 1) != 0)
    }
    ///Bit 21 - Wake-up from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wake-up from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
    #[inline(always)]
    pub fn wus1(&self) -> WUS1_R {
        WUS1_R::new(((self.bits >> 21) & 1) != 0)
    }
    ///Bit 22 - Wake-up from low-power mode interrupt enable This bit is set and cleared by software. Note: WUFIE must be set before entering in low-power mode. Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
    #[inline(always)]
    pub fn wufie(&self) -> WUFIE_R {
        WUFIE_R::new(((self.bits >> 22) & 1) != 0)
    }
    ///Bit 23 - TXFIFO threshold interrupt enable This bit is set and cleared by software.
    #[inline(always)]
    pub fn txftie(&self) -> TXFTIE_R {
        TXFTIE_R::new(((self.bits >> 23) & 1) != 0)
    }
    ///Bits 25:27 - Receive FIFO threshold configuration Remaining combinations: Reserved.
    #[inline(always)]
    pub fn rxftcfg(&self) -> RXFTCFG_R {
        RXFTCFG_R::new(((self.bits >> 25) & 7) as u8)
    }
    ///Bit 28 - RXFIFO threshold interrupt enable This bit is set and cleared by software.
    #[inline(always)]
    pub fn rxftie(&self) -> RXFTIE_R {
        RXFTIE_R::new(((self.bits >> 28) & 1) != 0)
    }
    ///Bits 29:31 - TXFIFO threshold configuration Remaining combinations: Reserved.
    #[inline(always)]
    pub fn txftcfg(&self) -> TXFTCFG_R {
        TXFTCFG_R::new(((self.bits >> 29) & 7) as u8)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("LPUART_CR3")
            .field("eie", &self.eie())
            .field("hdsel", &self.hdsel())
            .field("dmar", &self.dmar())
            .field("dmat", &self.dmat())
            .field("rtse", &self.rtse())
            .field("ctse", &self.ctse())
            .field("ctsie", &self.ctsie())
            .field("ovrdis", &self.ovrdis())
            .field("ddre", &self.ddre())
            .field("dem", &self.dem())
            .field("dep", &self.dep())
            .field("wus0", &self.wus0())
            .field("wus1", &self.wus1())
            .field("wufie", &self.wufie())
            .field("txftie", &self.txftie())
            .field("rxftcfg", &self.rxftcfg())
            .field("rxftie", &self.rxftie())
            .field("txftcfg", &self.txftcfg())
            .finish()
    }
}
impl W {
    ///Bit 0 - Error interrupt enable Error Interrupt Enable Bit is required to enable interrupt generation in case of a framing error, overrun error or noise flag (FE=1 or ORE=1 or NE=1 in the LPUART_ISR register).
    #[inline(always)]
    pub fn eie(&mut self) -> EIE_W<LPUART_CR3rs> {
        EIE_W::new(self, 0)
    }
    ///Bit 3 - Half-duplex selection Selection of Single-wire Half-duplex mode This bit can only be written when the LPUART is disabled (UE=0).
    #[inline(always)]
    pub fn hdsel(&mut self) -> HDSEL_W<LPUART_CR3rs> {
        HDSEL_W::new(self, 3)
    }
    ///Bit 6 - DMA enable receiver This bit is set/reset by software
    #[inline(always)]
    pub fn dmar(&mut self) -> DMAR_W<LPUART_CR3rs> {
        DMAR_W::new(self, 6)
    }
    ///Bit 7 - DMA enable transmitter This bit is set/reset by software
    #[inline(always)]
    pub fn dmat(&mut self) -> DMAT_W<LPUART_CR3rs> {
        DMAT_W::new(self, 7)
    }
    ///Bit 8 - RTS enable This bit can only be written when the LPUART is disabled (UE=0).
    #[inline(always)]
    pub fn rtse(&mut self) -> RTSE_W<LPUART_CR3rs> {
        RTSE_W::new(self, 8)
    }
    ///Bit 9 - CTS enable This bit can only be written when the LPUART is disabled (UE=0)
    #[inline(always)]
    pub fn ctse(&mut self) -> CTSE_W<LPUART_CR3rs> {
        CTSE_W::new(self, 9)
    }
    ///Bit 10 - CTS interrupt enable
    #[inline(always)]
    pub fn ctsie(&mut self) -> CTSIE_W<LPUART_CR3rs> {
        CTSIE_W::new(self, 10)
    }
    ///Bit 12 - Overrun Disable This bit is used to disable the receive overrun detection. the ORE flag is not set and the new received data overwrites the previous content of the LPUART_RDR register. This bit can only be written when the LPUART is disabled (UE=0). Note: This control bit enables checking the communication flow w/o reading the data.
    #[inline(always)]
    pub fn ovrdis(&mut self) -> OVRDIS_W<LPUART_CR3rs> {
        OVRDIS_W::new(self, 12)
    }
    ///Bit 13 - DMA Disable on Reception Error This bit can only be written when the LPUART is disabled (UE=0). Note: The reception errors are: parity error, framing error or noise error.
    #[inline(always)]
    pub fn ddre(&mut self) -> DDRE_W<LPUART_CR3rs> {
        DDRE_W::new(self, 13)
    }
    ///Bit 14 - Driver enable mode This bit enables the user to activate the external transceiver control, through the DE signal. This bit can only be written when the LPUART is disabled (UE=0).
    #[inline(always)]
    pub fn dem(&mut self) -> DEM_W<LPUART_CR3rs> {
        DEM_W::new(self, 14)
    }
    ///Bit 15 - Driver enable polarity selection This bit can only be written when the LPUART is disabled (UE=0).
    #[inline(always)]
    pub fn dep(&mut self) -> DEP_W<LPUART_CR3rs> {
        DEP_W::new(self, 15)
    }
    ///Bit 20 - Wake-up from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wake-up from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
    #[inline(always)]
    pub fn wus0(&mut self) -> WUS0_W<LPUART_CR3rs> {
        WUS0_W::new(self, 20)
    }
    ///Bit 21 - Wake-up from low-power mode interrupt flag selection This bitfield specifies the event which activates the WUF (Wake-up from low-power mode flag). This bitfield can only be written when the LPUART is disabled (UE=0). Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
    #[inline(always)]
    pub fn wus1(&mut self) -> WUS1_W<LPUART_CR3rs> {
        WUS1_W::new(self, 21)
    }
    ///Bit 22 - Wake-up from low-power mode interrupt enable This bit is set and cleared by software. Note: WUFIE must be set before entering in low-power mode. Note: If the USART does not support the wake-up from Stop feature, this bit is reserved and must be kept at reset value. Refer to Section132.3: LPUART implementation on page1914.
    #[inline(always)]
    pub fn wufie(&mut self) -> WUFIE_W<LPUART_CR3rs> {
        WUFIE_W::new(self, 22)
    }
    ///Bit 23 - TXFIFO threshold interrupt enable This bit is set and cleared by software.
    #[inline(always)]
    pub fn txftie(&mut self) -> TXFTIE_W<LPUART_CR3rs> {
        TXFTIE_W::new(self, 23)
    }
    ///Bits 25:27 - Receive FIFO threshold configuration Remaining combinations: Reserved.
    #[inline(always)]
    pub fn rxftcfg(&mut self) -> RXFTCFG_W<LPUART_CR3rs> {
        RXFTCFG_W::new(self, 25)
    }
    ///Bit 28 - RXFIFO threshold interrupt enable This bit is set and cleared by software.
    #[inline(always)]
    pub fn rxftie(&mut self) -> RXFTIE_W<LPUART_CR3rs> {
        RXFTIE_W::new(self, 28)
    }
    ///Bits 29:31 - TXFIFO threshold configuration Remaining combinations: Reserved.
    #[inline(always)]
    pub fn txftcfg(&mut self) -> TXFTCFG_W<LPUART_CR3rs> {
        TXFTCFG_W::new(self, 29)
    }
}
/**LPUART control register 3

You can [`read`](crate::Reg::read) this register and get [`lpuart_cr3::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`lpuart_cr3::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U083.html#LPUART1:LPUART_CR3)*/
pub struct LPUART_CR3rs;
impl crate::RegisterSpec for LPUART_CR3rs {
    type Ux = u32;
}
///`read()` method returns [`lpuart_cr3::R`](R) reader structure
impl crate::Readable for LPUART_CR3rs {}
///`write(|w| ..)` method takes [`lpuart_cr3::W`](W) writer structure
impl crate::Writable for LPUART_CR3rs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets LPUART_CR3 to value 0
impl crate::Resettable for LPUART_CR3rs {
    const RESET_VALUE: u32 = 0;
}
