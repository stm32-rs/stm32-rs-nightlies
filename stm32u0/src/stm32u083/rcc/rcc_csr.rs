...
/**Field `MSISRANGE` reader - MSI range after Standby mode Set by software to chose the MSI frequency at startup. This range is used after exiting Standby mode until MSIRGSEL is set. After a pad or a power-on reset, the range is always 41MHz. MSISRANGE\[3:0\]
...
/**Field `MSISRANGE` writer - MSI range after Standby mode Set by software to chose the MSI frequency at startup. This range is used after exiting Standby mode until MSIRGSEL is set. After a pad or a power-on reset, the range is always 41MHz. MSISRANGE\[3:0\]
can be written only when MSIRGSEL1=11. Others: Reserved Note: Changing the MSISRANGE\[3:0\]
does not change the current MSI frequency.*/
...
impl R {
    ...
    /**Bits 8:11 - MSI range after Standby mode Set by software to chose the MSI frequency at startup. This range is used after exiting Standby mode until MSIRGSEL is set. After a pad or a power-on reset, the range is always 41MHz. MSISRANGE\[3:0\]
    can be written only when MSIRGSEL1=11. Others: Reserved Note: Changing the MSISRANGE\[3:0\]
    does not change the current MSI frequency.*/
    #[inline(always)]
...
impl W {
    ///Bit 0 - LSI oscillator enable Set and cleared by software to enable/disable the LSI oscillator:
    #[inline(always)]
    pub fn lsion(&mut self) -> LSION_W<RCC_CSRrs> {
        LSION_W::new(self, 0)
    }
    ///Bit 2 - Internal low-speed oscillator pre-divided by 128 Set and reset by hardware to indicate when the low-speed internal RC oscillator has to be divided by 128. The software has to switch off the LSI before changing this bit.
    #[inline(always)]
    pub fn lsiprediv(&mut self) -> LSIPREDIV_W<RCC_CSRrs> {
    ...
    does not change the current MSI frequency.*/
    #[inline(always)]
    pub fn msisrange(&mut self) -> MSISRANGE_W<RCC_CSRrs> {
...
///`reset()` method sets RCC_CSR to value 0
