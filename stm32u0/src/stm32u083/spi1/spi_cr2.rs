...
///Field `SSOE` reader - SS output enable Note: This bit is not used in SPI TI mode.
pub type SSOE_R = crate::BitReader;
///Field `SSOE` writer - SS output enable Note: This bit is not used in SPI TI mode.
...
///Field `FRF` reader - Frame format 1 SPI TI mode Note: This bit must be written only when the SPI is disabled (SPE=0).
pub type FRF_R = crate::BitReader;
///Field `FRF` writer - Frame format 1 SPI TI mode Note: This bit must be written only when the SPI is disabled (SPE=0).
pub type FRF_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `ERRIE` reader - Error interrupt enable This bit controls the generation of an interrupt when an error condition occurs (CRCERR, OVR, MODF in SPI mode, FRE at TI mode).
pub type ERRIE_R = crate::BitReader;
///Field `ERRIE` writer - Error interrupt enable This bit controls the generation of an interrupt when an error condition occurs (CRCERR, OVR, MODF in SPI mode, FRE at TI mode).
...
///Field `DS` reader - Data size These bits configure the data length for SPI transfers. If software attempts to write one of the Not used values, they are forced to the value 0111 (8-bit)
pub type DS_R = crate::FieldReader;
///Field `DS` writer - Data size These bits configure the data length for SPI transfers. If software attempts to write one of the Not used values, they are forced to the value 0111 (8-bit)
pub type DS_W<'a, REG> = crate::FieldWriter<'a, REG, 4>;
///Field `FRXTH` reader - FIFO reception threshold This bit is used to set the threshold of the RXFIFO that triggers an RXNE event
pub type FRXTH_R = crate::BitReader;
///Field `FRXTH` writer - FIFO reception threshold This bit is used to set the threshold of the RXFIFO that triggers an RXNE event
...
impl R {
    ...
    ///Bit 2 - SS output enable Note: This bit is not used in SPI TI mode.
    #[inline(always)]
    ...
    ///Bit 4 - Frame format 1 SPI TI mode Note: This bit must be written only when the SPI is disabled (SPE=0).
    #[inline(always)]
    pub fn frf(&self) -> FRF_R {
        FRF_R::new(((self.bits >> 4) & 1) != 0)
    }
    ///Bit 5 - Error interrupt enable This bit controls the generation of an interrupt when an error condition occurs (CRCERR, OVR, MODF in SPI mode, FRE at TI mode).
    #[inline(always)]
    ...
    ///Bits 8:11 - Data size These bits configure the data length for SPI transfers. If software attempts to write one of the Not used values, they are forced to the value 0111 (8-bit)
    #[inline(always)]
    pub fn ds(&self) -> DS_R {
        DS_R::new(((self.bits >> 8) & 0x0f) as u8)
    }
    ///Bit 12 - FIFO reception threshold This bit is used to set the threshold of the RXFIFO that triggers an RXNE event
    #[inline(always)]
...
impl W {
    ///Bit 0 - Rx buffer DMA enable When this bit is set, a DMA request is generated whenever the RXNE flag is set.
    #[inline(always)]
    pub fn rxdmaen(&mut self) -> RXDMAEN_W<SPI_CR2rs> {
        RXDMAEN_W::new(self, 0)
    }
    ///Bit 1 - Tx buffer DMA enable When this bit is set, a DMA request is generated whenever the TXE flag is set.
    #[inline(always)]
    pub fn txdmaen(&mut self) -> TXDMAEN_W<SPI_CR2rs> {
        TXDMAEN_W::new(self, 1)
    }
    ///Bit 2 - SS output enable Note: This bit is not used in SPI TI mode.
    #[inline(always)]
    pub fn ssoe(&mut self) -> SSOE_W<SPI_CR2rs> {
        SSOE_W::new(self, 2)
    }
    ///Bit 3 - NSS pulse management This bit is used in master mode only. it allows the SPI to generate an NSS pulse between two consecutive data when doing continuous transfers. In the case of a single data transfer, it forces the NSS pin high level after the transfer. It has no meaning if CPHA = 1, or FRF = 1. Note: 1. This bit must be written only when the SPI is disabled (SPE=0). Note: 2. This bit is not used in SPI TI mode.
    #[inline(always)]
    pub fn nssp(&mut self) -> NSSP_W<SPI_CR2rs> {
        NSSP_W::new(self, 3)
    }
    ///Bit 4 - Frame format 1 SPI TI mode Note: This bit must be written only when the SPI is disabled (SPE=0).
    #[inline(always)]
    pub fn frf(&mut self) -> FRF_W<SPI_CR2rs> {
        FRF_W::new(self, 4)
    }
    ///Bit 5 - Error interrupt enable This bit controls the generation of an interrupt when an error condition occurs (CRCERR, OVR, MODF in SPI mode, FRE at TI mode).
    #[inline(always)]
    pub fn errie(&mut self) -> ERRIE_W<SPI_CR2rs> {
        ERRIE_W::new(self, 5)
    }
    ///Bit 6 - RX buffer not empty interrupt enable
    #[inline(always)]
    pub fn rxneie(&mut self) -> RXNEIE_W<SPI_CR2rs> {
        RXNEIE_W::new(self, 6)
    }
    ///Bit 7 - Tx buffer empty interrupt enable
    #[inline(always)]
    pub fn txeie(&mut self) -> TXEIE_W<SPI_CR2rs> {
        TXEIE_W::new(self, 7)
    }
    ///Bits 8:11 - Data size These bits configure the data length for SPI transfers. If software attempts to write one of the Not used values, they are forced to the value 0111 (8-bit)
    #[inline(always)]
    pub fn ds(&mut self) -> DS_W<SPI_CR2rs> {
        DS_W::new(self, 8)
    }
    ///Bit 12 - FIFO reception threshold This bit is used to set the threshold of the RXFIFO that triggers an RXNE event
    #[inline(always)]
    pub fn frxth(&mut self) -> FRXTH_W<SPI_CR2rs> {
        FRXTH_W::new(self, 12)
    }
    ///Bit 13 - Last DMA transfer for reception This bit is used in data packing mode, to define if the total number of data to receive by DMA is odd or even. It has significance only if the RXDMAEN bit in the SPI_CR2 register is set and if packing mode is used (data length =< 8-bit and write access to SPI_DR is 16-bit wide). It has to be written when the SPI is disabled (SPE = 0 in the SPI_CR1 register). Note: Refer to Procedure for disabling the SPI on page1954 if the CRCEN bit is set.
    #[inline(always)]
    pub fn ldma_rx(&mut self) -> LDMA_RX_W<SPI_CR2rs> {
        LDMA_RX_W::new(self, 13)
    }
    ///Bit 14 - Last DMA transfer for transmission This bit is used in data packing mode, to define if the total number of data to transmit by DMA is odd or even. It has significance only if the TXDMAEN bit in the SPI_CR2 register is set and if packing mode is used (data length =< 8-bit and write access to SPI_DR is 16-bit wide). It has to be written when the SPI is disabled (SPE = 0 in the SPI_CR1 register). Note: Refer to Procedure for disabling the SPI on page1954 if the CRCEN bit is set.
    #[inline(always)]
    pub fn ldma_tx(&mut self) -> LDMA_TX_W<SPI_CR2rs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U083.html#SPI1:SPI_CR2)*/
