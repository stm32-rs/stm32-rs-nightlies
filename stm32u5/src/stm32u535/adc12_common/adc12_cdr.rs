///Register `ADC12_CDR` reader
pub type R = crate::R<ADC12_CDRrs>;
/**Field `RDATA_MST` reader - Regular data of the master ADC. In dual mode, these bits contain the regular data of the master ADC. Refer to . The data alignment is applied as described in offset (ADC_DR, ADC_JDRy, OFFSETy, OFFSETy_CH, OVSS, LSHIFT, USAT, SSAT)) In DAMDF\[1:0\]
...
impl R {
    /**Bits 0:15 - Regular data of the master ADC. In dual mode, these bits contain the regular data of the master ADC. Refer to . The data alignment is applied as described in offset (ADC_DR, ADC_JDRy, OFFSETy, OFFSETy_CH, OVSS, LSHIFT, USAT, SSAT)) In DAMDF\[1:0\]
    = 11 mode, bits 15:8 contains SLV_ADC_DR\[7:0\], bits 7:0 contains MST_ADC_DR\[7:0\].*/
    #[inline(always)]
...
You can [`read`](crate::Reg::read) this register and get [`adc12_cdr::R`](R). See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U535.html#ADC12_Common:ADC12_CDR)*/
pub struct ADC12_CDRrs;
impl crate::RegisterSpec for ADC12_CDRrs {
    type Ux = u32;
}
///`read()` method returns [`adc12_cdr::R`](R) reader structure
impl crate::Readable for ADC12_CDRrs {}
///`reset()` method sets ADC12_CDR to value 0
