///Register `DAC_SHSR1` reader
pub type R = crate::R<DAC_SHSR1rs>;
///Register `DAC_SHSR1` writer
pub type W = crate::W<DAC_SHSR1rs>;
///Field `TSAMPLE1` reader - DAC Channel 1 sample Time (only valid in sample &amp;amp; hold mode)
pub type TSAMPLE1_R = crate::FieldReader<u16>;
///Field `TSAMPLE1` writer - DAC Channel 1 sample Time (only valid in sample &amp;amp; hold mode)
pub type TSAMPLE1_W<'a, REG> = crate::FieldWriter<'a, REG, 10, u16>;
impl R {
    ///Bits 0:9 - DAC Channel 1 sample Time (only valid in sample &amp;amp; hold mode)
    #[inline(always)]
    pub fn tsample1(&self) -> TSAMPLE1_R {
        TSAMPLE1_R::new((self.bits & 0x03ff) as u16)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("DAC_SHSR1")
            .field("tsample1", &self.tsample1())
            .finish()
    }
}
impl W {
    ///Bits 0:9 - DAC Channel 1 sample Time (only valid in sample &amp;amp; hold mode)
    #[inline(always)]
    #[must_use]
    pub fn tsample1(&mut self) -> TSAMPLE1_W<DAC_SHSR1rs> {
        TSAMPLE1_W::new(self, 0)
    }
}
/**DAC Sample and Hold sample time register 1

You can [`read`](crate::Reg::read) this register and get [`dac_shsr1::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`dac_shsr1::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U535.html#DAC1:DAC_SHSR1)*/
pub struct DAC_SHSR1rs;
impl crate::RegisterSpec for DAC_SHSR1rs {
    type Ux = u32;
}
///`read()` method returns [`dac_shsr1::R`](R) reader structure
impl crate::Readable for DAC_SHSR1rs {}
///`write(|w| ..)` method takes [`dac_shsr1::W`](W) writer structure
impl crate::Writable for DAC_SHSR1rs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets DAC_SHSR1 to value 0
impl crate::Resettable for DAC_SHSR1rs {
    const RESET_VALUE: u32 = 0;
}
