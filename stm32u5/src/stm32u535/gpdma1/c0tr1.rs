...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn ssec(&mut self) -> SSEC_W<C0TR1rs> {
        SSEC_W::new(self, 15)
    }
    /**Bits 16:17 - binary logarithm of the destination data width of a burst, in bytes- 00: byte- 01: half-word (2 bytes)- 10: word (4 bytes)- 11: a user setting error is reported and no transfer is issued.Note: Setting a 8-byte data width is causing a user setting error to be reported and none transfer is issued.Note: A destination burst transfer must have an aligned address with its data width (c.f. start address GPDMA_CxDAR\[2:0\]
    and address offset GPDMA_CxTR3.DAO\[2:0\]
    ...
    #[inline(always)]
    #[must_use]
    pub fn dsec(&mut self) -> DSEC_W<C0TR1rs> {
        DSEC_W::new(self, 31)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U535.html#GPDMA1:C0TR1)*/
pub struct C0TR1rs;
