...
impl W {
    ...
    vs GPDMA_CxTR1.SDW_LOG2\[1:0\]). Else a user setting error is reported and none transfer is issued.*/
    #[inline(always)]
    pub fn sao(&mut self) -> SAO_W<C13TR3rs> {
        SAO_W::new(self, 0)
    }
    /**Bits 16:28 - destination address offset increment The destination address, pointed by GPDMA_CxDAR, is incremented or decremented (depending on GPDMA_CxBR1.DDEC) by this offset DAO\[12:0\]
    for each programmed destination burst. This offset is not including and is added to the programmed burst size when the completed burst is addressed in incremented mode i.e. if GPDMA_CxTR1.DINC=1. Note: A destination address offset must be aligned with the programmed data width of a destination burst (c.f. DAO\[2:0\]
    ...
    is not applied.*/
    #[inline(always)]
    pub fn dao(&mut self) -> DAO_W<C13TR3rs> {
        DAO_W::new(self, 16)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U535.html#GPDMA1:C13TR3)*/
pub struct C13TR3rs;
