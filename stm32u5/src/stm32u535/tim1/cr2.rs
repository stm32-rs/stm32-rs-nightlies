...
///Field `OISN(1-4)` reader - Output Idle state (OC%sN output)
...
///Field `OISN(1-4)` writer - Output Idle state (OC%sN output)
...
impl R {
    ...
    ///Output Idle state (OC(1-4)N output)
    ///
    ...
    pub fn oisn_iter(&self) -> impl Iterator<Item = OISN_R> + '_ {
        (0..4).map(move |n| OISN_R::new(((self.bits >> (n * 2 + 9)) & 1) != 0))
    }
    ...
    #[inline(always)]
    pub fn ois4n(&self) -> OISN_R {
        OISN_R::new(((self.bits >> 15) & 1) != 0)
    }
...
impl W {
    ...
    ///Bits 4:6 - Master mode selection
    #[inline(always)]
    pub fn mms0_2(&mut self) -> MMS0_2_W<CR2rs> {
        MMS0_2_W::new(self, 4)
    }
    ///Bit 7 - TI1 selection
    #[inline(always)]
    ...
    ///Bit 15 - Output Idle state (OC4N output)
    #[inline(always)]
    pub fn ois4n(&mut self) -> OISN_W<CR2rs> {
        OISN_W::new(self, 15)
    }
    ///Bits 20:23 - Master mode selection 2
    #[inline(always)]
