///Register `GPDMA_C12SR` reader
pub type R = crate::R<GPDMA_C12SRrs>;
///Field `IDLEF` reader - idle flag - 0: the channel is not in idle state - 1: the channel is in idle state This idle flag is de-asserted by hardware when the channel is enabled (i.e. is written 1 into GPDMA_CxCR.EN) with a valid channel configuration (i.e. no USEF to be immediately reported). This idle flag is asserted after hard reset or by hardware when the channel is back in idle state (i.e. in suspended or disabled state).
...
///Field `HTF` reader - half transfer flag - 0: no half transfer event - 1: an half transfer event occurred An half transfer event is either an half block transfer or an half 2D/repeated block transfer, depending on the transfer complete event mode i.e. GPDMA_CxTR2.TCEM\[1:0\]. An half block transfer occurs when half of the bytes of the source block size (i.e. rounded up integer of GPDMA_CxBR1.BNDT\[15:0\]/2) has been transferred to the destination. Half 2D/repeated block transfer occurs when half of the repeated blocks (i.e. rounded up integer of (GPDMA_CxBR1.BRC\[10:0\]+1)/2) have been transferred to the destination.
...
You can [`read`](crate::Reg::read) this register and get [`gpdma_c12sr::R`](R). See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U545.html#GPDMA1:GPDMA_C12SR)*/
pub struct GPDMA_C12SRrs;
impl crate::RegisterSpec for GPDMA_C12SRrs {
    type Ux = u32;
}
///`read()` method returns [`gpdma_c12sr::R`](R) reader structure
impl crate::Readable for GPDMA_C12SRrs {}
///`reset()` method sets GPDMA_C12SR to value 0x01
