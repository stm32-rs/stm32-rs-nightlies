...
/**Calibration factor latch enable bit This bit latches the calibration factor in the CALFACT\[31:0\]
bits.
...
pub enum LATCH_COEF {
    ///0: No effect
    NoEffect = 0,
    /**1: Calibration factor latched in the analog block on LATCH_COEF bit transition from 0 to 1. Prior to latching the calibration factor, CALFACT\[31:0\]
    bits must be programmed with the content of CALINDEX\[3:0\]
    bits.*/
    Latch = 1,
}
...
impl LATCH_COEF_R {
    ...
    /**Calibration factor latched in the analog block on LATCH_COEF bit transition from 0 to 1. Prior to latching the calibration factor, CALFACT\[31:0\]
    bits must be programmed with the content of CALINDEX\[3:0\]
    bits.*/
    #[inline(always)]
...
pub enum CAPTURE_COEF {
    ...
    Disabled = 0,
    /**1: Calibration factor available in CALFACT\[31:0\]
    bits, the calibration factor index being defined by CALINDEX\[3:0\]
    bits*/
    Enabled = 1,
}
...
impl CAPTURE_COEF_R {
    ...
    /**Calibration factor available in CALFACT\[31:0\]
    bits, the calibration factor index being defined by CALINDEX\[3:0\]
    bits*/
    #[inline(always)]
