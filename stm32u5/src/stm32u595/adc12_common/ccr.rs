...
pub enum DUAL {
    ///0: Independent mode
    Independent = 0,
    ///1: Combined regular simultaneous + injected simultaneous mode
    RegularSimultaneousInjectedSimultaneous = 1,
    ///2: Combined regular simultaneous + alternate trigger mode
    RegularSimultaneousAlternateTrigger = 2,
    ///3: Combined interleaved mode + injected simultaneous mode
    InterleavedInjectedSimultaneous = 3,
    ///5: Injected simultaneous mode only
    InjectedSimultaneous = 5,
    ///6: Regular simultaneous mode only
    RegularSimultaneous = 6,
    ///7: Interleaved mode only
    Interleaved = 7,
    ///9: Alternate trigger mode only
    AlternateTrigger = 9,
}
...
impl DUAL_R {
    ...
    pub const fn variant(&self) -> Option<DUAL> {
        match self.bits {
            ...
            1 => Some(DUAL::RegularSimultaneousInjectedSimultaneous),
            2 => Some(DUAL::RegularSimultaneousAlternateTrigger),
            3 => Some(DUAL::InterleavedInjectedSimultaneous),
            5 => Some(DUAL::InjectedSimultaneous),
            6 => Some(DUAL::RegularSimultaneous),
            7 => Some(DUAL::Interleaved),
            9 => Some(DUAL::AlternateTrigger),
            _ => None,
        }
    }
    ///Independent mode
    ...
    #[inline(always)]
    pub fn is_regular_simultaneous_injected_simultaneous(&self) -> bool {
        *self == DUAL::RegularSimultaneousInjectedSimultaneous
    }
    ///Combined regular simultaneous + alternate trigger mode
    #[inline(always)]
    pub fn is_regular_simultaneous_alternate_trigger(&self) -> bool {
        *self == DUAL::RegularSimultaneousAlternateTrigger
    }
    ///Combined interleaved mode + injected simultaneous mode
    #[inline(always)]
    pub fn is_interleaved_injected_simultaneous(&self) -> bool {
        *self == DUAL::InterleavedInjectedSimultaneous
    }
    ///Injected simultaneous mode only
    #[inline(always)]
    pub fn is_injected_simultaneous(&self) -> bool {
        *self == DUAL::InjectedSimultaneous
    }
    ///Regular simultaneous mode only
    #[inline(always)]
    pub fn is_regular_simultaneous(&self) -> bool {
        *self == DUAL::RegularSimultaneous
    }
    ///Interleaved mode only
    #[inline(always)]
    pub fn is_interleaved(&self) -> bool {
        *self == DUAL::Interleaved
    }
    ///Alternate trigger mode only
    #[inline(always)]
    pub fn is_alternate_trigger(&self) -> bool {
        *self == DUAL::AlternateTrigger
    }
}
    ...
    REG::Ux: From<u8>,
{
    ...
    ///Combined regular simultaneous + injected simultaneous mode
    #[inline(always)]
    pub fn regular_simultaneous_injected_simultaneous(self) -> &'a mut crate::W<REG> {
        self.variant(DUAL::RegularSimultaneousInjectedSimultaneous)
    }
    ///Combined regular simultaneous + alternate trigger mode
    #[inline(always)]
    pub fn regular_simultaneous_alternate_trigger(self) -> &'a mut crate::W<REG> {
        self.variant(DUAL::RegularSimultaneousAlternateTrigger)
    }
    ///Combined interleaved mode + injected simultaneous mode
    #[inline(always)]
    pub fn interleaved_injected_simultaneous(self) -> &'a mut crate::W<REG> {
        self.variant(DUAL::InterleavedInjectedSimultaneous)
    }
    ///Injected simultaneous mode only
    #[inline(always)]
    pub fn injected_simultaneous(self) -> &'a mut crate::W<REG> {
        self.variant(DUAL::InjectedSimultaneous)
    }
    ///Regular simultaneous mode only
    #[inline(always)]
    pub fn regular_simultaneous(self) -> &'a mut crate::W<REG> {
        self.variant(DUAL::RegularSimultaneous)
    }
    ///Interleaved mode only
    #[inline(always)]
    pub fn interleaved(self) -> &'a mut crate::W<REG> {
        self.variant(DUAL::Interleaved)
    }
    ///Alternate trigger mode only
    #[inline(always)]
    pub fn alternate_trigger(self) -> &'a mut crate::W<REG> {
        self.variant(DUAL::AlternateTrigger)
    }
}
///Field `DELAY` reader - Delay between the end of the master ADC sampling phase and the beginning of the slave ADC sampling phase. These bits are set and cleared by software. These bits are used in dual interleaved modes. Refer to for the value of ADC resolution versus DELAY bits values. Note: The software is allowed to write these bits only when the ADCs are disabled (ADCAL = 0, JADSTART = 0, ADSTART = 0, ADSTP = 0, ADDIS = 0 and ADEN = 0).
pub type DELAY_R = crate::FieldReader;
///Field `DELAY` writer - Delay between the end of the master ADC sampling phase and the beginning of the slave ADC sampling phase. These bits are set and cleared by software. These bits are used in dual interleaved modes. Refer to for the value of ADC resolution versus DELAY bits values. Note: The software is allowed to write these bits only when the ADCs are disabled (ADCAL = 0, JADSTART = 0, ADSTART = 0, ADSTP = 0, ADDIS = 0 and ADEN = 0).
pub type DELAY_W<'a, REG> = crate::FieldWriter<'a, REG, 4, u8, crate::Safe>;
/**Dual ADC Mode Data Format This bit-field is set and cleared by software. It specifies the data format in the common data register ADC12_CDR. Note: The software is allowed to write these bits only when ADSTART = 0 (which ensures that no regular conversion is ongoing).
...
#[repr(u8)]
pub enum DAMDF {
    ///0: Dual ADC mode without data packing
    NoPacking = 0,
    ///2: Data formatting mode for 32 down to 10-bit resolution
    Format32To10 = 2,
    ///3: Data formatting mode for 8-bit resolution
    Format8 = 3,
}
...
///Field `DAMDF` reader - Dual ADC Mode Data Format This bit-field is set and cleared by software. It specifies the data format in the common data register ADC12_CDR. Note: The software is allowed to write these bits only when ADSTART = 0 (which ensures that no regular conversion is ongoing).
...
impl DAMDF_R {
    ...
    pub const fn variant(&self) -> Option<DAMDF> {
        match self.bits {
            0 => Some(DAMDF::NoPacking),
            2 => Some(DAMDF::Format32To10),
            3 => Some(DAMDF::Format8),
            _ => None,
        }
    }
    ///Dual ADC mode without data packing
    #[inline(always)]
    pub fn is_no_packing(&self) -> bool {
        *self == DAMDF::NoPacking
    }
    ///Data formatting mode for 32 down to 10-bit resolution
    #[inline(always)]
    pub fn is_format32_to_10(&self) -> bool {
...
///Field `DAMDF` writer - Dual ADC Mode Data Format This bit-field is set and cleared by software. It specifies the data format in the common data register ADC12_CDR. Note: The software is allowed to write these bits only when ADSTART = 0 (which ensures that no regular conversion is ongoing).
    ...
    REG::Ux: From<u8>,
{
    ///Dual ADC mode without data packing
    #[inline(always)]
    pub fn no_packing(self) -> &'a mut crate::W<REG> {
        self.variant(DAMDF::NoPacking)
    }
    ///Data formatting mode for 32 down to 10-bit resolution
    #[inline(always)]
    pub fn format32_to_10(self) -> &'a mut crate::W<REG> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U595.html#ADC12_Common:CCR)*/
pub struct CCRrs;
