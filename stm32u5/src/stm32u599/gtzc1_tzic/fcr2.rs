...
///Field `CLTDCF` writer - clear the illegal access flag for LTDC
pub type CLTDCF_W<'a, REG> = crate::BitWriter<'a, REG>;
///Field `CDSIF` writer - clear the illegal access flag for DSI
...
impl W {
    ...
    ///Bit 9 - clear the illegal access flag for LTDC
    #[inline(always)]
    pub fn cltdcf(&mut self) -> CLTDCF_W<'_, FCR2rs> {
        CLTDCF_W::new(self, 9)
    }
    ///Bit 10 - clear the illegal access flag for DSI
    #[inline(always)]
    pub fn cdsif(&mut self) -> CDSIF_W<'_, FCR2rs> {
        CDSIF_W::new(self, 10)
    }
}
