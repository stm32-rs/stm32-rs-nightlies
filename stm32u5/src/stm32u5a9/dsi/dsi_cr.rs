///Register `DSI_CR` reader
pub type R = crate::R<DSI_CRrs>;
///Register `DSI_CR` writer
pub type W = crate::W<DSI_CRrs>;
///Field `EN` reader - Enable This bit configures the DSI Host in either power-up mode or to reset.
pub type EN_R = crate::BitReader;
///Field `EN` writer - Enable This bit configures the DSI Host in either power-up mode or to reset.
pub type EN_W<'a, REG> = crate::BitWriter<'a, REG>;
impl R {
    ///Bit 0 - Enable This bit configures the DSI Host in either power-up mode or to reset.
    #[inline(always)]
    pub fn en(&self) -> EN_R {
        EN_R::new((self.bits & 1) != 0)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("DSI_CR").field("en", &self.en()).finish()
    }
}
impl W {
    ///Bit 0 - Enable This bit configures the DSI Host in either power-up mode or to reset.
    #[inline(always)]
    #[must_use]
    pub fn en(&mut self) -> EN_W<DSI_CRrs> {
        EN_W::new(self, 0)
    }
}
/**DSI Host control register

You can [`read`](crate::Reg::read) this register and get [`dsi_cr::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`dsi_cr::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32U5A9.html#DSI:DSI_CR)*/
pub struct DSI_CRrs;
impl crate::RegisterSpec for DSI_CRrs {
    type Ux = u32;
}
///`read()` method returns [`dsi_cr::R`](R) reader structure
impl crate::Readable for DSI_CRrs {}
///`write(|w| ..)` method takes [`dsi_cr::W`](W) writer structure
impl crate::Writable for DSI_CRrs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
///`reset()` method sets DSI_CR to value 0
impl crate::Resettable for DSI_CRrs {
    const RESET_VALUE: u32 = 0;
}
