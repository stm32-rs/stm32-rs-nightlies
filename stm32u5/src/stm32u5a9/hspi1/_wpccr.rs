...
impl W {
    ///Bits 0:2 - Instruction mode This field defines the instruction phase mode of operation. 101-111: Reserved
    #[inline(always)]
    pub fn imode(&mut self) -> IMODE_W<_WPCCRrs> {
        IMODE_W::new(self, 0)
    }
    ///Bit 3 - Instruction double transfer rate This bit sets the DTR mode for the instruction phase.
    #[inline(always)]
    pub fn idtr(&mut self) -> IDTR_W<_WPCCRrs> {
        IDTR_W::new(self, 3)
    }
    ///Bits 4:5 - Instruction size This field defines instruction size.
    #[inline(always)]
    pub fn isize(&mut self) -> ISIZE_W<_WPCCRrs> {
        ISIZE_W::new(self, 4)
    }
    ///Bits 8:10 - Address mode This field defines the address phase mode of operation. 101-111: Reserved
    #[inline(always)]
    pub fn admode(&mut self) -> ADMODE_W<_WPCCRrs> {
        ADMODE_W::new(self, 8)
    }
    ///Bit 11 - Address double transfer rate This bit sets the DTR mode for the address phase.
    #[inline(always)]
    pub fn addtr(&mut self) -> ADDTR_W<_WPCCRrs> {
        ADDTR_W::new(self, 11)
    }
    ///Bits 12:13 - Address size This field defines address size.
    #[inline(always)]
    pub fn adsize(&mut self) -> ADSIZE_W<_WPCCRrs> {
        ADSIZE_W::new(self, 12)
    }
    ///Bits 16:18 - Alternate-byte mode This field defines the alternate byte phase mode of operation.
    #[inline(always)]
    pub fn abmode(&mut self) -> ABMODE_W<_WPCCRrs> {
        ABMODE_W::new(self, 16)
    }
    ///Bit 19 - Alternate bytes double transfer rate This bit sets the DTR mode for the alternate bytes phase.
    #[inline(always)]
    pub fn abdtr(&mut self) -> ABDTR_W<_WPCCRrs> {
        ABDTR_W::new(self, 19)
    }
    ///Bits 20:21 - Alternate bytes size This bit defines alternate bytes size.
    #[inline(always)]
    pub fn absize(&mut self) -> ABSIZE_W<_WPCCRrs> {
        ABSIZE_W::new(self, 20)
    }
    ///Bits 24:26 - Data mode This field defines the data phase mode of operation. 101; Data on 16 lines 110-111: Reserved
    #[inline(always)]
    pub fn dmode(&mut self) -> DMODE_W<_WPCCRrs> {
        DMODE_W::new(self, 24)
    }
    ///Bit 27 - Data double transfer rate This bit sets the DTR mode for the data phase.
    #[inline(always)]
    pub fn ddtr(&mut self) -> DDTR_W<_WPCCRrs> {
        DDTR_W::new(self, 27)
    }
    ///Bit 29 - DQS enable This bit enables the data strobe management.
    #[inline(always)]
    pub fn dqse(&mut self) -> DQSE_W<_WPCCRrs> {
        DQSE_W::new(self, 29)
    }
}
