#[doc = "Register `HSPI_SR` reader"]
pub type R = crate::R<HSPI_SRrs>;
...
#[doc = "Field `SMF` reader - Status match flag This bit is set in Automatic-polling mode when the unmasked received data matches the corresponding bits in the match register (HSPI_PSMAR). It is cleared by writing 1 to CSMF."]
...
#[doc = "Field `FLEVEL` reader - FIFO level This field gives the number of valid bytes that are being held in the FIFO. FLEVELÂ =Â 0 when the FIFO is empty, and 64 when it is full. In Automatic-status polling mode, FLEVEL is zero."]
...
impl R {
    ...
    #[doc = "Bit 3 - Status match flag This bit is set in Automatic-polling mode when the unmasked received data matches the corresponding bits in the match register (HSPI_PSMAR). It is cleared by writing 1 to CSMF."]
    #[inline(always)]
    ...
    #[doc = "Bits 8:14 - FIFO level This field gives the number of valid bytes that are being held in the FIFO. FLEVELÂ =Â 0 when the FIFO is empty, and 64 when it is full. In Automatic-status polling mode, FLEVEL is zero."]
    #[inline(always)]
    pub fn flevel(&self) -> FLEVEL_R {
        FLEVEL_R::new(((self.bits >> 8) & 0x7f) as u8)
    }
}
#[doc = "\n\nYou can [`read`](crate::generic::Reg::read) this register and get [`hspi_sr::R`](R).  See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct HSPI_SRrs;
impl crate::RegisterSpec for HSPI_SRrs {
    type Ux = u32;
}
#[doc = "`read()` method returns [`hspi_sr::R`](R) reader structure"]
impl crate::Readable for HSPI_SRrs {}
#[doc = "`reset()` method sets HSPI_SR to value 0"]
impl crate::Resettable for HSPI_SRrs {
