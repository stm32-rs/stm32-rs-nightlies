#[doc = "Register `LPDMA_C2CR` reader"]
pub type R = crate::R<LPDMA_C2CRrs>;
#[doc = "Register `LPDMA_C2CR` writer"]
pub type W = crate::W<LPDMA_C2CRrs>;
...
#[doc = "Field `LSM` writer - Link Step mode:- 0: channel is executed for the full linked-list, and completed at the end (if any) of the last LLI i.e. when LPDMA_CxLLR=0: the 16 low significant bits of the link address are null (LA\\[15:0\\]=0) and all the update bits are null i.e. UT1=UB1=UT2=USA=UDA=UB2 =UT3=ULL=0. Then LPDMA_CxBR1.BRC\\[10:0\\]=0 and LPDMA_CxBR1.BNDT\\[15:0\\]=0.- 1: channel is executed once for the current LLI:* First the (possibly 2D/repeated) block transfer is executed as defined by the current internal register file until that (LPDMA_CxBR1.BRC\\[10:0\\]=0 and LPDMA_CxBR1.BNDT\\[15:0\\]=0).* Secondly the next linked-list data structure is conditionally uploaded from memory as defined by LPDMA_CxLLR register. Then channel execution is completed.Note: This bit must be written when EN=0. This bit is read-only when EN=1."]
...
impl R {
    ...
    #[doc = "Bit 16 - Link Step mode:- 0: channel is executed for the full linked-list, and completed at the end (if any) of the last LLI i.e. when LPDMA_CxLLR=0: the 16 low significant bits of the link address are null (LA\\[15:0\\]=0) and all the update bits are null i.e. UT1=UB1=UT2=USA=UDA=UB2 =UT3=ULL=0. Then LPDMA_CxBR1.BRC\\[10:0\\]=0 and LPDMA_CxBR1.BNDT\\[15:0\\]=0.- 1: channel is executed once for the current LLI:* First the (possibly 2D/repeated) block transfer is executed as defined by the current internal register file until that (LPDMA_CxBR1.BRC\\[10:0\\]=0 and LPDMA_CxBR1.BNDT\\[15:0\\]=0).* Secondly the next linked-list data structure is conditionally uploaded from memory as defined by LPDMA_CxLLR register. Then channel execution is completed.Note: This bit must be written when EN=0. This bit is read-only when EN=1."]
    #[inline(always)]
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn en(&mut self) -> EN_W<LPDMA_C2CRrs> {
        EN_W::new(self, 0)
    }
    #[doc = "Bit 1 - reset - 0: no channel reset - 1: channel reset This bit is write only. Writing 0 has no impact. Writing 1 implies/will imply the reset of the FIFO, the reset of the channel internal state, and the reset of the SUSP and EN bits, whatever is written in respectively bit 2 and bit 0. The reset is/will be effective when the channel is in state i.e. either i) the active channel is in suspended state (i.e. LPDMA_CxSR.SUSPF=1 and LPDMA_CxSR.IDLEF=1 and LPDMA_CxCR.EN=1) or ii) the channel is in disabled state (i.e. LPDMA_CxSR.IDLEF=1 and LPDMA_CxCR.EN=0). After writing a RESET, if the user wants to continue using this channel, the user should explicitly reconfigure the channel including the hardware-modified configuration registers LPDMA_CxBR1, LPDMA_CxSAR and LPDMA_CxDAR, before enabling again the channel. Following the programming sequence in Figure 4: DMA channel abort and restart sequence."]
    #[inline(always)]
    #[must_use]
    pub fn reset(&mut self) -> RESET_W<LPDMA_C2CRrs> {
        RESET_W::new(self, 1)
    }
    #[doc = "Bit 2 - suspend - 0: write: resume channel, read: channel not suspended - 1: write: suspend channel, read: channel suspended Writing 1 into the field RESET (bit 1) causes the hardware to de-assert this bit, whatever is written into this bit 2. Else: Software must write 1 in order to suspend an active channel i.e. a channel with an on-going DMA transfer over its master ports. Software must write 0 in order to resume a suspended channel, following the programming sequence in Figure 3: DMA channel suspend and resume sequence."]
    #[inline(always)]
    #[must_use]
    pub fn susp(&mut self) -> SUSP_W<LPDMA_C2CRrs> {
        SUSP_W::new(self, 2)
    }
    #[doc = "Bit 8 - transfer complete interrupt enable - 0: interrupt disabled - 1: interrupt enabled"]
    #[inline(always)]
    #[must_use]
    pub fn tcie(&mut self) -> TCIE_W<LPDMA_C2CRrs> {
        TCIE_W::new(self, 8)
    }
    #[doc = "Bit 9 - half transfer complete interrupt enable - 0: interrupt disabled - 1: interrupt enabled"]
    #[inline(always)]
    #[must_use]
    pub fn htie(&mut self) -> HTIE_W<LPDMA_C2CRrs> {
        HTIE_W::new(self, 9)
    }
    #[doc = "Bit 10 - data transfer error interrupt enable - 0: interrupt disabled - 1: interrupt enabled"]
    #[inline(always)]
    #[must_use]
    pub fn dteie(&mut self) -> DTEIE_W<LPDMA_C2CRrs> {
        DTEIE_W::new(self, 10)
    }
    #[doc = "Bit 11 - update link transfer error interrupt enable - 0: interrupt disabled - 1: interrupt enabled"]
    #[inline(always)]
    #[must_use]
    pub fn uleie(&mut self) -> ULEIE_W<LPDMA_C2CRrs> {
        ULEIE_W::new(self, 11)
    }
    #[doc = "Bit 12 - user setting error interrupt enable - 0: interrupt disabled - 1: interrupt enabled"]
    #[inline(always)]
    #[must_use]
    pub fn useie(&mut self) -> USEIE_W<LPDMA_C2CRrs> {
        USEIE_W::new(self, 12)
    }
    #[doc = "Bit 13 - completed suspension interrupt enable - 0: interrupt disabled - 1: interrupt enabled"]
    #[inline(always)]
    #[must_use]
    pub fn suspie(&mut self) -> SUSPIE_W<LPDMA_C2CRrs> {
        SUSPIE_W::new(self, 13)
    }
    #[doc = "Bit 16 - Link Step mode:- 0: channel is executed for the full linked-list, and completed at the end (if any) of the last LLI i.e. when LPDMA_CxLLR=0: the 16 low significant bits of the link address are null (LA\\[15:0\\]=0) and all the update bits are null i.e. UT1=UB1=UT2=USA=UDA=UB2 =UT3=ULL=0. Then LPDMA_CxBR1.BRC\\[10:0\\]=0 and LPDMA_CxBR1.BNDT\\[15:0\\]=0.- 1: channel is executed once for the current LLI:* First the (possibly 2D/repeated) block transfer is executed as defined by the current internal register file until that (LPDMA_CxBR1.BRC\\[10:0\\]=0 and LPDMA_CxBR1.BNDT\\[15:0\\]=0).* Secondly the next linked-list data structure is conditionally uploaded from memory as defined by LPDMA_CxLLR register. Then channel execution is completed.Note: This bit must be written when EN=0. This bit is read-only when EN=1."]
    #[inline(always)]
    #[must_use]
    pub fn lsm(&mut self) -> LSM_W<LPDMA_C2CRrs> {
        LSM_W::new(self, 16)
    }
    #[doc = "Bits 22:23 - priority level of the DMA transfer of the channel x vs others- 00: low priority, low weight- 01: low priority, mid weight- 10: low priority, high weight- 11: high priorityNote: This bit must be written when EN=0. This bit is read-only when EN=1."]
    #[inline(always)]
    #[must_use]
    pub fn prio(&mut self) -> PRIO_W<LPDMA_C2CRrs> {
        PRIO_W::new(self, 22)
    }
}
#[doc = "channel x control register\n\nYou can [`read`](crate::generic::Reg::read) this register and get [`lpdma_c2cr::R`](R).  You can [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`write_with_zero`](crate::generic::Reg::write_with_zero) this register using [`lpdma_c2cr::W`](W). You can also [`modify`](crate::generic::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct LPDMA_C2CRrs;
impl crate::RegisterSpec for LPDMA_C2CRrs {
    type Ux = u32;
}
#[doc = "`read()` method returns [`lpdma_c2cr::R`](R) reader structure"]
impl crate::Readable for LPDMA_C2CRrs {}
#[doc = "`write(|w| ..)` method takes [`lpdma_c2cr::W`](W) writer structure"]
impl crate::Writable for LPDMA_C2CRrs {
    type Safety = crate::Unsafe;
    const ZERO_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
    const ONE_TO_MODIFY_FIELDS_BITMAP: u32 = 0;
}
#[doc = "`reset()` method sets LPDMA_C2CR to value 0"]
impl crate::Resettable for LPDMA_C2CRrs {
