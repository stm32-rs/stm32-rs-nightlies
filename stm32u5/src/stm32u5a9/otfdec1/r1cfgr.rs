...
///Field `REG1_VERSION` reader - region firmware version
pub type REG1_VERSION_R = crate::FieldReader<u16>;
///Field `REG1_VERSION` writer - region firmware version
pub type REG1_VERSION_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    ...
    #[inline(always)]
    pub fn reg1_version(&self) -> REG1_VERSION_R {
        REG1_VERSION_R::new(((self.bits >> 16) & 0xffff) as u16)
    }
}
impl core::fmt::Debug for R {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("R1CFGR")
            ...
            .field("reg1_version", &self.reg1_version())
            .finish()
    }
}
impl W {
    ///Bit 0 - region on-the-fly decryption enable
    #[inline(always)]
    pub fn reg_en(&mut self) -> REG_EN_W<'_, R1CFGRrs> {
        REG_EN_W::new(self, 0)
    }
    ///Bit 1 - region config lock
    #[inline(always)]
    pub fn configlock(&mut self) -> CONFIGLOCK_W<'_, R1CFGRrs> {
        CONFIGLOCK_W::new(self, 1)
    }
    ///Bit 2 - region key lock
    #[inline(always)]
    pub fn keylock(&mut self) -> KEYLOCK_W<'_, R1CFGRrs> {
        KEYLOCK_W::new(self, 2)
    }
    ///Bits 4:5 - operating mode
    #[inline(always)]
    pub fn mode(&mut self) -> MODE_W<'_, R1CFGRrs> {
        MODE_W::new(self, 4)
    }
    ///Bits 16:31 - region firmware version
    #[inline(always)]
    pub fn reg1_version(&mut self) -> REG1_VERSION_W<'_, R1CFGRrs> {
        REG1_VERSION_W::new(self, 16)
    }
}
