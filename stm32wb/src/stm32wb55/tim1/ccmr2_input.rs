...
pub type C3PSC_R = crate::FieldReader;
///Field `C3PSC` writer - Input capture 3 prescaler
pub type C3PSC_W<'a, REG> = crate::FieldWriter<'a, REG, 2>;
...
///Field `ICPSC(4-4)` reader - Input capture %s prescaler
pub use super::ccmr1_input::ICPSC_R;
///Field `ICPSC(4-4)` writer - Input capture %s prescaler
...
impl R {
    ...
    ///Input capture (4-4) prescaler
    ///
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `IC4PSC` field.</div>
    #[inline(always)]
    pub fn icpsc(&self, n: u8) -> ICPSC_R {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        ICPSC_R::new(((self.bits >> (n * 0 + 10)) & 3) as u8)
    }
    ///Iterator for array of:
    ///Input capture (4-4) prescaler
    #[inline(always)]
    pub fn icpsc_iter(&self) -> impl Iterator<Item = ICPSC_R> + '_ {
        (0..1).map(move |n| ICPSC_R::new(((self.bits >> (n * 0 + 10)) & 3) as u8))
    }
...
impl W {
    ...
    ///Bits 2:3 - Input capture 3 prescaler
    #[inline(always)]
    pub fn c3psc(&mut self) -> C3PSC_W<'_, CCMR2_INPUTrs> {
        C3PSC_W::new(self, 2)
    }
    ///Input capture (3-4) filter
    ///
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WB55.html#TIM1:CCMR2_Input)*/
