...
///Field `LCK(0-11)` reader - Port x lock pin %s
...
///Field `LCK(0-11)` writer - Port x lock pin %s
...
impl R {
    ...
    pub fn lck_iter(&self) -> impl Iterator<Item = LCK_R> + '_ {
        (0..12).map(move |n| LCK_R::new(((self.bits >> n) & 1) != 0))
    }
...
impl W {
    ...
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `LCK0` field.</div>
    #[inline(always)]
    pub fn lck(&mut self, n: u8) -> LCK_W<'_, LCKRrs> {
        #[allow(clippy::no_effect)]
        [(); 12][n as usize];
        LCK_W::new(self, n)
    }
    ///Bit 0 - Port x lock pin 0
    #[inline(always)]
    pub fn lck0(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 0)
    }
    ///Bit 1 - Port x lock pin 1
    #[inline(always)]
    pub fn lck1(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 1)
    }
    ///Bit 2 - Port x lock pin 2
    #[inline(always)]
    pub fn lck2(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 2)
    }
    ///Bit 3 - Port x lock pin 3
    #[inline(always)]
    pub fn lck3(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 3)
    }
    ///Bit 4 - Port x lock pin 4
    #[inline(always)]
    pub fn lck4(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 4)
    }
    ///Bit 5 - Port x lock pin 5
    #[inline(always)]
    pub fn lck5(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 5)
    }
    ///Bit 6 - Port x lock pin 6
    #[inline(always)]
    pub fn lck6(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 6)
    }
    ///Bit 7 - Port x lock pin 7
    #[inline(always)]
    pub fn lck7(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 7)
    }
    ///Bit 8 - Port x lock pin 8
    #[inline(always)]
    pub fn lck8(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 8)
    }
    ///Bit 9 - Port x lock pin 9
    #[inline(always)]
    pub fn lck9(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 9)
    }
    ///Bit 10 - Port x lock pin 10
    #[inline(always)]
    pub fn lck10(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 10)
    }
    ///Bit 11 - Port x lock pin 11
    #[inline(always)]
    pub fn lck11(&mut self) -> LCK_W<'_, LCKRrs> {
        LCK_W::new(self, 11)
    }
    ///Bit 16 - LCKK: Lock key This bit can be read any time. It can only be modified using the lock key write sequence. -0: Port configuration lock key not active -1: Port configuration lock key active. The GPIOx_LCKR register is locked until the next MCU reset or peripheral reset. LOCK key write sequence: WR LCKR\[16\] = 1' + LCKR\[15:0\] WR LCKR\[16\] = 0' + LCKR\[15:0\] WR LCKR\[16\] = 1' + LCKR\[15:0\] RD LCKR RD LCKR\[16\] = 1' (this read operation is optional but it confirms that the lock is active) Note: During the LOCK key write sequence, the value of LCK\[15:0\] must not change. Any error in the lock sequence aborts the lock. After the first lock sequence on any bit of the port, any read access on the LCKK bit will return 1' until the next MCU reset or peripheral reset
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WB05.html#GPIOA:LCKR)*/
pub struct LCKRrs;
