...
///Field `MODE(0-11)` reader - Port x configuration pin %s
...
///Field `MODE(0-11)` writer - Port x configuration pin %s
...
impl R {
    ...
    pub fn mode_iter(&self) -> impl Iterator<Item = MODE_R> + '_ {
        (0..12).map(move |n| MODE_R::new(((self.bits >> (n * 2)) & 3) as u8))
    }
...
///`reset()` method sets MODER to value 0xa0
