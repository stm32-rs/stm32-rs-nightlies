...
///Field `OD(0-11)` reader - Port output data pin %s
...
///Field `OD(0-11)` writer - Port output data pin %s
...
impl R {
    ...
    pub fn od_iter(&self) -> impl Iterator<Item = OD_R> + '_ {
        (0..12).map(move |n| OD_R::new(((self.bits >> n) & 1) != 0))
    }
