...
///Field `OSPEED(0-11)` reader - Port x configuration pin %s
...
///Field `OSPEED(0-11)` writer - Port x configuration pin %s
...
impl R {
    ...
    pub fn ospeed_iter(&self) -> impl Iterator<Item = OSPEED_R> + '_ {
        (0..12).map(move |n| OSPEED_R::new(((self.bits >> (n * 2)) & 3) as u8))
    }
