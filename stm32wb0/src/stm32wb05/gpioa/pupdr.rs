...
///Field `PUPD(0-11)` reader - Port x configuration pin %s
...
///Field `PUPD(0-11)` writer - Port x configuration pin %s
...
impl R {
    ...
    pub fn pupd_iter(&self) -> impl Iterator<Item = PUPD_R> + '_ {
        (0..12).map(move |n| PUPD_R::new(((self.bits >> (n * 2)) & 3) as u8))
    }
...
///`reset()` method sets PUPDR to value 0x0055_0095
impl crate::Resettable for PUPDRrs {
    const RESET_VALUE: u32 = 0x0055_0095;
}
