...
impl W {
    ///Bit 0 - semaphore locked (= one port granted) interrupt enable
    #[inline(always)]
    pub fn lock(&mut self) -> LOCK_W<SEMA_IRQ_ENABLErs> {
        LOCK_W::new(self, 0)
    }
    ///Bit 1 - semaphore unlocked (=no port selected) interrupt enable
    #[inline(always)]
    pub fn unlock(&mut self) -> UNLOCK_W<SEMA_IRQ_ENABLErs> {
        UNLOCK_W::new(self, 1)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WB05.html#RRM:SEMA_IRQ_ENABLE)*/
