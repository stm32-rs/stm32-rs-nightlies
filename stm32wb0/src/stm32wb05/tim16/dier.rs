...
/**COMIE: COM interrupt enable 0: COM interrupt disabled 1: COM interrupt enabled
...
/**UDE: Update DMA request enable 0: Update DMA request disabled 1: Update DMA request enabled
...
///Field `CCDE(1,U)` reader - Capture/Compare %s DMA request enable
...
///Field `CCDE(1,U)` writer - Capture/Compare %s DMA request enable
...
impl R {
    ...
    ///Capture/Compare (1,U) DMA request enable
    ///
    ...
    pub fn ccde(&self, n: u8) -> CCDE_R {
        #[allow(clippy::no_effect)]
        [(); 2][n as usize];
        CCDE_R::new(((self.bits >> (n * 4 + 9)) & 1) != 0)
    }
    ///Iterator for array of:
    ///Capture/Compare (1,U) DMA request enable
    #[inline(always)]
    pub fn ccde_iter(&self) -> impl Iterator<Item = CCDE_R> + '_ {
        (0..2).map(move |n| CCDE_R::new(((self.bits >> (n * 4 + 9)) & 1) != 0))
    }
    ...
    ///Bit 13 - Capture/Compare U DMA request enable
    #[inline(always)]
    pub fn ccude(&self) -> CCDE_R {
        CCDE_R::new(((self.bits >> 13) & 1) != 0)
    }
...
impl W {
    ...
    ///Bit 13 - Capture/Compare U DMA request enable
    #[inline(always)]
    pub fn ccude(&mut self) -> CCDE_W<DIERrs> {
        CCDE_W::new(self, 13)
    }
    ///Bit 15 - BDE: Break DMA request Enable. Not used in Blue51. Not available in IUM 0: Break DMA request disabled. 1: Break DMA request enabled.
    #[inline(always)]
    pub fn bde(&mut self) -> BDE_W<DIERrs> {
        BDE_W::new(self, 15)
    }
}
/**DIER register
