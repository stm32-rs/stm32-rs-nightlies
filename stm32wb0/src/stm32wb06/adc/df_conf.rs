...
impl W {
    ///Bits 0:6
    #[inline(always)]
    pub fn df_cic_dec_factor(&mut self) -> DF_CIC_DEC_FACTOR_W<DF_CONFrs> {
        DF_CIC_DEC_FACTOR_W::new(self, 0)
    }
    ///Bit 7 - CIC filter decimator half factor
    #[inline(always)]
    pub fn df_cic_dhf(&mut self) -> DF_CIC_DHF_W<DF_CONFrs> {
        DF_CIC_DHF_W::new(self, 7)
    }
    ///Bit 8 - 1.2 fractional interpolator enable
    #[inline(always)]
    pub fn df_itp1p2(&mut self) -> DF_ITP1P2_W<DF_CONFrs> {
        DF_ITP1P2_W::new(self, 8)
    }
    ///Bit 9 - select signed/unsigned format for input
    #[inline(always)]
    pub fn df_i_u2s(&mut self) -> DF_I_U2S_W<DF_CONFrs> {
        DF_I_U2S_W::new(self, 9)
    }
    ///Bit 10 - select signed/unsigned format for data output
    #[inline(always)]
    pub fn df_o_s2u(&mut self) -> DF_O_S2U_W<DF_CONFrs> {
        DF_O_S2U_W::new(self, 10)
    }
    ///Bits 11:14 - select the PDM clock rate.
    #[inline(always)]
    pub fn pdm_rate(&mut self) -> PDM_RATE_W<DF_CONFrs> {
        PDM_RATE_W::new(self, 11)
    }
    ///Bit 15 - left/right channel selection on digital microphone
    #[inline(always)]
    pub fn df_microl_rn(&mut self) -> DF_MICROL_RN_W<DF_CONFrs> {
        DF_MICROL_RN_W::new(self, 15)
    }
    ///Bit 16 - high pass filter enable.
    #[inline(always)]
    pub fn df_hpf_en(&mut self) -> DF_HPF_EN_W<DF_CONFrs> {
        DF_HPF_EN_W::new(self, 16)
    }
    ///Bit 17 - half dynamic enable.
    #[inline(always)]
    pub fn df_half_d_en(&mut self) -> DF_HALF_D_EN_W<DF_CONFrs> {
