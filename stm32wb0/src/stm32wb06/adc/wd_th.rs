...
///Field `WD_LT` reader - analog watchdog low level threshold.
pub type WD_LT_R = crate::FieldReader<u16>;
///Field `WD_LT` writer - analog watchdog low level threshold.
pub type WD_LT_W<'a, REG> = crate::FieldWriter<'a, REG, 12, u16>;
///Field `WD_HT` reader - analog watchdog high level threshold.
pub type WD_HT_R = crate::FieldReader<u16>;
///Field `WD_HT` writer - analog watchdog high level threshold.
...
impl W {
    ///Bits 0:11 - analog watchdog low level threshold.
    #[inline(always)]
    pub fn wd_lt(&mut self) -> WD_LT_W<WD_THrs> {
        WD_LT_W::new(self, 0)
    }
    ///Bits 16:27 - analog watchdog high level threshold.
    #[inline(always)]
    pub fn wd_ht(&mut self) -> WD_HT_W<WD_THrs> {
        WD_HT_W::new(self, 16)
    }
}
