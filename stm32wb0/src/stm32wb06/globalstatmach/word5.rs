...
impl W {
    ///Bit 0 - Automatic transfer (TX or RX) skip enable.
    #[inline(always)]
    pub fn auto_tx_rxskip_en(&mut self) -> AUTO_TX_RXSKIP_EN_W<WORD5rs> {
        AUTO_TX_RXSKIP_EN_W::new(self, 0)
    }
    ///Bit 2 - Active bit Auto Clear Enable.
    #[inline(always)]
    pub fn chk_flag_auto_clear_ena(&mut self) -> CHK_FLAG_AUTO_CLEAR_ENA_W<WORD5rs> {
        CHK_FLAG_AUTO_CLEAR_ENA_W::new(self, 2)
    }
    ///Bit 20 - Address pointer error interrupt enable.
    #[inline(always)]
    pub fn int_add_point_error(&mut self) -> INT_ADD_POINT_ERROR_W<WORD5rs> {
        INT_ADD_POINT_ERROR_W::new(self, 20)
    }
    ///Bit 21 - All table ready error interrupt enable.
    #[inline(always)]
    pub fn int_all_table_ready_error(&mut self) -> INT_ALL_TABLE_READY_ERROR_W<WORD5rs> {
        INT_ALL_TABLE_READY_ERROR_W::new(self, 21)
    }
    ///Bit 22 - Transmission data payload ready error interrupt enable.
    #[inline(always)]
    pub fn int_tx_data_ready_error(&mut self) -> INT_TX_DATA_READY_ERROR_W<WORD5rs> {
        INT_TX_DATA_READY_ERROR_W::new(self, 22)
    }
    ///Bit 23 - Active bit low value reading interrupt enable.
    #[inline(always)]
    pub fn int_no_active_lerror(&mut self) -> INT_NO_ACTIVE_LERROR_W<WORD5rs> {
        INT_NO_ACTIVE_LERROR_W::new(self, 23)
    }
    ///Bit 25 - Too long received payload length interrupt enable.
    #[inline(always)]
    pub fn int_rcv_length_error(&mut self) -> INT_RCV_LENGTH_ERROR_W<WORD5rs> {
        INT_RCV_LENGTH_ERROR_W::new(self, 25)
    }
    ///Bit 26 - Semaphore timeout error interrupt enable.
    #[inline(always)]
    pub fn int_sema_timeout_error(&mut self) -> INT_SEMA_TIMEOUT_ERROR_W<WORD5rs> {
        INT_SEMA_TIMEOUT_ERROR_W::new(self, 26)
    }
    ///Bit 28 - Sequencer end of task interrupt enable.
    #[inline(always)]
    pub fn int_seq_done(&mut self) -> INT_SEQ_DONE_W<WORD5rs> {
        INT_SEQ_DONE_W::new(self, 28)
    }
    ///Bit 29 - Transmission or reception skip interrupt enable.
    #[inline(always)]
    pub fn int_tx_rx_skip(&mut self) -> INT_TX_RX_SKIP_W<WORD5rs> {
        INT_TX_RX_SKIP_W::new(self, 29)
    }
    ///Bit 30 - not in ACTIVE2 information from Radio FSM received on time interrupt enable.
    #[inline(always)]
    pub fn int_active2err(&mut self) -> INT_ACTIVE2ERR_W<WORD5rs> {
        INT_ACTIVE2ERR_W::new(self, 30)
    }
    ///Bit 31 - Configuration error interrupt enable.
    #[inline(always)]
    pub fn int_config_error(&mut self) -> INT_CONFIG_ERROR_W<WORD5rs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WB06.html#GLOBALSTATMACH:WORD5)*/
pub struct WORD5rs;
