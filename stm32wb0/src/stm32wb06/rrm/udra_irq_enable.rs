...
impl W {
    ///Bit 0 - UDRA interrupt enable (reload radio config pointer)
    #[inline(always)]
    pub fn radio_cfg_ptr_reloaded(&mut self) -> RADIO_CFG_PTR_RELOADED_W<UDRA_IRQ_ENABLErs> {
        RADIO_CFG_PTR_RELOADED_W::new(self, 0)
    }
    ///Bit 1 - UDRA interrupt enable (command start)
    #[inline(always)]
    pub fn cmd_start(&mut self) -> CMD_START_W<UDRA_IRQ_ENABLErs> {
        CMD_START_W::new(self, 1)
    }
    ///Bit 2 - UDRA interrupt enable (command end)
    #[inline(always)]
    pub fn cmd_end(&mut self) -> CMD_END_W<UDRA_IRQ_ENABLErs> {
