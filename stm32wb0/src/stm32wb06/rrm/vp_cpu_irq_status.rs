...
impl W {
    ///Bit 0 - CPU virtual port granted interrupt status.
    #[inline(always)]
    pub fn port_grant(&mut self) -> PORT_GRANT_W<VP_CPU_IRQ_STATUSrs> {
        PORT_GRANT_W::new(self, 0)
    }
    ///Bit 1 - virtual port released interrupt status.
    #[inline(always)]
    pub fn port_release(&mut self) -> PORT_RELEASE_W<VP_CPU_IRQ_STATUSrs> {
        PORT_RELEASE_W::new(self, 1)
    }
    ///Bit 2 - CPU virtual port preemption (at semaphore level) interrupt status.
    #[inline(always)]
    pub fn port_preempt(&mut self) -> PORT_PREEMPT_W<VP_CPU_IRQ_STATUSrs> {
        PORT_PREEMPT_W::new(self, 2)
    }
    ///Bit 3 - CPU virtual port command start interrupt status.
    #[inline(always)]
    pub fn cmd_start(&mut self) -> CMD_START_W<VP_CPU_IRQ_STATUSrs> {
        CMD_START_W::new(self, 3)
    }
    ///Bit 4 - CPU virtual port command end interrupt status.
    #[inline(always)]
    pub fn cmd_end(&mut self) -> CMD_END_W<VP_CPU_IRQ_STATUSrs> {
