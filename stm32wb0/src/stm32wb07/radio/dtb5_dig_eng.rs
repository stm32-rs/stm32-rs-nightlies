...
impl W {
    ///Bit 0 - enable the possibility to control some signals by the other register bits instead of system design:
    #[inline(always)]
    pub fn rxtx_start_sel(&mut self) -> RXTX_START_SEL_W<DTB5_DIG_ENGrs> {
        RXTX_START_SEL_W::new(self, 0)
    }
    ///Bit 1 - Force TX_ACTIVE signal
    #[inline(always)]
    pub fn tx_active(&mut self) -> TX_ACTIVE_W<DTB5_DIG_ENGrs> {
        TX_ACTIVE_W::new(self, 1)
    }
    ///Bit 2 - Force RX_ACTIVE signal
    #[inline(always)]
    pub fn rx_active(&mut self) -> RX_ACTIVE_W<DTB5_DIG_ENGrs> {
        RX_ACTIVE_W::new(self, 2)
    }
    ///Bit 3 - Force INITIALIZE signal (emulate a token request of the IP_BLE)
    #[inline(always)]
    pub fn initialize(&mut self) -> INITIALIZE_W<DTB5_DIG_ENGrs> {
        INITIALIZE_W::new(self, 3)
    }
    ///Bit 4 - enable port selection
    #[inline(always)]
    pub fn port_selected_en(&mut self) -> PORT_SELECTED_EN_W<DTB5_DIG_ENGrs> {
        PORT_SELECTED_EN_W::new(self, 4)
    }
    ///Bit 5 - force port_selected\[0\] signal
    #[inline(always)]
    pub fn port_selected_0(&mut self) -> PORT_SELECTED_0_W<DTB5_DIG_ENGrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WB07.html#RADIO:DTB5_DIG_ENG)*/
