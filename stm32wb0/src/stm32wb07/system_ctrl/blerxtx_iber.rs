...
impl W {
    ///Bit 0 - TX_IBE: interrupt edge register on TX_SEQUENCE signal: 0: detection on single edge (default). 1: detection on both edges
    #[inline(always)]
    pub fn tx_ibe(&mut self) -> TX_IBE_W<BLERXTX_IBERrs> {
        TX_IBE_W::new(self, 0)
    }
    ///Bit 1 - RX_IBE: interrupt edge register on RX_SEQUENCE signal: 0: detection on single edge (default). 1: detection on both edges
    #[inline(always)]
    pub fn rx_ibe(&mut self) -> RX_IBE_W<BLERXTX_IBERrs> {
        RX_IBE_W::new(self, 1)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WB07.html#SYSTEM_CTRL:BLERXTX_IBER)*/
