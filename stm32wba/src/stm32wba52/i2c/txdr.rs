...
///Field `TXDATA` reader - 8-bit transmit data Data byte to be transmitted to the Isup2/supC bus Note: These bits can be written only when TXE = 1.
pub type TXDATA_R = crate::FieldReader;
///Field `TXDATA` writer - 8-bit transmit data Data byte to be transmitted to the Isup2/supC bus Note: These bits can be written only when TXE = 1.
pub type TXDATA_W<'a, REG> = crate::FieldWriter<'a, REG, 8, u8, crate::Safe>;
impl R {
    ///Bits 0:7 - 8-bit transmit data Data byte to be transmitted to the Isup2/supC bus Note: These bits can be written only when TXE = 1.
    #[inline(always)]
...
impl W {
    ///Bits 0:7 - 8-bit transmit data Data byte to be transmitted to the Isup2/supC bus Note: These bits can be written only when TXE = 1.
    #[inline(always)]
    pub fn txdata(&mut self) -> TXDATA_W<'_, TXDRrs> {
        TXDATA_W::new(self, 0)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WBA52.html#I2C:TXDR)*/
pub struct TXDRrs;
