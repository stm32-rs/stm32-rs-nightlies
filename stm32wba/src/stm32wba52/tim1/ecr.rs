...
///Field `IBLK` reader - Index blanking This bit indicates if the Index event is conditioned by the tim_ti3 or tim_ti4 input
pub type IBLK_R = crate::FieldReader;
///Field `IBLK` writer - Index blanking This bit indicates if the Index event is conditioned by the tim_ti3 or tim_ti4 input
...
impl W {
    ///Bit 0 - Index enable This bit indicates if the Index event resets the counter.
    #[inline(always)]
    pub fn ie(&mut self) -> IE_W<'_, ECRrs> {
        IE_W::new(self, 0)
    }
    ///Bits 1:2 - Index direction This bit indicates in which direction the Index event resets the counter. Note: The IDR\[1:0\] bitfield must be written when IE bit is reset (index disabled).
    #[inline(always)]
    pub fn idir(&mut self) -> IDIR_W<'_, ECRrs> {
        IDIR_W::new(self, 1)
    }
    ///Bits 3:4 - Index blanking This bit indicates if the Index event is conditioned by the tim_ti3 or tim_ti4 input
    #[inline(always)]
    pub fn iblk(&mut self) -> IBLK_W<'_, ECRrs> {
        IBLK_W::new(self, 3)
    }
    ///Bit 5 - First index This bit indicates if the first index only is taken into account
    #[inline(always)]
    pub fn fidx(&mut self) -> FIDX_W<'_, ECRrs> {
        FIDX_W::new(self, 5)
    }
    ///Bits 6:7 - Index positioning In quadrature encoder mode (SMS\[3:0\] = 0001, 0010, 0011, 1110, 1111), this bit indicates in which AB input configuration the Index event resets the counter. In directional clock mode or clock plus direction mode (SMS\[3:0\] = 1010, 1011, 1100, 1101), these bits indicates on which level the Index event resets the counter. In bidirectional clock mode, this applies for both clock inputs. x0: Index resets the counter when clock is 0 x1: Index resets the counter when clock is 1 Note: IPOS\[1\] bit is not significant
    #[inline(always)]
    pub fn ipos(&mut self) -> IPOS_W<'_, ECRrs> {
        IPOS_W::new(self, 6)
    }
    ///Bits 16:23 - Pulse width This bitfield defines the pulse duration, as following: tsubPW/sub = PW\[7:0\] x tsubPWG/sub
    #[inline(always)]
    pub fn pw(&mut self) -> PW_W<'_, ECRrs> {
        PW_W::new(self, 16)
    }
    ///Bits 24:26 - Pulse width prescaler This bitfield sets the clock prescaler for the pulse generator, as following: tsubPWG/sub = (2sup(PWPRSC\[2:0\])/sup) x tsubtim_ker_ck/sub
    #[inline(always)]
    pub fn pwprsc(&mut self) -> PWPRSC_W<'_, ECRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WBA52.html#TIM1:ECR)*/
pub struct ECRrs;
