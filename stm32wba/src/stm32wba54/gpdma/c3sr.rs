///Register `C3SR` reader
pub type R = crate::R<C3SRrs>;
...
///Field `TCF` reader - transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM\[1:0\]).
pub type TCF_R = crate::BitReader;
///Field `HTF` reader - half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT\[15:0\]/2) has been transferred to the destination.
...
impl R {
    ...
    ///Bit 8 - transfer complete flag A transfer complete event is either a block transfer complete, or a LLI transfer complete including the upload of the next LLI if any, or the full linked-list completion, depending on the transfer complete event mode (GPDMA_CxTR2.TCEM\[1:0\]).
    #[inline(always)]
    pub fn tcf(&self) -> TCF_R {
        TCF_R::new(((self.bits >> 8) & 1) != 0)
    }
    ///Bit 9 - half transfer flag A half block transfer occurs when half of the bytes of the source block size (rounded up integer of GPDMA_CxBR1.BNDT\[15:0\]/2) has been transferred to the destination.
    #[inline(always)]
...
You can [`read`](crate::Reg::read) this register and get [`c3sr::R`](R). See [API](https://docs.rs/svd2rust/#read--modify--write-api).

See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WBA54.html#GPDMA:C3SR)*/
pub struct C3SRrs;
impl crate::RegisterSpec for C3SRrs {
    type Ux = u32;
}
///`read()` method returns [`c3sr::R`](R) reader structure
impl crate::Readable for C3SRrs {}
///`reset()` method sets C3SR to value 0x01
impl crate::Resettable for C3SRrs {
