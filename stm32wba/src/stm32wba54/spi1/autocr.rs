...
///Field `TRIGSEL` reader - trigger selection (refer Section: Description of SPI interconnections). ... Note: these bits can be written only when SPE=0.
pub type TRIGSEL_R = crate::FieldReader;
///Field `TRIGSEL` writer - trigger selection (refer Section: Description of SPI interconnections). ... Note: these bits can be written only when SPE=0.
...
/**HW control of CSTART triggering enable Note: if user cannot prevent trigger event during write, the TRIGEN must be changed when SPI is disabled
...
///Field `TRIGEN` reader - HW control of CSTART triggering enable Note: if user cannot prevent trigger event during write, the TRIGEN must be changed when SPI is disabled
...
///Field `TRIGEN` writer - HW control of CSTART triggering enable Note: if user cannot prevent trigger event during write, the TRIGEN must be changed when SPI is disabled
...
impl R {
    ///Bits 16:19 - trigger selection (refer Section: Description of SPI interconnections). ... Note: these bits can be written only when SPE=0.
    #[inline(always)]
    ...
    ///Bit 21 - HW control of CSTART triggering enable Note: if user cannot prevent trigger event during write, the TRIGEN must be changed when SPI is disabled
    #[inline(always)]
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WBA54.html#SPI1:AUTOCR)*/
pub struct AUTOCRrs;
