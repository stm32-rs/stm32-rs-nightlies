...
///Field `SPLCK(0-0)` reader - Security/privilege configuration lock super-block; This bit is set by software and can be cleared only by system reset.; note that bit \[3:2\] are reserved on sales type STM32WBA5xEx for MPCBB1.
pub type SPLCK_R = crate::BitReader;
///Field `SPLCK(0-0)` writer - Security/privilege configuration lock super-block; This bit is set by software and can be cleared only by system reset.; note that bit \[3:2\] are reserved on sales type STM32WBA5xEx for MPCBB1.
