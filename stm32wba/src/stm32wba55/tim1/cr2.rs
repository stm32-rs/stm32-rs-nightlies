...
pub type MMS_W<'a, REG> = crate::FieldWriter<'a, REG, 3, u8, crate::Safe>;
/**tim_ti1 selection
...
///Field `MMS_3` reader - MMS\[3\]
pub type MMS_3_R = crate::BitReader;
///Field `MMS_3` writer - MMS\[3\]
...
impl W {
    ...
    ///Bit 15 - Output Idle state (OC4N output)
    #[inline(always)]
    pub fn ois4n(&mut self) -> OISN_W<'_, CR2rs> {
        OISN_W::new(self, 15)
    }
    ///Bits 20:23 - Master mode selection 2 These bits allow the information to be sent to ADC for synchronization (tim_trgo2) to be selected. The combination is as follows: Note: The clock of the slave timer or ADC must be enabled prior to receive events from the master timer, and must not be changed on-the-fly while triggers are received from the master timer.
    #[inline(always)]
    ...
    ///Bit 25 - MMS\[3\]
    #[inline(always)]
    pub fn mms_3(&mut self) -> MMS_3_W<'_, CR2rs> {
        MMS_3_W::new(self, 25)
    }
}
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WBA55.html#TIM1:CR2)*/
pub struct CR2rs;
