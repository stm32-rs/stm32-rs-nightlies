...
///Field `PDB` reader - PDB\[x\]: Pull Down Port B Pull Down activation on port B\[i\] pad when APC bit of PWRC CR1 is set - 1: Pull-Down activated on Port B\[i\] when APC bit of PWRC CR1 bit is set - 0: Pull-Down not activated on Port B\[i\]
pub type PDB_R = crate::FieldReader<u16>;
///Field `PDB` writer - PDB\[x\]: Pull Down Port B Pull Down activation on port B\[i\] pad when APC bit of PWRC CR1 is set - 1: Pull-Down activated on Port B\[i\] when APC bit of PWRC CR1 bit is set - 0: Pull-Down not activated on Port B\[i\]
pub type PDB_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    ///Bits 0:15 - PDB\[x\]: Pull Down Port B Pull Down activation on port B\[i\] pad when APC bit of PWRC CR1 is set - 1: Pull-Down activated on Port B\[i\] when APC bit of PWRC CR1 bit is set - 0: Pull-Down not activated on Port B\[i\]
    #[inline(always)]
