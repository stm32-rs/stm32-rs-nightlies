...
///Field `PUB` reader - PUB\[x\] : Pull Up Port B Pull up activation on port B\[i\] pad when APC bit of PWRC CR1 is set - 1: Pull-Up activated on port B\[i\] when APC bit of PWRC CR1 bit is set and PWR_PDCRB\[x\] is reset - 0: Pull-Up not activated on port B\[i\]
pub type PUB_R = crate::FieldReader<u16>;
///Field `PUB` writer - PUB\[x\] : Pull Up Port B Pull up activation on port B\[i\] pad when APC bit of PWRC CR1 is set - 1: Pull-Up activated on port B\[i\] when APC bit of PWRC CR1 bit is set and PWR_PDCRB\[x\] is reset - 0: Pull-Up not activated on port B\[i\]
pub type PUB_W<'a, REG> = crate::FieldWriter<'a, REG, 16, u16>;
impl R {
    ///Bits 0:15 - PUB\[x\] : Pull Up Port B Pull up activation on port B\[i\] pad when APC bit of PWRC CR1 is set - 1: Pull-Up activated on port B\[i\] when APC bit of PWRC CR1 bit is set and PWR_PDCRB\[x\] is reset - 0: Pull-Up not activated on port B\[i\]
    #[inline(always)]
...
///`reset()` method sets PUCRB to value 0xffff
