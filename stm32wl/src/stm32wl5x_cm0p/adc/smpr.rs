...
///Field `SMPSEL(0-17)` reader - Channel-%s sampling time selection
...
///Field `SMPSEL(0-17)` writer - Channel-%s sampling time selection
...
impl R {
    ...
    ///Channel-(0-17) sampling time selection
    ///
    ...
    pub fn smpsel_iter(&self) -> impl Iterator<Item = SMPSEL_R> + '_ {
        (0..18).map(move |n| SMPSEL_R::new(((self.bits >> (n + 8)) & 1) != 0))
    }
