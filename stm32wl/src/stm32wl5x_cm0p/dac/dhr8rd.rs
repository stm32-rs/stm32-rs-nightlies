...
///Field `DACCDHR(1-1)` reader - DAC channel%s 8-bit right-aligned data
pub type DACCDHR_R = crate::FieldReader;
///Field `DACCDHR(1-1)` writer - DAC channel%s 8-bit right-aligned data
pub type DACCDHR_W<'a, REG> = crate::FieldWriter<'a, REG, 8, u8, crate::Safe>;
impl R {
    ///DAC channel(1-1) 8-bit right-aligned data
    ///
    ...
    pub fn daccdhr(&self, n: u8) -> DACCDHR_R {
        ...
        [(); 1][n as usize];
        DACCDHR_R::new(((self.bits >> (n * 0)) & 0xff) as u8)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) 8-bit right-aligned data
    #[inline(always)]
    pub fn daccdhr_iter(&self) -> impl Iterator<Item = DACCDHR_R> + '_ {
        (0..1).map(move |n| DACCDHR_R::new(((self.bits >> (n * 0)) & 0xff) as u8))
    }
...
impl W {
    ...
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `DACC1DHR` field.</div>
    #[inline(always)]
    pub fn daccdhr(&mut self, n: u8) -> DACCDHR_W<'_, DHR8RDrs> {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        DACCDHR_W::new(self, n * 0)
    }
    ///Bits 0:7 - DAC channel1 8-bit right-aligned data
    #[inline(always)]
    pub fn dacc1dhr(&mut self) -> DACCDHR_W<'_, DHR8RDrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WL5X_CM0P.html#DAC:DHR8RD)*/
pub struct DHR8RDrs;
