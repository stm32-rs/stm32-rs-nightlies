...
///Field `TREFRESH(1-1)` reader - DAC channel%s refresh time (only valid in Sample and hold mode)
pub type TREFRESH_R = crate::FieldReader;
///Field `TREFRESH(1-1)` writer - DAC channel%s refresh time (only valid in Sample and hold mode)
pub type TREFRESH_W<'a, REG> = crate::FieldWriter<'a, REG, 8, u8, crate::Safe>;
impl R {
    ///DAC channel(1-1) refresh time (only valid in Sample and hold mode)
    ///
    ...
    pub fn trefresh(&self, n: u8) -> TREFRESH_R {
        ...
        [(); 1][n as usize];
        TREFRESH_R::new(((self.bits >> (n * 0)) & 0xff) as u8)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) refresh time (only valid in Sample and hold mode)
    #[inline(always)]
    pub fn trefresh_iter(&self) -> impl Iterator<Item = TREFRESH_R> + '_ {
        (0..1).map(move |n| TREFRESH_R::new(((self.bits >> (n * 0)) & 0xff) as u8))
    }
...
impl W {
    ...
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `TREFRESH1` field.</div>
    #[inline(always)]
    pub fn trefresh(&mut self, n: u8) -> TREFRESH_W<'_, SHRRrs> {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        TREFRESH_W::new(self, n * 0)
    }
    ///Bits 0:7 - DAC channel1 refresh time (only valid in Sample and hold mode)
    #[inline(always)]
    pub fn trefresh1(&mut self) -> TREFRESH_W<'_, SHRRrs> {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WL5X_CM0P.html#DAC:SHRR)*/
pub struct SHRRrs;
