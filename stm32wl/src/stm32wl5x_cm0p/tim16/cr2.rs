...
///Field `OIS(1-1)` reader - Output Idle state (OC%s output)
...
///Field `OIS(1-1)` writer - Output Idle state (OC%s output)
...
impl R {
    ...
    ///Output Idle state (OC(1-1) output)
    ///
    ...
    pub fn ois(&self, n: u8) -> OIS_R {
        ...
        [(); 1][n as usize];
        OIS_R::new(((self.bits >> (n * 0 + 8)) & 1) != 0)
    }
    ///Iterator for array of:
    ///Output Idle state (OC(1-1) output)
    #[inline(always)]
    pub fn ois_iter(&self) -> impl Iterator<Item = OIS_R> + '_ {
        (0..1).map(move |n| OIS_R::new(((self.bits >> (n * 0 + 8)) & 1) != 0))
    }
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WL5X_CM0P.html#TIM16:CR2)*/
pub struct CR2rs;
