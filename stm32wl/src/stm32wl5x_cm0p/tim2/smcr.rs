    ...
    REG::Ux: From<u8>,
{
    ///Slave mode disabled - if CEN = ‘1 then the prescaler is clocked directly by the internal clock. If SMS\[3\]=1 then Combined reset + trigger mode - Rising edge of the selected trigger input (TRGI) reinitializes the counter, generates an update of the registers and starts the counter
    #[inline(always)]
    pub fn disabled_or_combined(self) -> &'a mut crate::W<REG> {
        self.variant(SMS::DisabledOrCombined)
    }
    ///Encoder mode 1 - Counter counts up/down on TI2FP1 edge depending on TI1FP2 level
    #[inline(always)]
...
impl From<SMS_3> for bool {
    #[inline(always)]
    fn from(variant: SMS_3) -> Self {
        variant as u8 != 0
    }
}
///Field `SMS_3` reader - Slave mode selection - bit 3
pub type SMS_3_R = crate::BitReader<SMS_3>;
impl SMS_3_R {
    ///Get enumerated values variant
    #[inline(always)]
    pub const fn variant(&self) -> SMS_3 {
        match self.bits {
            false => SMS_3::Disabled,
            true => SMS_3::CombinedResetTrigger,
        }
    }
    ...
    pub fn is_combined_reset_trigger(&self) -> bool {
        *self == SMS_3::CombinedResetTrigger
    }
}
///Field `SMS_3` writer - Slave mode selection - bit 3
pub type SMS_3_W<'a, REG> = crate::BitWriter<'a, REG, SMS_3>;
impl<'a, REG> SMS_3_W<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
{
    ///Slave mode disabled (see SMS\[0:2\])
    #[inline(always)]
    pub fn disabled(self) -> &'a mut crate::W<REG> {
        self.variant(SMS_3::Disabled)
    }
    ///SMS\[0:2\] must be 0b000 (DisabledOrCombined). Combined reset + trigger mode - Rising edge of the selected trigger input (TRGI) reinitializes the counter, generates an update of the registers and starts the counter
    #[inline(always)]
    pub fn combined_reset_trigger(self) -> &'a mut crate::W<REG> {
        self.variant(SMS_3::CombinedResetTrigger)
    }
}
impl R {
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WL5X_CM0P.html#TIM2:SMCR)*/
pub struct SMCRrs;
