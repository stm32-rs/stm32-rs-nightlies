    ...
    REG::Ux: From<u8>,
{
    ...
    ///LPTIM3 OUT event
    #[inline(always)]
    pub fn lptim3out(self) -> &'a mut crate::W<REG> {
...
///Field `CEN(1-1)` reader - DAC channel%s calibration enable
...
///Field `CEN(1-1)` writer - DAC channel%s calibration enable
...
impl R {
    ...
    pub fn ten(&self, n: u8) -> TEN_R {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        TEN_R::new(((self.bits >> (n * 0 + 1)) & 1) != 0)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) trigger enable
    #[inline(always)]
    pub fn ten_iter(&self) -> impl Iterator<Item = TEN_R> + '_ {
        (0..1).map(move |n| TEN_R::new(((self.bits >> (n * 0 + 1)) & 1) != 0))
    }
    ...
    ///DAC channel(1-1) calibration enable
    ///
    ...
    pub fn cen(&self, n: u8) -> CEN_R {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        CEN_R::new(((self.bits >> (n * 0 + 14)) & 1) != 0)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) calibration enable
    #[inline(always)]
    pub fn cen_iter(&self) -> impl Iterator<Item = CEN_R> + '_ {
        (0..1).map(move |n| CEN_R::new(((self.bits >> (n * 0 + 14)) & 1) != 0))
    }
