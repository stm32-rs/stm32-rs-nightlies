...
///Field `MODE(1-1)` reader - DAC channel%s mode
...
///Field `MODE(1-1)` writer - DAC channel%s mode
...
impl R {
    ...
    pub fn mode(&self, n: u8) -> MODE_R {
        ...
        [(); 1][n as usize];
        MODE_R::new(((self.bits >> (n * 0)) & 7) as u8)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) mode
    #[inline(always)]
    pub fn mode_iter(&self) -> impl Iterator<Item = MODE_R> + '_ {
        (0..1).map(move |n| MODE_R::new(((self.bits >> (n * 0)) & 7) as u8))
    }
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WL5X_CM4.html#DAC:MCR)*/
pub struct MCRrs;
