...
///Field `DMAUDR(1-1)` reader - DAC channel%s DMA underrun flag
...
///Field `DMAUDR(1-1)` writer - DAC channel%s DMA underrun flag
...
///Field `CAL_FLAG(1-1)` reader - DAC channel%s calibration offset status
...
///Field `BWST(1-1)` reader - DAC channel%s busy writing sample time flag
...
impl R {
    ///DAC channel(1-1) DMA underrun flag
    ///
    ...
    pub fn dmaudr(&self, n: u8) -> DMAUDR_R {
        ...
        [(); 1][n as usize];
        DMAUDR_R::new(((self.bits >> (n * 0 + 13)) & 1) != 0)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) DMA underrun flag
    #[inline(always)]
    pub fn dmaudr_iter(&self) -> impl Iterator<Item = DMAUDR_R> + '_ {
        (0..1).map(move |n| DMAUDR_R::new(((self.bits >> (n * 0 + 13)) & 1) != 0))
    }
    ...
    ///DAC channel(1-1) calibration offset status
    ///
    ...
    pub fn cal_flag(&self, n: u8) -> CAL_FLAG_R {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        CAL_FLAG_R::new(((self.bits >> (n * 0 + 14)) & 1) != 0)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) calibration offset status
    #[inline(always)]
    pub fn cal_flag_iter(&self) -> impl Iterator<Item = CAL_FLAG_R> + '_ {
        (0..1).map(move |n| CAL_FLAG_R::new(((self.bits >> (n * 0 + 14)) & 1) != 0))
    }
    ...
    ///DAC channel(1-1) busy writing sample time flag
    ///
    ...
    pub fn bwst(&self, n: u8) -> BWST_R {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        BWST_R::new(((self.bits >> (n * 0 + 15)) & 1) != 0)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) busy writing sample time flag
    #[inline(always)]
    pub fn bwst_iter(&self) -> impl Iterator<Item = BWST_R> + '_ {
        (0..1).map(move |n| BWST_R::new(((self.bits >> (n * 0 + 15)) & 1) != 0))
    }
...
See register [structure](https://stm32-rs.github.io/stm32-rs/STM32WL5X_CM4.html#DAC:SR)*/
pub struct SRrs;
