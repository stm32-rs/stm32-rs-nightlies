...
///Field `OTRIM(1-1)` reader - DAC channel%s offset trimming value
pub type OTRIM_R = crate::FieldReader;
///Field `OTRIM(1-1)` writer - DAC channel%s offset trimming value
pub type OTRIM_W<'a, REG> = crate::FieldWriter<'a, REG, 5, u8, crate::Safe>;
impl R {
    ///DAC channel(1-1) offset trimming value
    ///
    ...
    pub fn otrim(&self, n: u8) -> OTRIM_R {
        ...
        [(); 1][n as usize];
        OTRIM_R::new(((self.bits >> (n * 0)) & 0x1f) as u8)
    }
    ///Iterator for array of:
    ///DAC channel(1-1) offset trimming value
    #[inline(always)]
    pub fn otrim_iter(&self) -> impl Iterator<Item = OTRIM_R> + '_ {
        (0..1).map(move |n| OTRIM_R::new(((self.bits >> (n * 0)) & 0x1f) as u8))
    }
...
impl W {
    ...
    ///<div class="warning">`n` is number of field in register. `n == 0` corresponds to `OTRIM1` field.</div>
    #[inline(always)]
    pub fn otrim(&mut self, n: u8) -> OTRIM_W<'_, CCRrs> {
        #[allow(clippy::no_effect)]
        [(); 1][n as usize];
        OTRIM_W::new(self, n * 0)
    }
    ///Bits 0:4 - DAC channel1 offset trimming value
    #[inline(always)]
    pub fn otrim1(&mut self) -> OTRIM_W<'_, CCRrs> {
        OTRIM_W::new(self, 0)
    }
}
